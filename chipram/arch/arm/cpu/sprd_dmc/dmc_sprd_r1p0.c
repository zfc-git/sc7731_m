...
#define PD_SELECT_AC 	4
#define PD_SELECT_DS0 	4
#define PD_SELECT_DS1 	4
#define PD_SELECT_DS2 	4
#define PD_SELECT_DS3 	4
...
static void dmc_pub_pin_remap(void)
{
	...
	for(i = 0; i < MAX_CHANNEL_NUM; i++)
	{
		switch(chipsize)
		{
			/*4G bit*/
			case 0x20000000:
				REG32(0x300e3008+i*4) = 0x98989898;
				break;
			/*6G bit*/
			case 0x30000000:
				REG32(0x300e3008+i*4) = 0x8a988a98;
				break;
			/*8G bit*/
			case 0x40000000:
				REG32(0x300e3008+i*4) = 0xba98ba98;
				break;
			/*12G bit*/
			case 0x60000000:
				if(i == 9)//dsp port
				...
				{
					REG32(0x300e3008+i*4) = 0x98dcba98;
				}
				break;
			/*16G bit*/
			case 0x80000000:
			default:
				...
				{
					REG32(0x300e3008+i*4) = 0xfedcba98;
				}
				break;
		}
	}
...
int update_cs_num(void)
{
	...
	{
		cs_num++;
		if((mr5_cs[0] == mr5_cs[1]) && ((mr8_cs[0]&0xc3) == (mr8_cs[1]&0xc3)))
		{
			/*lpddr3*/
			if(3 == (mr8_cs[0]&0x3))
			{
...
void save_param_for_uboot(void)
{
	...
	else if(ddr_chip_cur.cs_num == 1)
	{
		if (0x80000000 == total_size)
		{
			pMem->cs0_size -= SYS_MAP_RESERVE_SIZE + BIST_RESERVE_SIZE;
		}
		else
		{
			pMem->cs0_size -= BIST_RESERVE_SIZE;
		}
	}
...
void dll_fix_delay_line(void)
{
	PIKE_DMC_REG_INFO_PTR pdmc = (PIKE_DMC_REG_INFO_PTR)DMC_REG_ADDR_BASE_PHY;

	/*fix dll delay line val*/
	pdmc->dmc_clkwr_dll_ac = 0x00000011;

	pdmc->dmc_clkwr_dll_ds0 = 0x00000014;
	pdmc->dmc_dqsin_pos_dll_ds0 = 0x0000000E;
	pdmc->dmc_dqsin_neg_dll_ds0 = 0x0000000E;

	pdmc->dmc_clkwr_dll_ds1 = 0x00000014;
	pdmc->dmc_dqsin_pos_dll_ds1 = 0x0000000E;
	pdmc->dmc_dqsin_neg_dll_ds1 = 0x0000000E;

	pdmc->dmc_clkwr_dll_ds2 = 0x00000014;
	pdmc->dmc_dqsin_pos_dll_ds2 = 0x0000000E;
	pdmc->dmc_dqsin_neg_dll_ds2 = 0x0000000E;

	pdmc->dmc_clkwr_dll_ds3 = 0x00000014;
	pdmc->dmc_dqsin_pos_dll_ds3 = 0x0000000E;
	pdmc->dmc_dqsin_neg_dll_ds3 = 0x0000000E;

	pdmc->dmc_bist[15] |= 0x10004000;/*dll return -4*/
}
...
//	dmc_print_str("DDR init begin...\r\n");
	...
	if (0 != dmc_pin_init())
	{
		...
		pdmc->dmc_bist[15] |= 0x10000001;/*return -1*/
	}
	sdram_clk_init();
#ifdef DDR_AUTO_DETECT
	...
	if (0 != sdram_final_init())
	{
		...
		pdmc->dmc_bist[15] |= 0x10050000;
	}
#if 1
	if(!((pdmc->dmc_bist[15]>>12)&0x4))	/*dll init return value is not -4*/
	{
		if(0 != ddr_scan_online_sprd_r1p0())
		{
			dmc_print_str("error:-6\r\n");
			pdmc->dmc_bist[15] |= 0x10600000;
		}
		dmc_bist_disable();
	}
#else
