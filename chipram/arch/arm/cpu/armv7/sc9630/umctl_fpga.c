...
#include <asm/arch-sc9630/umctl2_reg_fpga.h>
#include <asm/arch-sc9630/dram_phy_fpga.h>
...
typedef enum
{
	IO_BUSWIDTH_32BIT = 0x0,
	IO_BUSWIDTH_16BIT,
	IO_BUSWIDTH_8BIT
}IO_BUSWIDTH_E;
void dmc_reg_bit_set(uint32 addr,uint32 start_bit,uint32 bits_num,
							uint32 val)
{
...
void dmc_glb_ops(uint32 isreset)
{
	volatile uint32 val,i;
	if(isreset)
	{
		val = REG32(REG_AON_TOP_DDR_SLEEP_CTRL);
		val |=  (7 << 0);
		REG32(REG_AON_TOP_DDR_SLEEP_CTRL) = val;
		for(i = 0; i < 1000; i++);
		...
		val = REG32(REG_AON_TOP_DDR_PHY_RET_CFG);
		val |= (1 << 16);
		REG32(REG_AON_TOP_DDR_PHY_RET_CFG) = val;
		for(i = 0; i < 1000; i++);
...
void umctl2_core_init(MEM_TYPE_E mem_type,IO_BUSWIDTH_E io_buswidth)
{
	volatile uint32 val;
	REG32(UMCTL_DFIMISC) = 0;
	while(REG32(UMCTL_STAT) & 0X03);
	val = REG32(UMCTL_MSTR);

	dmc_reg_bit_set((uint32)&val,12,2,io_buswidth);
	switch(mem_type)
	{
		case LPDDR2:
			dmc_reg_bit_set((uint32)&val,24,4,0x03);
			dmc_reg_bit_set((uint32)&val,16,4,0x02);
			dmc_reg_bit_set((uint32)&val,8,1,0x00);
			dmc_reg_bit_set((uint32)&val,0,4,0x04);
		break;
		case DDR3:
			dmc_reg_bit_set((uint32)&val,24,4,0x03);
			dmc_reg_bit_set((uint32)&val,16,4,0x04);
			dmc_reg_bit_set((uint32)&val,8,1,0x00);
			dmc_reg_bit_set((uint32)&val,0,4,0x01);
		break;
		case LPDDR3:
			dmc_reg_bit_set((uint32)&val,24,4,0x03);
			dmc_reg_bit_set((uint32)&val,16,4,0x04);
			dmc_reg_bit_set((uint32)&val,8,1,0x00);
			dmc_reg_bit_set((uint32)&val,0,4,0x08);
		break;
		default:
			dmc_reg_bit_set((uint32)&val,24,4,0x03);
			dmc_reg_bit_set((uint32)&val,16,4,0x02);
	...
	dmc_reg_bit_set(UMCTL_MSTR,0,32,val);
}
void umctl2_init_seq(MEM_TYPE_E mem_type)
{
	switch(mem_type)
	{
		case LPDDR2:
			dmc_reg_bit_set(UMCTL_INIT0,0,32,0x00020002);
			dmc_reg_bit_set(UMCTL_INIT1,0,32,0x0002080d);
			dmc_reg_bit_set(UMCTL_INIT2,0,32,0x00000e06);
			dmc_reg_bit_set(UMCTL_INIT3,0,32,0x00820004);
			dmc_reg_bit_set(UMCTL_INIT4,0,32,0x00010000);
			dmc_reg_bit_set(UMCTL_INIT5,0,32,0x000e0005);
		break;
		case DDR3:
			dmc_reg_bit_set(UMCTL_INIT0,0,32,0x00020002);
			dmc_reg_bit_set(UMCTL_INIT1,0,32,0x0002080d);
			dmc_reg_bit_set(UMCTL_INIT2,0,32,0x00000e06);
			dmc_reg_bit_set(UMCTL_INIT3,0,32,0x00120003);
			dmc_reg_bit_set(UMCTL_INIT4,0,32,0x00080000);
			dmc_reg_bit_set(UMCTL_INIT5,0,32,0x00120005);
		break;
		case LPDDR3:
			dmc_reg_bit_set(UMCTL_INIT0,0,32,0x00020002);
			dmc_reg_bit_set(UMCTL_INIT1,0,32,0x0002110d);
			dmc_reg_bit_set(UMCTL_INIT2,0,32,0x00001106);
			dmc_reg_bit_set(UMCTL_INIT3,0,32,0x00830004);
			dmc_reg_bit_set(UMCTL_INIT4,0,32,0x00010000);
			dmc_reg_bit_set(UMCTL_INIT5,0,32,0x000E0005);
		break;
		default:
			...
			dmc_reg_bit_set(UMCTL_INIT2,0,32,0x00000e06);
			dmc_reg_bit_set(UMCTL_INIT3,0,32,0x00820004);
			dmc_reg_bit_set(UMCTL_INIT4,0,32,0x00020000);
			dmc_reg_bit_set(UMCTL_INIT5,0,32,0x000E0005);
		break;
	}
	dmc_reg_bit_set(UMCTL_DIMMCTL,0,32,0x00000000);
	dmc_reg_bit_set(UMCTL_RANKCTL,0,32,0x00000663);
}
void umctl2_timing_init(MEM_TYPE_E mem_type)
{
	switch(mem_type)
	{
		case LPDDR2:
			REG32(UMCTL_DRAMTMG0) = 0x0f147f11;
			REG32(UMCTL_DRAMTMG1) = 0x00040413;
			REG32(UMCTL_DRAMTMG2) = 0x0306090a;
			REG32(UMCTL_DRAMTMG3) = 0x00502000;
			REG32(UMCTL_DRAMTMG4) = 0x0a04040a;
			REG32(UMCTL_DRAMTMG5) = 0x02020606;
			REG32(UMCTL_DRAMTMG6) = 0x02020005;
			REG32(UMCTL_DRAMTMG7) = 0x00000202;
			REG32(UMCTL_DRAMTMG8) = 0x00000003;

			dmc_reg_bit_set(UMCTL_RFSHCTL0,4,5,3);
			REG32(UMCTL_RFSHTMG) = 0x000C0007;
		break;
		case DDR3:
			REG32(UMCTL_DRAMTMG0) = 0x13147f11;
			REG32(UMCTL_DRAMTMG1) = 0x00050413;
			REG32(UMCTL_DRAMTMG2) = 0x0606090F;
			REG32(UMCTL_DRAMTMG3) = 0x0050400C;
			REG32(UMCTL_DRAMTMG4) = 0x0a04040a;
			REG32(UMCTL_DRAMTMG5) = 0x06060606;
			REG32(UMCTL_DRAMTMG6) = 0x00000000;
			REG32(UMCTL_DRAMTMG7) = 0x00000000;
			REG32(UMCTL_DRAMTMG8) = 0x00000010;

			REG32(UMCTL_RFSHTMG) = 0x000C000A;
		break;
		case LPDDR3:
			REG32(UMCTL_DRAMTMG0) = 0x111B2517;
			REG32(UMCTL_DRAMTMG1) = 0x00040320;
			REG32(UMCTL_DRAMTMG2) = 0x0306090A;
			REG32(UMCTL_DRAMTMG3) = 0x00A04000;
			REG32(UMCTL_DRAMTMG4) = 0x0804060a;
			REG32(UMCTL_DRAMTMG5) = 0x02020803;
			REG32(UMCTL_DRAMTMG6) = 0x02020006;
			REG32(UMCTL_DRAMTMG7) = 0x00000202;
			REG32(UMCTL_DRAMTMG8) = 0x00000003;

			REG32(UMCTL_RFSHTMG) = 0x00410046;
		break;
		default:
			REG32(UMCTL_DRAMTMG0) = 0x0c147f11;
			REG32(UMCTL_DRAMTMG1) = 0x00030313;
			REG32(UMCTL_DRAMTMG2) = 0x03060909;
			REG32(UMCTL_DRAMTMG3) = 0x00502000;
			REG32(UMCTL_DRAMTMG4) = 0x0a03040a;
			REG32(UMCTL_DRAMTMG5) = 0x02020606;
			REG32(UMCTL_DRAMTMG6) = 0x02020005;
			REG32(UMCTL_DRAMTMG7) = 0x00000202;
			REG32(UMCTL_DRAMTMG8) = 0x00000003;

			REG32(UMCTL_RFSHTMG) = 0x00620038;
		break;
	}
	REG32(UMCTL_ZQCTL0) = 0xC0900024;
	REG32(UMCTL_ZQCTL1) = 0x01400070;
	REG32(UMCTL_ZQCTL2) = 0x00000000;

}
void umctl2_dfi_init(MEM_TYPE_E mem_type)
{
	switch(mem_type)
	{
		case LPDDR2:case LPDDR3:
			REG32(UMCTL_DFITMG0) = 0x08040000;
			break;
		case DDR3:
			REG32(UMCTL_DFITMG0) = 0x08020002;
			break;
		default:
			REG32(UMCTL_DFITMG0) = 0x02050103;
			break;
	}
	REG32(UMCTL_DFITMG1) = 0x00030202;
	REG32(UMCTL_DFILPCFG0) = 0x0700F100;
	REG32(UMCTL_DFIUPD0) = 0x80400003;
	REG32(UMCTL_DFIUPD1) = 0x001A0021;
	REG32(UMCTL_DFIUPD2) = 0x026904C9;
	REG32(UMCTL_DFIUPD3) = 0x030E051F;
}
void umctl2_addrmapping_cfg(MEM_TYPE_E mem_type)
{
	switch(mem_type)
	{
		...
		case LPDDR2:
			REG32(UMCTL_ADDRMAP0) = 0x00001f15;
			REG32(UMCTL_ADDRMAP1) = 0x00080808;
			REG32(UMCTL_ADDRMAP2) = 0x00000000;
			REG32(UMCTL_ADDRMAP3) = 0x00000000;
			REG32(UMCTL_ADDRMAP4) = 0x00000f0f;
			REG32(UMCTL_ADDRMAP5) = 0x07070707;
			REG32(UMCTL_ADDRMAP6) = 0x0f0f0707;
		break;
#else
		case LPDDR2:
			REG32(UMCTL_ADDRMAP0) = 0x00001f14;
			REG32(UMCTL_ADDRMAP1) = 0x00070707;
			REG32(UMCTL_ADDRMAP2) = 0x00000000;
			REG32(UMCTL_ADDRMAP3) = 0x00000000;
			REG32(UMCTL_ADDRMAP4) = 0x00000f0f;
			REG32(UMCTL_ADDRMAP5) = 0x06060606;
			REG32(UMCTL_ADDRMAP6) = 0x0f0f0606;
		break;
#endif
		case DDR3:
			REG32(UMCTL_ADDRMAP0) = 0x00001f15;
			REG32(UMCTL_ADDRMAP1) = 0x00070707;
			REG32(UMCTL_ADDRMAP2) = 0x00000000;
			REG32(UMCTL_ADDRMAP3) = 0x0f000000;
			REG32(UMCTL_ADDRMAP4) = 0x00000f0f;
			REG32(UMCTL_ADDRMAP5) = 0x06060606;
			REG32(UMCTL_ADDRMAP6) = 0x0f0f0606;
		break;
...
void umctl2_misc_init(MEM_TYPE_E mem_type)
{
	if(LPDDR3 == mem_type)
		REG32(UMCTL_ODTCFG) = 0x0400040C;
	else
		REG32(UMCTL_ODTCFG) = 0x02010205;
	switch(mem_type)
	{
		case LPDDR1:case LPDDR2:
			REG32(UMCTL_ODTMAP) = 0x00000000;
		break;
		case DDR3:
			REG32(UMCTL_ODTMAP) = 0x00001221;
		break;
		case LPDDR3:
			REG32(UMCTL_ODTMAP) = 0x00001221;
		break;
	...
	REG32(UMCTL_SCHED) = 0x00070c01;
	REG32(UMCTL_PERFHPR0) = 0x00000000;
	REG32(UMCTL_PERFHPR1) = 0x10000000;
	REG32(UMCTL_PERFLPR0) = 0x00000100;
	REG32(UMCTL_PERFLPR1) = 0x40000100;
	REG32(UMCTL_PERFWR0) = 0x00000000;
	REG32(UMCTL_PERFWR1) = 0x10000000;
}
void umctl2_port_cfg(void)
{
	uint32 idx;
	REG32(UMCTL_PCCFG) = 0x00000011;
	for(idx = 0; idx <= 9; idx++)
	{
		REG32(UMCTL2_REG_(0x0404+(idx)*0xB0)) = 0x0001f000;
		REG32(UMCTL2_REG_(0x0408+(idx)*0xB0)) = 0x00004005;
		REG32(UMCTL2_REG_(0x0490+(idx)*0xB0)) = 0x00000001;
	}
}
void publ_modereg_init(MEM_TYPE_E mem_type)
{
	uint32 tmp_val[3] = {0};
	switch(mem_type)
	{
		case LPDDR2:
			tmp_val[1] |= ((0x2 << 0) | (0x4 << 5));
			tmp_val[2] = 0x04;
		break;
		case DDR3:
			tmp_val[1] = 0x110;
		...
		break;
		case LPDDR3:
			tmp_val[1] |= ((0x3 << 0) | (0x4 << 5));
			tmp_val[2] = 0x04;
		break;
		default:break;
	}
	REG32(PUBL_MR0) = tmp_val[0];
	REG32(PUBL_MR1) = tmp_val[1];
	REG32(PUBL_MR2) = tmp_val[2];
	REG32(PUBL_MR3) = 0x2;
}
void publ_timing_cfg(MEM_TYPE_E mem_type)
{
	uint32 tmp_val[4] = {0};
	tmp_val[3] = REG32(PUBL_DTPR3);
	tmp_val[3] &= ~((0x7 << 0) | (0x7 << 3));
	switch(mem_type)
	{
		case LPDDR2:
			tmp_val[0] = ((0x3 << 0)|(0x4 << 4)|(0xa << 8)|
				(0x6 << 12)|(0x11 << 16)|(0x4 << 22) | (0x1b << 26));
			tmp_val[1] = ((0x2 << 0)|(0x4 << 2)|(0x26 << 5)|
				(0x34 << 11)|(0x28 << 20)|(0x8 << 26) | (0x0 << 30));
			tmp_val[2] = ((0xc8 << 0)|(0x8 << 10)|(0x3 << 15)|
				(0x80 << 19));
			tmp_val[3] = ((0x1 << 0)|(0x3 << 3));
		break;
		case DDR3:
			tmp_val[0] = ((0x3 << 0)|(0x4 << 4)|(0xa << 8)|
				(0x8 << 12)|(0x11 << 16)|(0x4 << 22) | (0x1b << 26));
			tmp_val[1] = ((0x2 << 0)|(0x4 << 2)|(0x26 << 5)|
				(0x46 << 11)|(0x28 << 20)|(0x8 << 26) | (0x0 << 30));
			tmp_val[2] = ((0xc8 << 0)|(0xC << 10)|(0x3 << 15)|
				(0x80 << 19));
			tmp_val[3] = ((0x1 << 0)|(0x3 << 3));
		break;
		case LPDDR3:
			tmp_val[0] = ((0x4 << 0)|(0x4 << 4)|(0xa << 8)|
				(0x8 << 12)|(0x17 << 16)|(0x6 << 22) | (0x21 << 26));
			tmp_val[1] = ((0x2 << 0)|(0x4 << 2)|(0x26 << 5)|
				(0x46 << 11)|(0x28 << 20)|(0x8 << 26) | (0x0 << 30));
			tmp_val[2] = ((0xc8 << 0)|(0x8 << 10)|(0x3 << 15)|
				(0x80 << 19));
	...
	REG32(PUBL_DTPR0) = tmp_val[0];
	REG32(PUBL_DTPR1) = tmp_val[1];
	REG32(PUBL_DTPR2) = tmp_val[2];
	REG32(PUBL_DTPR3) = tmp_val[3];
}
void publ_misc_init(MEM_TYPE_E mem_type)
{
	volatile uint32 tmp_val,i;
	tmp_val = REG32(PUBL_DXCCR);
	tmp_val &= ~((1 << 0)|(1 << 1)|(0xf << 5)|(0xf << 9));
	switch(mem_type)
	{
		case LPDDR2:
			tmp_val |= ((0x0 << 0)|(0x0 << 1)|(0x4 << 5)|(0xc << 9));
		break;
		case DDR3:
			tmp_val |= ((0x1 << 0)|(0x0 << 1)|(0x4 << 5)|(0xc << 9));
		break;
		case LPDDR3:
			tmp_val |= ((0x0 << 0)|(0x0 << 1)|(0x4 << 5)|(0xc << 9));
		break;
	...
	REG32(PUBL_DXCCR) = tmp_val;
	dmc_reg_bit_set(PUBL_PGCR1,9,1,0);

	dmc_reg_bit_set(PUBL_DX0GCR0,30,1,0);
	dmc_reg_bit_set(PUBL_DX1GCR0,30,1,0);
	dmc_reg_bit_set(PUBL_DX2GCR0,30,1,0);
	dmc_reg_bit_set(PUBL_DX3GCR0,30,1,0);

	dmc_reg_bit_set(PUBL_DX0BDLR2,16,5,0x1F);
	dmc_reg_bit_set(PUBL_DX1BDLR2,16,5,0x1F);
	dmc_reg_bit_set(PUBL_DX2BDLR2,16,5,0x1F);
	dmc_reg_bit_set(PUBL_DX3BDLR2,16,5,0x1F);

	while((REG32(PUBL_PGSR0) & 0x03) != 0x3);

	REG32(PUBL_PIR) = (1 << 18) | (1 << 0);
	for(i = 0; i < 1000; i++);

	while((REG32(PUBL_PGSR0) & 0x01) != 0x1);
}

void publ_init(MEM_TYPE_E mem_type)
{
	uint32 tmp_val;

#ifdef CONFIG_ARCH_SCX20L
	REG32(PUBL_IOVCR0) = 0x48484848;
	REG32(PUBL_IOVCR1) = 0X48;
	REG32(PUBL_ACIOCR1) = 0;
	REG32(PUBL_ACIOCR2) = 0;
	REG32(PUBL_ACIOCR3) = 0;
	REG32(PUBL_ACIOCR4) = 0;
	REG32(PUBL_ACIOCR5) = 0;
	REG32(PUBL_DX0GCR1) = 0;
	REG32(PUBL_DX0GCR2) = 0;
	REG32(PUBL_DX0GCR3) = 0;
	REG32(PUBL_DX1GCR1) = 0;
	REG32(PUBL_DX1GCR2) = 0;
	REG32(PUBL_DX1GCR3) = 0;
	REG32(PUBL_DX2GCR1) = 0;
	REG32(PUBL_DX2GCR2) = 0;
	REG32(PUBL_DX2GCR3) = 0;
	REG32(PUBL_DX3GCR1) = 0;
	REG32(PUBL_DX3GCR2) = 0;
	REG32(PUBL_DX3GCR3) = 0;
#endif

	REG32(PUBL_DTAR0) = (0x7 << 28) | (0x1fff << 12) | (0x3f0 << 0);
	REG32(PUBL_PTR0) = 0x05002b54;
	REG32(PUBL_PTR1) = 0x00820020;
	REG32(PUBL_PTR2) = 0x00041084;
	dmc_reg_bit_set(PUBL_PGCR3,28,4,0x0000000f);
	REG32(PUBL_PLLCR) = 0x01000003;

	tmp_val = REG32(PUBL_DSGCR);
	tmp_val |= ((0 << 6) | (1 << 19));
	REG32(PUBL_DSGCR) = tmp_val;

	tmp_val = REG32(PUBL_DCR);
	tmp_val &= ~((0x7 << 0) | (0x1 << 3));
	...
	REG32(PUBL_DCR) = tmp_val;
	publ_modereg_init(mem_type);
	publ_timing_cfg(mem_type);
	publ_misc_init(mem_type);

}
void dmc_datatraining(MEM_TYPE_E mem_type)
{
	volatile uint32 i,tmp_val;
	for(i = 0; i < 1000; i++);

	tmp_val = 0x60040000;
	tmp_val |= ((1 << 10) | (1 << 0));
	REG32(PUBL_DTCR) = 0X43043001;
	REG32(PUBL_PIR) = tmp_val;

	while((REG32(PUBL_PGSR0) & 0x01) != 0x01);
	if( DDR3 == mem_type )
	{
	}
	for(i = 0; i < 2000; i++);

	dmc_reg_bit_set(REG_AON_TOP_DDR_CHN_SLEEP_CTRL0,31,1,0x1);
	dmc_reg_bit_set(REG_AON_TOP_DDR_CHN_SLEEP_CTRL0,0,10,0x3FF);
	for(i = 0; i < 1000; i++);
}
void DMC_init(void)
{
	...
	int result,len;
	dmc_glb_ops(TRUE);
	umctl2_core_init(LPDDR2,IO_BUSWIDTH_32BIT);
	umctl2_init_seq(LPDDR2);
	umctl2_timing_init(LPDDR2);
	umctl2_dfi_init(LPDDR2);
	umctl2_addrmapping_cfg(LPDDR2);
	umctl2_misc_init(LPDDR2);
	...
	REG32(UMCTL_DFIMISC) = 0x01;

#ifdef CONFIG_ARCH_SCX20L
	dmc_glb_ops(FALSE);
#endif
	while((REG32(UMCTL_STAT) & 0x03 ) != 0x1);
	result = 0;
	for(i = 0; i < 0x20; i++)
	{
		REG32(0x300102a4) = i << 16;
		REG32(0x30010324) = i << 16;
		REG32(0x300103a4) = i << 16;
		REG32(0x30010424) = i << 16;
		dmc_datatraining(LPDDR2);
		if((REG32(0x30010018) & (1 << 22)) == 0x0)
		{
			result |= (1 << i);
		}
		REG32(0x30010004) |= 0x08000000;
		REG32(0x30010004) &= 0xf7ffffff;
		REG32(0x30010004) |= 0x41;
	}
	len = 0;
	max = 0;
	min = 0;
	...
	REG32(0x300102a4) = ((min + max) / 2) << 16;
	REG32(0x30010324) = ((min + max) / 2) << 16;
	REG32(0x300103a4) = ((min + max) / 2) << 16;
	REG32(0x30010424) = ((min + max) / 2) << 16;
	REG32(0x20) = result;
	REG32(0X24) = min;
	REG32(0x28) = max;
	...
	REG32(0X64) = min;
	REG32(0x68) = max;
	REG32(0x300102a4) = 0x12;
	REG32(0x30010324) = 0x08;
	REG32(0x300103a4) = 0x12;
	REG32(0x30010424) = 0x08;

	dmc_datatraining(LPDDR2);
	for(i = 0; i < 0x1000; i = i + 4)
	{
		REG32(0x80000000 + i) = (((i + 3) & 0xff) << 24) |
		(((i + 2) & 0xff ) << 16) | (((i + 1) & 0xff) << 8)
		 | ((i & 0xff) << 0);
