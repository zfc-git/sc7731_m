...
void umctl2_odt_init(DRAM_INFO* dram) 
{
    ...
    #ifdef DDR_DDR3
    reg_bits_set(UMCTL_ODTCFG,24,4,6); //wr_odt_hold
    reg_bits_set(UMCTL_ODTCFG,16,4,1); //wr_odt_delay
    reg_bits_set(UMCTL_ODTMAP,4, 4,0);//rank0_rd_odt
    reg_bits_set(UMCTL_ODTMAP,0, 4,1);//rank0_wr_odt
    #else
    //to be confirm by johnnywang
...
void umctl2_dramtiming_init(DRAM_INFO* dram,CLK_TYPE_E umctl2_clk) {
    ...
    */
	...
	#endif
#if 1
    reg_bits_set(UMCTL_DRAMTMG0, 24, 6, ((WL+(BL>>1)+tWR)+1+ns_to_xclock(15,umctl2_clk)));/*wr2pre*/
    ...
    reg_bits_set(UMCTL_DRAMTMG2,  0, 6, (WL+(BL>>1)+tWTR+1));
...
//                                    (IS_LPDDR3(dram_type)?(ms_to_x1024(tINIT0,umctl2_clk)+ns_to_x1024(tINIT1,umctl2_clk)):0x00)|
    ...
    */
	...
	#ifdef DDR3_WRITE_ODT_SUPPORT
        //emr bit9-6-2: 0_0_1:60ohm 0_1_0:120ohm 0_1_1:40ohm
	reg_bits_set((uint32)&emr,  9, 1, 0);
        reg_bits_set((uint32)&emr,  6, 1, 1);
        reg_bits_set((uint32)&emr,  2, 1, 0);
        #endif
...
void publ_basic_mode_init(CLK_TYPE_E clk,DRAM_INFO* dram)
{
    ...
    #if defined(DDR_DDR3)
    reg_bits_set(PUBL_ZQ0CR1,4,4,1); //ddr3 odt, 1:120ohm 5:60ohm 8:40ohm
    reg_bits_set(PUBL_ZQ1CR1,4,4,1); //ddr3 odt, 1:120ohm 5:60ohm 8:40ohm
    ...
    #if defined(CONFIG_SPX15)
    //only for dolphin,fdtrm = 11
    reg_bits_set(PUBL_DLLGCR,19,1,1);
    reg_bits_set(PUBL_DLLGCR,15,1,1);
    #endif
...
#ifdef DDR_READ_ODT_SUPPORT
    REG32(PUBL_DX0GCR) |= (0x3<<9); //enable DQ/DQS Dynamic RTT Control when read odt
    REG32(PUBL_DX1GCR) |= (0x3<<9); //enable DQ/DQS Dynamic RTT Control when read odt
    REG32(PUBL_DX2GCR) |= (0x3<<9); //enable DQ/DQS Dynamic RTT Control when read odt
    REG32(PUBL_DX3GCR) |= (0x3<<9); //enable DQ/DQS Dynamic RTT Control when read odt
#else
