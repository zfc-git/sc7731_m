...
static const CARD_CmdCtlFlg_t s_cmdDetail[] = {
	...
	{CARD_CMD18_READ_MULTIPLE_BLOCK, 18, SIG_TRANS_CMP | CARD_SDIO_R1 | ERR_DATA_END | ERR_DATA_CRC | ERR_DATA_TIMEOUT,
	 TRANS_MODE_MULTI_BLOCK | TRANS_MODE_READ | TRANS_MODE_BLOCK_COUNT_EN | TRANS_MODE_DMA_EN | CMD_HAVE_DATA},
...
uint32 sdHighCap = FALSE;
extern BOOLEAN Emmc_Read(CARD_Partition_e  cardPartiton, uint32 startBlk,uint32 num,uint8* buf);
...
PUBLIC void SDHOST_NML_INT_SigSt_Dis(SDIO_Hd_Ptr pHd, uint32 msg)
{
	...
	if (0 != (msg & SIG_ERR)) {
		tmpReg |= (BIT_28 | BIT_24 | BIT_23 | BIT_22 | BIT_21 | BIT_20 | BIT_19 | BIT_18 | BIT_17 | BIT_16);
	}
...
*********************************************************************/
PUBLIC SDIO_Hd_Ptr EMMC_HOST_Register(SDIO_CALLBACK fun)
{
...
**	2013/05/17			ypxie				optimization
*********************************************************************/
PUBLIC void SDHOST_SetCmd(SDIO_Hd_Ptr pHd, uint32 cmdIndex, uint32 transmode, SDIO_CmdType_e cmd_type, CARD_CmdRsp_e Response)
{
	volatile uint32 tmpReg;
...
**	2013/05/17			ypxie				optimization
*********************************************************************/
PUBLIC void SDHOST_GetRspFromBuf(SDIO_Hd_Ptr pHd, CARD_CmdRsp_e Response, uint8 * rspBuf)
{
	uint32 tmpRspBuf[4];
	uint32 i;
...
**	2013/05/17			ypxie				optimization
*********************************************************************/
PUBLIC SDIO_Error_e SDIO_SendCmd(SDIO_Hd_Ptr pHd, CARD_Cmd_e cmd, uint32 argument, DATA_Param_t * dataParam, uint8 * rspBuf)
{
	uint32 tmpIntFilter;
	...
	SDHOST_SetCmd(pHd, s_cmdDetail[cmd].cmd_index, s_cmdDetail[cmd].transmode, CMD_TYPE_NORMAL, s_cmdDetail[cmd].response);
...
PUBLIC BOOLEAN CARD_SDIO_SetBlockLength(SDIO_Hd_Ptr pHd, uint32 length)
{
	...
	if (0 != SDIO_SendCmd(pHd, CARD_CMD16_SET_BLOCKLEN, length, (DATA_Param_t *) NULL, rspBuf)) {
		return FALSE;
	}
...
LOCAL BOOLEAN get_card_status(SDIO_Hd_Ptr pHd,uint8 *rspBuf)
{
	return SDIO_SendCmd(pHd, CARD_CMD13_SEND_STATUS, 1 << 16, NULL, rspBuf);
}
/*********************************************************************
** Description :
**  This function used to check card is in programme mode.
** zhenxiang.li add 2016.Jan
**********************************************************************/
LOCAL BOOLEAN IsCardPgmMode(SDIO_Hd_Ptr pHd)
{
	...
	do
	{
		if(0 != get_card_status(pHd, rspBuf))
		{
			return FALSE;
		}
...
LOCAL BOOLEAN CARD_SDIO_SetBusWidth(SDIO_Hd_Ptr pHd, SDIO_BusWidth_e busWidth)
{
	...
	argu = CMD6_ACCESS_MODE_WRITE_BYTE | (EXT_CSD_BUS_WIDTH_INDEX << CMD6_BIT_MODE_OFFSET_INDEX & CMD6_BIT_MODE_MASK_INDEX);
...
*********************************************************************/
PUBLIC BOOLEAN CARD_SDIO_InitCard(SDIO_Hd_Ptr pHd, uint32 sdioClk, SDIO_BusWidth_e busWidth)
{
	uint8 rspBuf[16];
    uint8 rspCID[16];
	uint32 pre_tick, cur_tick;
	...
	pre_tick = SCI_GetTickCount();	/*set start tick value */
	do {
		if (0 != SDIO_SendCmd(pHd, CARD_CMD1_SEND_OP_COND, 0x00FF8000 | SECTOR_MODE, NULL, rspBuf)) {
			continue;
		}
	...
	/* Get CID */
	if (0 != SDIO_SendCmd(pHd, CARD_CMD2_ALL_SEND_CID, 0, NULL, rspCID)) {
		return FALSE;
	}
    ...
    //CID->PNM[4] = CIDbuf[7];
    //CID->PNM[5] = CIDbuf[8];
    if(0x15 == rspCID[0]) //CID_MANFID_SAMSUNG
    {       
		/*
        //KLM8G1WEMB-B031: 0x38574d423352(Product Name in CID Register) 
        if(0x38 == rspCID[3] && 0x57 == rspCID[4] &&\
           0x4d == rspCID[5] && 0x42 == rspCID[6] &&\
           0x33 == rspCID[7] && 0x52 == rspCID[8] )
        {
            if(0 != SDIO_SendCmd(pHd,CARD_ACMD6_SET_EXT_CSD, 0x03A20100,NULL,rspBuf))
            {
                return FALSE;
            }
        }*/
        
		//KLM8G1WEPD-B031: 0x385750443352(Product Name in CID Register) 
        if(0x38 == rspCID[3] && 0x57 == rspCID[4] &&\
           0x50 == rspCID[5] && 0x44 == rspCID[6] &&\
           0x33 == rspCID[7] && 0x52 == rspCID[8] )
        {
            if(0 != SDIO_SendCmd(pHd,CARD_ACMD6_SET_EXT_CSD, 0x03A20100,NULL,rspBuf))
            {
       ...
       /* //KLM4G1FEAB-B001: 0x344645414242(Product Name in CID Register) 
        if(0x34 == rspCID[3] && 0x46 == rspCID[4] &&\
           0x45 == rspCID[5] && 0x41 == rspCID[6] &&\
           0x42 == rspCID[7] && 0x42 == rspCID[8] )
        {
            if(0 != SDIO_SendCmd(pHd,CARD_ACMD6_SET_EXT_CSD, 0x03A20100,NULL,rspBuf))
            {
                return FALSE;
        ...
        //KLM4G1FEAC-B031: 0x344645414342(Product Name in CID Register)
        if(0x34 == rspCID[3] && 0x46 == rspCID[4] &&\
           0x45 == rspCID[5] && 0x41 == rspCID[6] &&\
           0x43 == rspCID[7] && 0x42 == rspCID[8] )
        {
            if(0 != SDIO_SendCmd(pHd,CARD_ACMD6_SET_EXT_CSD, 0x03A20100,NULL,rspBuf))
            {
                return FALSE;
...
PUBLIC BOOLEAN Emmc_Init(void)
{
	...
	p_EmmcHd = EMMC_HOST_Register(_irqCardProc);
...
**	2013/08/09			ypxie				optimization
*********************************************************************/
PUBLIC BOOLEAN CARD_SDIO_ReadMultiBlock(CARD_Partition_e cardPartiton, SDIO_Hd_Ptr pHd, uint32 startBlk, uint32 num, uint8 * buf)
{
	uint8 rspBuf[16];
...
**	2013/08/12			ypxie				support 1,8 bits buswidth
*********************************************************************/
PUBLIC BOOLEAN SD_CARD_SDIO_SetBusWidth(SDIO_Hd_Ptr pHd, SDIO_BusWidth_e busWidth)
{
	uint8 rspBuf[16];
...
**	2013/08/12			ypxie				add disable sdclk fot clear glitch
*********************************************************************/
PUBLIC SDIO_Hd_Ptr SD_HOST_Register(SDIO_CALLBACK fun)
{
	/* Set emmc controller base address first */
	s_EmmcCtl.host_cfg = (SDIO_REG_CFG *) ((volatile uint32 *)SD_CFG_ADDR);
	/* enable emmc ahb_clock, and reset it. */
	REG32(EMMC_AHB_EN) |= 1 << SD_AHB_EN_SHIFT;

	SDHOST_SdClk_Enable(&s_EmmcCtl, SDIO_OFF);

	REG32(EMMC_AHB_RST) |= 1 << SD_AHB_RST_SHIFT;
	SDIO_Mdelay(1);
	REG32(EMMC_AHB_RST) &= ~(1 << SD_AHB_RST_SHIFT);
...
**	2013/08/12			ypxie				added params for card
*********************************************************************/
PUBLIC BOOLEAN SD_CARD_SDIO_InitCard(SDIO_Hd_Ptr pHd, uint32 sdioClk, SDIO_BusWidth_e busWidth)
{
	uint8 rspBuf[16];
	uint16 rca;
	uint32 pre_tick, cur_tick;
...
PUBLIC BOOLEAN SD_Init(void)
{
	...
	p_EmmcHd = SD_HOST_Register(_irqCardProc);
	...
	p_EmmcHd->bus_width = SDIO_BUS_1_BIT;
	p_EmmcHd->cur_partition = PARTITION_MAX;
	ret = SD_CARD_SDIO_InitCard(p_EmmcHd, SDIO_BASE_CLK_26M, SDIO_BUS_4_BIT);
...
**	2013/08/09			ypxie				optimization
*********************************************************************/
PUBLIC BOOLEAN SD_CARD_SDIO_ReadMultiBlock(CARD_Partition_e cardPartiton, SDIO_Hd_Ptr pHd, uint32 startBlk, uint32 num, uint8 * buf)
{
	uint8 rspBuf[16];
	uint32 addr = 0xFFFFFFFF;
	DATA_Param_t data;
...
PUBLIC BOOLEAN SD_CARD_Read(CARD_Partition_e cardPartiton, uint32 startBlk, uint32 num, uint8 * buf)
{
	uint32 ret = 0;
	uint8 rspBuf[16];

	ret = SD_CARD_SDIO_ReadMultiBlock(cardPartiton, p_EmmcHd, startBlk, num, buf);
	return ret;
}
#endif
