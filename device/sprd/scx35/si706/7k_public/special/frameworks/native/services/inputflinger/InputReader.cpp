...
void InputReader::processEventsLocked(const RawEvent* rawEvents, size_t count) {
    if(rawEvents->value == 0 || rawEvents->value == -1){ // 0:A and B type key up or A type touch or move end; -1: B type touch or move up
...
void TouchInputMapper::configureSurface(nsecs_t when, bool* outResetNeeded) {
    ...
    if (viewportChanged) {
       ...
       int displayOrientation = 0;//DisplayState::eOrientationDefault; 
    char property[PROPERTY_VALUE_MAX]; 
    if (property_get("ro.sf.hwrotation", property, NULL) > 0) { 
        //displayOrientation 
        switch (atoi(property)) { 
            case 90: 
            displayOrientation = 1;//DisplayState::eOrientation90; 
            break; 
 
            case 180: 
            displayOrientation = 2;//DisplayState::eOrientation180; 
            break; 
 
            case 270: 
            displayOrientation = 3;//DisplayState::eOrientation270; 
            break; 
        } 
    } 
        mViewport.orientation +=displayOrientation; 
        mViewport.orientation &= 0x3; 
