...
static void btif_fm_upstreams_evt(UINT16 event, char* p_evt_data)
{
    ...
    switch (event)
    {
        case BTA_FM_ENABLE_EVT:
            BTIF_TRACE_DEBUG("%s: event= BTA_FM_ENABLE_EVT", __func__);
        ...
        case BTA_FM_TUNE_EVT:
            BTIF_TRACE_DEBUG("%s: BTA_FM_TUNE_EVT status = %i rssi = %i freq = %i ",
        ...
        case BTA_FM_DISABLE_EVT:
            BTIF_TRACE_DEBUG("%s: event= BTA_FM_DISABLE_EVT", __func__);
            // TODO Fine tune
            usleep(300000);
        ...
        case BTA_FM_AF_JMP_EVT:
            BTIF_TRACE_DEBUG("%s: BTA_FM_AF_JMP_EVT status = %i rssi = %i freq = %i ",
                 __func__, (int)p_fm_evt_data->chnl_info.status, (int)p_fm_evt_data->chnl_info.rssi,
                 (int)p_fm_evt_data->chnl_info.freq);
        ...
        case BTA_FM_SEARCH_CMPL_EVT:
            BTIF_TRACE_DEBUG("%s: BTA_FM_SEARCH_CMPL_EVT status = %i rssi = %i freq = %i ",
                 __func__, (int)p_fm_evt_data->chnl_info.status, (int)p_fm_evt_data->chnl_info.rssi,
                 (int)p_fm_evt_data->chnl_info.freq);
        ...
        case BTA_FM_SEARCH_EVT:
            BTIF_TRACE_DEBUG("%s: BTA_FM_SEARCH_EVT rssi = %i snr = %i freq = %i ",
        ...
        case BTA_FM_AUD_MODE_EVT:
            BTIF_TRACE_DEBUG("%s: event= BTA_FM_AUD_MODE_EVT,audio_mode = %d",
        ...
        case BTA_FM_AUD_DATA_EVT:
            BTIF_TRACE_DEBUG("%s: BTA_FM_SEARCH_EVT status = %i rssi = %i freq = %i ",
        ...
        case BTA_FM_AUD_PATH_EVT:
            BTIF_TRACE_DEBUG("%s: BTA_FM_AUD_PATH_EVT, bta audio_path: %d, status: %d",
        ...
        case BTA_FM_RDS_MODE_EVT:
            BTIF_TRACE_DEBUG("%s: BTA_FM_RDS_MODE_EVT: status=%d, RDS=%s, AF=%s", __func__,
        ...
        case BTA_FM_SET_DEEMPH_EVT:
            BTIF_TRACE_DEBUG("%s: BTA_FM_SET_DEEMPH_EVT", __func__);
        ...
        case BTA_FM_MUTE_AUD_EVT:
            BTIF_TRACE_DEBUG("%s: BTA_FM_MUTE_AUD_EVT", __func__);
        ...
        case BTA_FM_SCAN_STEP_EVT:
            BTIF_TRACE_DEBUG("%s: BTA_FM_SCAN_STEP_EVT", __func__);
        ...
        case BTA_FM_SET_REGION_EVT:
            BTIF_TRACE_DEBUG("%s: BTA_FM_SET_REGION_EVT", __func__);
            HAL_CBACK(bt_fm_callbacks, region_cb, p_fm_evt_data->region_info.status,
                 p_fm_evt_data->region_info.region);
            break;

        case BTA_FM_NFL_EVT:
            BTIF_TRACE_DEBUG("%s: BTA_FM_NFL_EVT: NFL = %d", __func__, (int)p_fm_evt_data->nfloor.rssi);
            HAL_CBACK(bt_fm_callbacks, nfl_cb, p_fm_evt_data->region_info.status,
                 p_fm_evt_data->region_info.region);
            break;

        case BTA_FM_RDS_TYPE_EVT:
            BTIF_TRACE_DEBUG("%s: BTA_FM_RDS_TYPE_EVT: status=%d rds_type=0x%x", __func__,
        ...
        case BTA_FM_VOLUME_EVT:
            {
             BTIF_TRACE_DEBUG("%s: BTA_FM_VOLUME_EVT: status = %d, volume = %d", __func__,
        ...
        default:
            BTIF_TRACE_DEBUG("%s: Unhandled event %i", __func__, event);
            break;
    }
}
...
static int btif_fm_set_volume(int volume)
{
    ...
    BTIF_TRACE_DEBUG( "btif_fm_volume_control(volume: %d) on current_bta_path: %d",
