...
/* SPRD: modify 20150915 Spreadtrum of 474961 Add log for global reference table overflow @{ */
  void AddDebugTrace(IndirectRef result);

  void RemoveDebugTrace(IndirectRef result);

  void DumpDebugTrace();

  char* LocalMd5(const char * data , const char * data2);

  static int PredStrHash(void *key);

  static bool PredStrEquals(void *key_a, void *key_b);

  static int PredSortTrace(const void * t1, const void * t2);

  static bool PredClollectMapValue(void * key, void * value, void * context);
/* 474961 @} */
...
/* SPRD: modify 20150915 Spreadtrum of 474961 Add log for global reference table overflow @{ */
  ...
  typedef struct Debug_data {
      char* sum_md5;
      IndirectRef dRference;
  } Debug_data;
  typedef std::vector<Debug_data> Debug_Data_List;
  Hashmap * g_hash_map = NULL;
  Debug_data debug_data;
  Debug_Data_List debug_data_list;
  size_t max_available_count = 51200;
  size_t debug_count = 45000;
  bool isEnableDebugTrce = false;
/* 474961 @} */
};
