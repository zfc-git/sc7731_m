...
static void RealPruneDalvikCache(const std::string& cache_dir_path) {
  if (!Runtime::Current()->IsZygote()) {
    LOG(ERROR) << "Failed to PruneDalvikCache, only Zygote process have the permission.";
    return;
  }
  ...
  for (struct dirent* de = readdir(cache_dir); de != nullptr; de = readdir(cache_dir)) {
    ...
    if (strcmp(name, ".") == 0 || strcmp(name, "..") == 0 || strcmp(name, ".booting") == 0) {
      continue;
    }
...
static void MarkZygoteStart(const InstructionSet isa, const uint32_t max_failed_boots) {
  ...
  LOG(INFO) << "Number of failed boots on : " << boot_marker << " = " << num_failed_boots << ", max_fail_count= " << max_failed_boots;
