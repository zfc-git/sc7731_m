...
#define CHECK_WATCH_DOG_PTHREAD_CALL(call, args, what) \
 ...
 private:
  ...
  static constexpr int64_t kWatchDogTimeoutSeconds = kSlowdownFactor * 10 * 60 * 3;
...
static constexpr size_t kMaxDexFileSizeForLowRamDevice = 35 * MB;
...
static bool CompileUseInterpreterOnly(bool is_image, std::vector<const DexFile*>& dex_files) {
  if (is_image || dex_files.size() < kMinDexFilesForSwap) {
    LOG(INFO) << "do not CompileUseInterpreterOnly, is_image= " << is_image << ", dex_files.size()= " << dex_files.size();
    return false;
  }
  ...
  bool have_property = property_get("ro.config.low_ram", buf, "false");
  if (have_property) {
    if (strcmp(buf, "true") == 0) {
      low_ram = true;
    }
  } else {
     LOG(INFO) << "do not CompileUseInterpreterOnly, do not have low_ram property.";
  }
#endif
  if (!low_ram) {
    LOG(INFO) << "do not CompileUseInterpreterOnly, not low_ram device.";
    return false;
  }
  ...
  LOG(INFO) << "is_image= " << is_image << ", there are ["
            << dex_files.size() << "] dex files, total size :" << dex_files_size
            << ", kMaxDexFileSizeForLowRamDevice= " << kMaxDexFileSizeForLowRamDevice;

  return dex_files_size >= kMaxDexFileSizeForLowRamDevice;
}
...
class Dex2Oat FINAL {
 public:
  ...
  bool Setup() {
    ...
    if (CompileUseInterpreterOnly(image_, dex_files_)) {
      LOG(INFO) << "dex2oat, Large Application on low_ram device, CompileUseInterpreterOnly.";
      compiler_options_->SetCompilerFilter(CompilerOptions::kInterpretOnly);
    } else {
      LOG(INFO) << "dex2oat, do not CompileUseInterpreterOnly";
    }
