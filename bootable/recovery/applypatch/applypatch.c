...
static int GenerateTarget(FileContents* source_file,
    ...
    if (output < 0) {
        ...
        if(verify_type != VARIFY_TYPE_NON){
            ...
            printf("GenerateTarget, target_filename = %s\n", target_filename);
            copy_target = strdup(target_filename);
            magic = strtok(copy_target, ":");
            dev_name = strtok(NULL, ":");
            printf("GenerateTarget, dev_name = %s\n", dev_name);

            if(isApplypatch){
                previous_fstype = last_fstype;
                previous_dev = last_dev;
                previous_type = last_sec;
            }else{
                get_previous_secureboot_for_device(dev_name, &previous_dev, &previous_type, &previous_fstype);
            }
            ...
            printf("previous_fstype = %s, previous_dev = %s, previous_type = %s\n", previous_fstype, previous_dev, previous_type);

            if (sprd_secure_boot_verify(verify_type, VARIFY_CONTENT_TYPE_DATA, msi.buffer, msi.pos, dev_name,
                        previous_data, previous_size, previous_verify_type) != 0) {
                printf("secure boot verify for %s failed\n", dev_name);
                if(copy_target) free(copy_target);
                free(msi.buffer);
                return 1;
            }
            if(copy_target) free(copy_target);
