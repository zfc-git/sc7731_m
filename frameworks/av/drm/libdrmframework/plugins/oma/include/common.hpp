...
#define JAVA_EMU_PATH		"/storage/emulated/0/temp_sd"
#define JAVA_EMU_PATH1		"/storage/emulated"
#define NATIVE_REAL_PATH	"/data/media/temp_sd"
#define NATIVE_REAL_PATH1	"/data/media"
...
namespace android {
	...
	*out = result;  
	...
	inline String8 redirectPath(const String8& path) {
		char* tmp = NULL;
		String8 ret;
		if ((tmp = strstr(path.string(), JAVA_EMU_PATH)) != NULL) {
			ret.appendFormat(NATIVE_REAL_PATH"%s", (tmp + strlen(JAVA_EMU_PATH)));
		} else if ((tmp = strstr(path.string(), JAVA_EMU_PATH1)) != NULL) {
			ret.appendFormat(NATIVE_REAL_PATH1"%s", (tmp + strlen(JAVA_EMU_PATH1)));
		} else {
			ret = path;
		}
		return ret;
