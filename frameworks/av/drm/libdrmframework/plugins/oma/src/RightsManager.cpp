...
String8 RightsManager::getFakeKey() {
	...
	SUNWAY_NOISY(ALOGE("RightsManager::getFakeKey: sFakeKey is null, try 2 query db"));
...
String8 RightsManager::getFakeUid() {
	...
	SUNWAY_NOISY(ALOGE("RightsManager::getFakeUid: sFakeUid is null, try 2 query db"));
	
    if (!db) {
		SUNWAY_NOISY(ALOGE("RightsManager::getFakeUid: db is null"));
		return sFakeUid;
    }
    ...
    if (rc != SQLITE_OK) {
		SUNWAY_NOISY(ALOGE("RightsManager::getFakeUid: failed: %s", sqlite3_errmsg(db)));
		return sFakeUid;
    }
    rc = sqlite3_step(stmt);
    if (rc == SQLITE_ROW) 
...
RightsParser RightsManager::query(const String8& uid) {
    SUNWAY_NOISY(ALOGE("RightsManager::query for uid %s", uid.string()));
	...
	SUNWAY_NOISY(ALOGE("RightsManager::query: version: %s, uid: %s, key: %s", ret.version.string(), ret.uid.string(), ret.key.string()));
	for (int i=0; i<ACTION_NUM; ++i) {
	    for (int j=0; j<CONSTRAINT_NUM; ++j) {
		ret.constraints[i][j]=sqlite3_column_int64(stmt,4+i*CONSTRAINT_NUM+j);
	    }
	}
	SUNWAY_NOISY(ALOGE("RightsManager::query: ok: %s", ret.uid.string()));
    } else {
...
bool RightsManager::insertOrUpdate(RightsParser& parser) {
    ...
    sql.append(")");
    SUNWAY_NOISY(ALOGE("RightsManager::insertOrUpdate: %s", sql.string()));
    char * zerr;
    int rc = sqlite3_exec(db,sql.string(),0,0,&zerr);
	SUNWAY_NOISY(ALOGE("RightsManager::insertOrUpdate: end"));
    if (rc != SQLITE_OK) {
