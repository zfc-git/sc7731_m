...
bool DmParser::parse() {
    ...
    SUNWAY_NOISY(ALOGE("DmParser::parse %s", dmFileName));
    rewind(file);
    if (! file) {
	...
	SUNWAY_NOISY(ALOGE("DmParser::parse %s, ok", dmFileName));
	SUNWAY_NOISY(ALOGE("DmParser::parse %s, parse result: rights: from %d to %d; data: from %d to %d, contentType: %s, contentId: %s, encoding: %d",dmFileName, rightsStartOffset, rightsEndOffset, dataStartOffset, dataEndOffset, dataContentType, dataContentId, dataTransferEncoding));
	return true;
    }
    return false;
}
...
void DmParser::seekBoundaryDelimiter() {
    ...
    while ((line = fgets(buffer, MAX_LINE, file)) && (!strlen(chomp(line)))) {}
	if (!line) return;
	SUNWAY_NOISY(ALOGE("DmParser::seekBoundaryDelimiter: line = %s, len = %d", line, strlen(line)));
	for (int i = 0; i < strlen(line); i++) {
		if (' ' == line[i] || '\t' == line[i]) {
			...
			SUNWAY_NOISY(ALOGE("DmParser::seekBoundaryDelimiter: boundary = %s", boundary));
			break;
		} else {
			break;
		}
