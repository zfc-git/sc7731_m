...
bool DcfParser::parse() {
    SUNWAY_NOISY(ALOGE("DcfParser::parse %s, fd: %d", dcfFileName, fd));
    if (fd == -1) {
    ...
    SUNWAY_NOISY(ALOGE("DcfParser::parse %s, version: %d", dcfFileName, version));
    if (version != 1) {
	...
	if (-1 == ::read(fd,&contentTypeLen,1)) {
	    ...
	    if (0 == contentTypeLen) {
			...
			SUNWAY_NOISY(ALOGE("DcfParser::parse %s, contentTypeLen: %d", dcfFileName, contentTypeLen));
		}
	}
    ...
    if (!contentUriLen) {
	    ...
	    SUNWAY_NOISY(ALOGE("DcfParser::parse %s, contentUriLen: %d", dcfFileName, contentUriLen));
	}
    ...
    SUNWAY_NOISY(ALOGE("DcfParser::parse %s, contentType: %s", dcfFileName, this->contentType.string()));
    ...
    SUNWAY_NOISY(ALOGE("DcfParser::parse %s, default action: %d", dcfFileName, mDefaultAct));	
    ...
    SUNWAY_NOISY(ALOGE("DcfParser::parse %s, contentUri: %s", dcfFileName, this->contentUri.string()));
    int headersLen = 0;
    char uintFragment=0;
	for (int i = 0; i < 5; i++) {
    ...
    SUNWAY_NOISY(ALOGE("DcfParser::parse %s, headersLen: %d", dcfFileName, headersLen));
    int dataLen = 0;
    uintFragment=0;
	for (int i = 0; i < 5; i++) {
		::read(fd,&uintFragment,1);
    ...
    SUNWAY_NOISY(ALOGE("DcfParser::parse %s, dataLen: %d", dcfFileName, dataLen));
    // parse headers
    ...
    SUNWAY_NOISY(ALOGE("DcfParser::parse: dataOffset: %d, dateLen: %d" , offset, dataLen));
    return true;
}

bool DcfParser::parseHeader(const char* headers, int size) {
    unsigned int parse_err_ctx = 0;
    SUNWAY_NOISY (ALOGE("DcfParser::parseHeader: %s", headers));
    int tokenStart = 0;
    String8 currentKey;
	...
	if (headers[i] == ':') {
	    ...
	    SUNWAY_NOISY (ALOGE("DcfParser::parseHeader: got key: %s", currentKey.string()));
	    // skip the succeeding ' '
	    i++;
	    tokenStart = i+1;
	    ...
	    SUNWAY_NOISY (ALOGE("DcfParser::parseHeader: got value: %s", currentValue.string()));
...
bool DcfParser::setKey(char* transKey) {
    SUNWAY_NOISY (ALOGE("DcfParser::setKey: %s",transKey));
...
/*
ssize_t DcfParser::read(char* plainText, ssize_t numBytes) {
    char* tmp = (char*)malloc(numBytes*2);
    bzero(tmp, numBytes*2);
    if (eof) {
	return 0;
    }
    if (firstRead) {
	EVP_CIPHER_CTX_init(decryptContext);
	EVP_DecryptInit_ex(decryptContext, EVP_aes_128_cbc(), NULL, key, NULL);
    }
    unsigned char * cipherText = (unsigned char*) malloc(numBytes);
    bzero(cipherText, numBytes);
    int cLen = ::read(fd, cipherText, numBytes);
    
    int pLen = 0;
    if (cLen != 0) {
	EVP_DecryptUpdate(decryptContext, (unsigned char*)tmp, &pLen, cipherText, cLen);
	memcpy(plainText, tmp, pLen);
	if (firstRead && pLen == 16) {
	    int tmpLen = 0;
	    EVP_DecryptFinal_ex(decryptContext, (unsigned char*)tmp, &tmpLen);
	    memcpy(plainText+pLen, tmp, tmpLen);
	    pLen += tmpLen;
	    // FIXME: make sure EOF reached
	    eof = true;
	}
    } else {
	EVP_DecryptFinal_ex(decryptContext, (unsigned char*)tmp, &pLen);
	memcpy(plainText, tmp, pLen);
	eof = true;
    }
    free(tmp);
	...
	firstRead = false;
	pLen -= 16;
	memmove(plainText, plainText+16, pLen);
    }
    return pLen;
}
*/
int DcfParser::readBlock(char* buffer, int firstBlock, int numBlocks) {
    SUNWAY_NOISY(ALOGE("DcfParser::readBlock firstBlock:%d, numBlocks: %d, from fd:%d", firstBlock, numBlocks, fd));
	...
	if (::lseek(fd, 0, SEEK_CUR) == length) {
	    ...
	    SUNWAY_NOISY(ALOGE("DcfParser::readBlock final %d, %d", plainTextLen, pLen));
	    plainTextLen += pLen;
	    break;
	}
    }
    memcpy(buffer, plainText, plainTextLen);
    SUNWAY_NOISY(ALOGE("DcfParser::readBlock returns %d", plainTextLen));
...
ssize_t DcfParser::readAt(char* outBuffer, ssize_t numBytes, int offset) {
    SUNWAY_NOISY(ALOGE("DcfParser::readAt %d:%d", numBytes, offset));
    // first prefix IV block is skipped
    int firstBlock = (int) (offset/16)+1;
    int numBlocks = (int)(numBytes/16)+10;

    SUNWAY_NOISY(ALOGE("DcfParser::readAt: numBlocks %d", numBlocks));
    ...
    SUNWAY_NOISY(ALOGE("DcfParser::readAt returns %d", ret));
