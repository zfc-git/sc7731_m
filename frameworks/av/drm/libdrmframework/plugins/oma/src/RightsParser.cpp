...
bool RightsParser::parse() {
	...
	if (key.isEmpty()) {
	    ...
	    RightsManager rm;
	    key = rm.getFakeKey();
		SUNWAY_NOISY(ALOGE("RightsParser::parse: no key, fake key = %s", key.string()));
	}
    } else {
	state = ERROR;
    }
...
bool RightsParser::parseXML() {
	...
	SUNWAY_NOISY(ALOGE("RightsParser::parseXML: origUid = %s", origUid));
    ...
    if (permissionNode) {
	SUNWAY_NOISY(ALOGE("RightsParser::parseXML: got permissionNode"));
	for (int i=0; i<sizeof(actionsName)/sizeof(char*); ++i) {
	    TiXmlNode* actionNode = permissionNode->FirstChild(actionsName[i]);
	    SUNWAY_NOISY(ALOGE("RightsParser::parseXML: checking for %s", actionsName[i]));
	    if (actionNode) {
		SUNWAY_NOISY(ALOGE("RightsParser::parseXML: got node for %s", actionsName[i]));
		...
		if (tmpNode) {
			...
			if (constraintNode) {
			    ...
			    SUNWAY_NOISY(ALOGE("RightsParser::parseXML: got node for %s = %s", constraintsName[j], tmpValue));
...
int RightsParser::checkRightsStatusForAction(int action) {
    SUNWAY_NOISY(ALOGE("RightsParser::checkRightsStatusForAction: %d", action));
	...
	if (intervalLimited) {
		if (constraintsForAction[INTERVAL_START] == UNSET) {
			...
			if (0xffffffffffffffff - (uint64_t)constraintsForAction[INTERVAL_START] <= (uint64_t)constraintsForAction[INTERVAL]) {
				...
				SUNWAY_NOISY(ALOGE("RightsParser::INTERVAL_START: %lld, INTERVAL: %llu, cTime: %llu", constraintsForAction[INTERVAL_START], constraintsForAction[INTERVAL], cTime));
...
DrmConstraints* RightsParser::getConstraintsForAction(int action) {
	...
	SUNWAY_NOISY(ALOGE("RightsParser::getConstraintsForAction: counts = %lld, datetime_start = %lld, datetime_end = %lld, interval_start = %lld, interval = %lld", constraintsForAction[COUNT], constraintsForAction[DATETIME_START], constraintsForAction[DATETIME_END], constraintsForAction[INTERVAL_START], constraintsForAction[INTERVAL]));
    return ret;
}
...
int64_t RightsParser::parseRELInterval(const char* interval) {
	...
	SUNWAY_NOISY(ALOGE("RightsParser::parseRELInterval: interval = %llu", relInterval));
