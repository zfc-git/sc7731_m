...
void RightsConsumer::setPlaybackStatus(int status) {
    SUNWAY_NOISY (ALOGE("RightsConsumer::setPlaybackStatus() : %d", status));
    time64_t currentTime = getCurrentTime();
    SUNWAY_NOISY (ALOGE("RightsConsumer::setPlaybackStatus:currentTime: %lld", currentTime));
	...
	SUNWAY_NOISY (ALOGE("RightsConsumer::setPlaybackStatus():START"));
	...
	if (startTime != TIME_INVALID) {
	    ...
	    SUNWAY_NOISY (ALOGE("RightsConsumer::setPlaybackStatus():PAUSE: update interval to %lld", interval));
	}
    } else if (status == Playback::STOP) {
	if (startTime != TIME_INVALID) {
	    interval += (currentTime-startTime);
	    startTime = TIME_INVALID;
	    SUNWAY_NOISY (ALOGE("RightsConsumer::setPlaybackStatus():STOP: update interval to %lld", interval));
	}
	if (interval != 0) {
	    needConsume = true;
	}
...
void RightsConsumer::consumeForAction(RightsParser& rightsParser, int action) {
    ...
    if (constraintsForAction[RightsParser::INTERVAL] != RightsParser::UNSET &&
		...
		SUNWAY_NOISY (ALOGE("RightsConsumer::consumeForAction: action is %d, interval_start is %lld", action, intervalStartTime));
		constraintsForAction[RightsParser::INTERVAL_START] = intervalStartTime;
	}
}

bool RightsConsumer::consume(RightsParser& rightsParser) {
    SUNWAY_NOISY (ALOGE("RightsConsumer::consume: interval is %lld", interval));
    if (needConsume) {
	needConsume = false;
