        ...
        synchronized (mLock) {
            ...
            if (DEBUG_POWER_ISSUE) {
                ...
                try{
                    ...
                    + ", when = "+when+ ", triggerTime = "+DateFormat.format("yyyy-MM-dd HH:mm:ss", when) + ", flags=0x" + Integer.toHexString(flags));
                }catch(Exception e){
        ...
        if ((a.flags&(AlarmManager.FLAG_ALLOW_WHILE_IDLE
            ...
            try{
                if(DEBUG_POWER_OPT && a.tiggerTimeAdjusted){
                    ...
                    +a.type +", interval = "+ a.repeatInterval +", when = "+a.when+", triggerTime = "+DateFormat.format("yyyy-MM-dd HH:mm:ss", adjustedTime)+", adjusted = "+a.tiggerTimeAdjusted);
                }
            }catch(Exception e){
    ...
    private class AlarmThread extends Thread
    {
        ...
        public void run()
        {
            ...
            while (true)
            {
                ...
                /* Add Debug log @{ */
                if (DEBUG_POWER_ISSUE) {
                    Slog.d(TAG, "waitForAlarm result:" + result);
                }
                /* @} */
