...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    public void init(Context context, IWindowManager windowManager,
	 ...
	 /*[A]enable root for user, laixiaojin, 20140412*/
	 filter = new IntentFilter();
	 filter.addAction("zed.adb.root");
	 context.registerReceiver(mAdbRootReceiver, filter);
	 /*END, laixiaojin, 20140412*/
        ...
        context.registerReceiver(mSDcardReceiver, sdcardfilter);
   ...
   /*[A]enable root for user, laixiaojin, 2140412*/
   BroadcastReceiver mAdbRootReceiver = new BroadcastReceiver() {
	public void onReceive(Context context, Intent intent) {
            SystemService.start("console");
	     SystemProperties.set("sys.adb.debug","1");
   ...
   /*END, laixiaojin, 20140412*/
	...
	BroadcastReceiver mSDcardReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (Intent.ACTION_MEDIA_MOUNTED.equals(intent.getAction())) {
               File packagefile = new File(getsdcardpatch(context)+"/zedlauncher.txt");  
			   File packagefilepcbtest = new File(getsdcardpatch(context)+"/zedpcbtest.txt");
			    if (packagefilepcbtest.exists()&&isApkEnabled(context, "com.zediel.pcbtest")) {               
                    Intent pcbtestintent = new Intent(Intent.ACTION_MAIN);
                    pcbtestintent.setClassName("com.zediel.pcbtest", "com.zediel.pcbtest.ZedielTools");                   
                    pcbtestintent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);               
                    mContext.startActivity(pcbtestintent);
                }
                if (packagefile.exists()&&isApkEnabled(context, "com.android.launcher3")) {               
                    Intent testintent = new Intent(Intent.ACTION_MAIN);
                    testintent.setClassName("com.android.launcher3","com.android.launcher3.Launcher");                     
	...
	private boolean isApkEnabled(Context context, String packageName) {
		final PackageManager packageManager = context.getPackageManager();
		if (isApkExist(context, packageName)) {
			if(packageManager.getApplicationEnabledSetting(packageName) ==  PackageManager.COMPONENT_ENABLED_STATE_DISABLED_USER){
				return false;
			}
			return true;
		}
	...
	private boolean isApkExist(Context ctx, String packageName){
		...
		try {
			...
			Log.e(TAG, "isApkExist not found");
			return false;
		}
		return true;
	}
