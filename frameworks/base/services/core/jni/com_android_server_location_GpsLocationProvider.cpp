        ...
        jintArray maskArray)
{
    ...
    for(int i = 0; i < 8; i++){
        mask[i] = sGpsSvStatus.ephemeris_mask[i];
    }
    for(int i = 0; i < 8; i++){
        mask[i+8] = sGpsSvStatus.almanac_mask[i];
    }
    for(int i = 0; i < 8; i++){
        mask[i+16] = sGpsSvStatus.used_in_fix_mask[i];
    }
    /* @} */
...
static void android_location_GpsLocationProvider_agps_set_reference_location_cellid(
        JNIEnv* /* env */, jobject /* obj */, jint type, jint mcc, jint mnc, jint lac, jint cid, jint phyId, jint mncLength)
{
    ...
    switch(type) {
        ...
        case AGPS_REG_LOCATION_TYPE_SIM:
        /* SPRD:AGPS support 4G network */
        case AGPS_REF_LOCATION_TYPE_LTE_CELLID:
            ...
            location.u.cellID.phyid = phyId;
            location.u.cellID.mnc_length = mncLength;
            break;
        default:
