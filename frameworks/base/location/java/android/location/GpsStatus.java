    ...
    synchronized void setStatus(int svCount, int[] prns, float[] snrs,
        ...
        for (int i = 0; i < svCount; i++) {
            ...
            if (prn > 0 && prn <= NUM_SATELLITES) {
                ...
                if (prn < 32) {
                    int prnShift = (1 << prn);
                    satellite.mHasEphemeris = ((ephemerisMask[0] & prnShift) != 0);
                    satellite.mHasAlmanac = ((almanacMask[0] & prnShift) != 0);
                    satellite.mUsedInFix = ((usedInFixMask[0] & prnShift) != 0);
                } else if (prn >= 32 && prn < 32 * 2) {
                    int prnShift = (1 << (prn - 32));
                    satellite.mHasEphemeris = ((ephemerisMask[1] & prnShift) != 0);
                    satellite.mHasAlmanac = ((almanacMask[1] & prnShift) != 0);
                    satellite.mUsedInFix = ((usedInFixMask[1] & prnShift) != 0);
                } else if (prn >= 32 * 2 && prn < 32 * 3) {
                    int prnShift = (1 << (prn - 32 * 2));
                    satellite.mHasEphemeris = ((ephemerisMask[2] & prnShift) != 0);
                    satellite.mHasAlmanac = ((almanacMask[2] & prnShift) != 0);
                    satellite.mUsedInFix = ((usedInFixMask[2] & prnShift) != 0);
                } else if (prn >= 32 * 3 && prn < 32 * 4) {
                    int prnShift = (1 << (prn - 32 * 3));
                    satellite.mHasEphemeris = ((ephemerisMask[3] & prnShift) != 0);
                    satellite.mHasAlmanac = ((almanacMask[3] & prnShift) != 0);
                    satellite.mUsedInFix = ((usedInFixMask[3] & prnShift) != 0);
                } else if (prn >= 32 * 4 && prn < 32 * 5) {
                    int prnShift = (1 << (prn - 32 * 4));
                    satellite.mHasEphemeris = ((ephemerisMask[4] & prnShift) != 0);
                    satellite.mHasAlmanac = ((almanacMask[4] & prnShift) != 0);
                    satellite.mUsedInFix = ((usedInFixMask[4] & prnShift) != 0);
                } else if (prn >= 32 * 5 && prn < 32 * 6) {
                    int prnShift = (1 << (prn - 32 * 5));
                    satellite.mHasEphemeris = ((ephemerisMask[5] & prnShift) != 0);
                    satellite.mHasAlmanac = ((almanacMask[5] & prnShift) != 0);
                    satellite.mUsedInFix = ((usedInFixMask[5] & prnShift) != 0);
                } else if (prn >= 32 * 6 && prn < 32 * 7) {
                    int prnShift = (1 << (prn - 32 * 6));
                    satellite.mHasEphemeris = ((ephemerisMask[6] & prnShift) != 0);
                    satellite.mHasAlmanac = ((almanacMask[6] & prnShift) != 0);
                    satellite.mUsedInFix = ((usedInFixMask[6] & prnShift) != 0);
                } else if (prn >= 32 * 7 && prn < 32 * 8) {
                    int prnShift = (1 << (prn - 32 * 7));
                    satellite.mHasEphemeris = ((ephemerisMask[7] & prnShift) != 0);
                    satellite.mHasAlmanac = ((almanacMask[7] & prnShift) != 0);
                    satellite.mUsedInFix = ((usedInFixMask[7] & prnShift) != 0);
                } else {
                }
            }
        }
