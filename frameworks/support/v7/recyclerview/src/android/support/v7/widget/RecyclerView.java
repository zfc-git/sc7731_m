...
public class RecyclerView extends ViewGroup implements ScrollingView, NestedScrollingChild {
    ...
    public void scrollTo(int x, int y) {
        // SPRD: Bug 378508 - Animation calls RecyclerView.scrollTo() at special time while rotating
        // screen. However, the new widget RecyclerView overrides super.scrollTo() and does not
        // support scrolling to an absolute position. We make it do nothing here this time. If
        // there's request for using it we'll implement it later.
            ...
            if (holder == null) {
                ...
                if (offsetPosition < 0 || offsetPosition >= mAdapter.getItemCount()) {
                    throw new IndexOutOfBoundsException("Inconsistency detected. Invalid item "
                            + "position " + position + "(offset:" + offsetPosition + ")."
                            + ", mAdapter = " + mAdapter
                            + ", adapter count = " + mAdapter == null ? "-1": mAdapter.getItemCount()
                            + "state:" + mState.getItemCount());
                }
