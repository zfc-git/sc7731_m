...
public final class WifiP2pServiceImpl extends IWifiP2pManager.Stub {
    ...
    public static Boolean mIsP2pConnectingOrConnected = false;
    ...
    class InactiveState extends State {
        ...
        public void enter() {
            ...
            mIsP2pConnectingOrConnected = false;
        }
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case WifiMonitor.P2P_INVITATION_RECEIVED_EVENT:
                    ...
                    //check if this group is created from P2P-INVITATION-RECEIVED sa=b8:f9:34:2a:c7:9d persistent=0
                    //if yes, correct the deviceAddress
                    int groupNetid = group.getNetworkId();
                    if (groupNetid >= 0) {
                        String savedGOAddress = mGroups.getOwnerAddr(groupNetid);

                        if ((config.deviceAddress != null) && (savedGOAddress != null)
                                && (!config.deviceAddress.equals(savedGOAddress)) && (!mThisDevice.deviceAddress.equals(savedGOAddress))) {
                            loge("the GO devAddress saved in INVITATION_RECEIVED does not match the GO saved in mGroups"
                                + ", savedGOAddress:" + savedGOAddress + " <-> " + config.deviceAddress);
                            config.deviceAddress = savedGOAddress;
                        }
                    }
    ...
    class GroupCreatingState extends State {
        ...
        public void enter() {
           ...
           mIsP2pConnectingOrConnected = true;
        }
    ...
    private void initializeP2pSettings() {
        if (SystemProperties.get(WIFI_P2P_COEXISTENCE, "true").equals("false")) {
            mWifiNative.setPersistentReconnect(false);
        } else {
