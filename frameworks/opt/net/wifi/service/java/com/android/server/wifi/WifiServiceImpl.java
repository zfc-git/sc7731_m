...
public final class WifiServiceImpl extends IWifiManager.Stub {
    ...
    /**
     * SPRD: add for softap and bt coexsit feature.
     */
    private void resetSoftapFlag () {
        ContentResolver cr = mContext.getContentResolver();

        if (Settings.Global.getInt(cr, Settings.Global.SOFTAP_ENABLING_OR_ENABLED, 0) != 0)
            Settings.Global.putInt(cr, Settings.Global.SOFTAP_ENABLING_OR_ENABLED, 0);

        if (Settings.Global.getInt(cr, Settings.Global.SOFTAP_REENABLING, 0) != 0)
    ...
    public DhcpInfo getDhcpInfo() {
        ...
        for (RouteInfo ri : dhcpResults.getRoutes(mInterfaceName)) {
            if (ri.isDefaultRoute() == false && ri.hasGateway() == false) {
                IpPrefix dest = ri.getDestination();
                if (dest.getAddress() instanceof Inet4Address) {
                    info.netmask = NetworkUtils.prefixLengthToNetmaskInt(dest.getPrefixLength());
                }
            }
        }
    ...
    public List<String> softApGetClientWhiteList() {
        enforceChangePermission();
        return mWifiStateMachine.syncSoftApGetClientWhiteList(mWifiStateMachineChannel);
    }
    ...
    public boolean softApIsWhiteListEnabled() {
        enforceChangePermission();
        return mWifiStateMachine.syncSoftApIsWhiteListEnabled(mWifiStateMachineChannel);
    }
    ...
    public boolean setPasspointEnable(boolean enable) {
        enforceChangePermission();
        return mWifiStateMachine.setPasspointEnable(enable);
    }
    ...
    public String addPasspointCred(WifiConfiguration config) {
        ...
        if (mWifiStateMachineChannel != null) {
            return mWifiStateMachine.syncAddPasspointCred(mWifiStateMachineChannel, config);
        } else {
            Slog.e(TAG, "mWifiStateMachineChannel is not initialized");
            return null;
        }
    ...
    public boolean removePasspointCred(String credStringId) {
        ...
        if (mWifiStateMachineChannel != null) {
            return mWifiStateMachine.syncRemovePasspointCred(mWifiStateMachineChannel, credStringId);
        } else {
            Slog.e(TAG, "mWifiStateMachineChannel is not initialized");
            return false;
        }
    ...
    public List<ScanResult> getPasspointApList() {
        enforceChangePermission();
        return mWifiStateMachine.syncGetPasspointApList();
    }


    /**
     * see {@link android.net.wifi.WifiManager#getPasspointConfiguredCreds()}
     * @return the list of configured creds
     */
    public List<WifiConfiguration> getPasspointConfiguredCreds() {
        enforceAccessPermission();
        if (mWifiStateMachineChannel != null) {
            return mWifiStateMachine.syncGetPasspointConfiguredCreds(mWifiStateMachineChannel);
        } else {
            Slog.e(TAG, "mWifiStateMachineChannel is not initialized");
            return null;
        }
    ...
    /**
     * see {@link android.net.wifi.WifiManager#getPasspointConfiguredCred()}
     * @return the list of configured creds
     */
    public WifiConfiguration getPasspointConfiguredCred(String credStringId) {
        enforceAccessPermission();
        if (mWifiStateMachineChannel != null) {
            return mWifiStateMachine.syncGetPasspointConfiguredCred(mWifiStateMachineChannel, credStringId);
        } else {
            Slog.e(TAG, "mWifiStateMachineChannel is not initialized");
            return null;
        }
