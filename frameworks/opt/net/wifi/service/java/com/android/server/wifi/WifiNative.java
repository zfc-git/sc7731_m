...
public class WifiNative {
    ...
    private native boolean connectToHostapdNative(String iface);

    private native void closeHostapdConnectionNative(String iface);

    private native void stopConnectToHostapdNative(String iface);

    private native String waitForHostapdEventNative();

    private native boolean doHostapdBooleanCommandNative(String command);

    private native int doHostapdIntCommandNative(String command);

    private native String doHostapdStringCommandNative(String command);
...
//            auth = 0;
    ...
    synchronized public boolean startBgscan(boolean enabled) {
        return doBooleanCommand("SET bgscan "
                + (enabled ? "\"learn:300:-75:400:/data/misc/wifi/wpa_supplicant/network1.bgscan\""
    ...
    private boolean doHostapdBooleanCommand(String command) {
        ...
        synchronized (mLock) {
            ...
            boolean result = doHostapdBooleanCommandNative(mInterfacePrefix + command);
    ...
    private int doHostapdIntCommand(String command) {
        if (DBG) Log.d(mTAG, "doInt: " + command);
        synchronized (mLock) {
            int cmdId = getNewCmdIdLocked();
            localLog(cmdId + "->" + mInterfacePrefix + command);
            int result = doHostapdIntCommandNative(mInterfacePrefix + command);
            localLog(cmdId + "<-" + result);
            if (DBG) Log.d(mTAG, "   returned " + result);
            return result;
        }
    ...
    private String doHostapdStringCommand(String command) {
        ...
        synchronized (mLock) {
            int cmdId = getNewCmdIdLocked();
            localLog(cmdId + "->" + mInterfacePrefix + command);
            String result = doHostapdStringCommandNative(mInterfacePrefix + command);
            localLog(cmdId + "<-" + result);
            if (DBG) Log.d(mTAG, "   returned " + result);
            return result;
        }
    ...
    public String waitForHostapdEvent() {
        // No synchronization necessary .. it is implemented in WifiMonitor
        return waitForHostapdEventNative();
    }

    public boolean connectToHostapd() {
        return connectToHostapdNative(mInterfaceName);
    }

    public void closeHostapdConnection() {
        closeHostapdConnectionNative(mInterfaceName);
    }

    public void stopConnectToHostapd() {
        stopConnectToHostapdNative(mInterfaceName);
    }

    public boolean softApSetBlockStation(String mac, boolean enabled) {
        if (enabled) {
            return doHostapdBooleanCommand("DRIVER BLOCK " + mac);
        } else {
            return doHostapdBooleanCommand("DRIVER UNBLOCK " + mac);
        }
    }

    private static final String UNKOWN_COMMAND = "UNKNOWN COMMAND";
    public String softApGetBlockStationList() {
        String blockList = doHostapdStringCommand("DRIVER BLOCK_LIST");
    ...
    public boolean softApSetStationToWhiteList(String mac, boolean enabled) {
        ...
        if (enabled) {
            return doHostapdBooleanCommand("DRIVER WHITE_ADD " + mac);
        } else {
            return doHostapdBooleanCommand("DRIVER WHITE_DEL " + mac);
        }
    }
    ...
    public boolean softApSetWhiteListEnabled(boolean enabled) {
        if (enabled) {
            return doHostapdBooleanCommand("DRIVER WHITE_EN");//set macaddr_acl 1
        } else {
            return doHostapdBooleanCommand("DRIVER WHITE_DIS");//set macaddr_acl 0
        }
    }

    public boolean softApStartWpsPbc() {
            return doHostapdBooleanCommand("WPS_PBC");
    }

    public boolean softApStartWpsPinKeypad(String pin) {
        if (TextUtils.isEmpty(pin)) return false;
        return doHostapdBooleanCommand("WPS_PIN any " + pin);
    }

    public boolean softApCancelWps() {
        return doHostapdBooleanCommand("WPS_CANCEL");
    }

    public boolean softApCheckWpsPin(String pin) {
        if (TextUtils.isEmpty(pin)) return false;
        String res = doHostapdStringCommand("WPS_CHECK_PIN " + pin);
