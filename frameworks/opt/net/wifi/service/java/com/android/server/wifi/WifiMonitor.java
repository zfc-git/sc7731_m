...
public class WifiMonitor {
   ...
   /* Network connection completed */
    ...
    public static final int HOSTAPD_CONNECTION_EVENT                 = BASE + 65;
    public static final int HOSTAPD_DISCONNECTION_EVENT                 = BASE + 66;
    ...
    //NOTE: Add for SoftAp Advance Feature -->
    /**
    * Below monitor of softap(hostapd), notify event such as STA connected to the softap.
    */
    public void startSoftApMonitoring() {
        new SoftApMonitorThread(mStateMachine, mWifiNative).start();
    }

    //to stop connect to hostapd to let softap monitor exit.
    public void stopSoftApMonitoring() {
        Log.i(TAG, "stop connect to Hostapd");
        mWifiNative.stopConnectToHostapd();
    }

    private static class SoftApMonitorThread extends Thread {
        private final WifiNative mWifiNative;
        private StateMachine mStateMachine = null;

        public SoftApMonitorThread(StateMachine wifiStateMachine, WifiNative wifiNative) {
            super("WifiMonitor");
            mWifiNative = wifiNative;
        ...
        public void run() {
            if (connectToHostapd()) {
                mStateMachine.sendMessage(HOSTAPD_CONNECTION_EVENT);
                Log.i(TAG, "Connect to hostapd succeed");
            } else {
                mStateMachine.sendMessage(HOSTAPD_DISCONNECTION_EVENT);
                Log.i(TAG, "Connect to hostapd fail");
                return;
            }

            for (;;) {
                String eventStr = mWifiNative.waitForHostapdEvent();

                String[] tokens = eventStr.split(" ");
                String macStr = "";

                Log.i(TAG, eventStr);
                if(eventStr.startsWith(AP_STA_CONNECTED_STR)) {//"AP-STA-CONNECTED"
                    //Log.i(TAG, "Show the connect toast");

                    /* AP-STA-CONNECTED 42:fc:89:a8:96:09*/
                    if(tokens.length < 2) {
                        Log.i(TAG, "Without MAC string");
                    }
                    else{
                        macStr = tokens[1];
                    }

                    mStateMachine.sendMessage(AP_STA_CONNECTED_EVENT, macStr);
                } else if(eventStr.startsWith(AP_STA_DISCONNECTED_STR)) {//"AP-STA-DISCONNECTED"
                    //Log.i(TAG, "Show the disconnect toast");
                    ...
                    mStateMachine.sendMessage(AP_STA_DISCONNECTED_EVENT, macStr);
                } else if(eventStr.equals("AP-TERMINATING")) {//"AP-TERMINATING"
                    //mWifiNative.closeHostapdConnection();
                    mStateMachine.sendMessage(HOSTAPD_DISCONNECTION_EVENT);
                    Log.i(TAG, "Hostapd is terminating");
                    break;
                } else {
        ...
        private boolean connectToHostapd() {
            ...
            while (true) {
                if (mWifiNative.connectToHostapd()) {
                    return true;
                }
                if (connectTries++ < 5) {
                    try {
