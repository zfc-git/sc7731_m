...
public class WifiAutoJoinController {
    ...
    */
    ...
    boolean attemptAutoJoin() {
        ...
        //NOTE: Add for SPRD Passpoint R1 Feature -->
        if (candidate == null && mWifiConfigStore.allowPasspointAutoConnections()) {
            logDbg("AutoJoinController: try to find a passpoint candidate ");

            // Get current date
            long nowMs = System.currentTimeMillis();
            int currentScore = -10000;
            // The passpoint network with highest score
            PasspointMatchInfo passpointCandidate = null;

            for (ScanDetail scanDetail : scanResultCache.values()) {
                ScanResult result = scanDetail.getScanResult();

                logDbg("AutoJoinController: try to find a passpoint candidate 111:  " + result + " untrusted:" + result.untrusted);

                // A passpoint should be a trusted AP. We look only at trusted networks with a valid SSID
                if (TextUtils.isEmpty(result.SSID) || result.untrusted ) {
                    continue;
                }
                ...
                if ((nowMs - result.seen) < mScanResultAutoJoinAge) {
                    ...
                    PasspointMatchInfo passpointMatchInfo = mWifiConfigStore.matchBestPasspointNetwork(scanDetail);
                    if (passpointMatchInfo != null)
                        score = passpointMatchInfo.getScore();

                    if (score != WifiNetworkScoreCache.INVALID_NETWORK_SCORE
                            && score > currentScore) {
                        // Highest score: Select this candidate
                        currentScore = score;
                        passpointCandidate = passpointMatchInfo;
                        if (VDBG) {
                            logDbg("AutoJoinController: found a passpoint candidate "
                                    + result.SSID
                                    + " RSSI=" + result.level
                                    + " freq=" + result.frequency
                                    + " score=" + score
                                    + " passpointCandidate=" + passpointCandidate);
                        }
                    }
                }
            }
            if (passpointCandidate != null) {
                // At this point, we have a passpoint network candidate.
                // Create the new passpoint configuration and see if we should switch over
                candidate =
                        mWifiConfigStore.wifiConfigurationFromPasspointMatch(passpointCandidate);
                if (candidate != null) {
                    //candidate.allowedKeyManagement.set(KeyMgmt.NONE);
                    //candidate.ephemeral = true;
                    candidate.dirty = true;
