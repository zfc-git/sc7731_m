    ...
    */
    ...
    private static WifiConfiguration buildSIMConfig(HomeSP homeSP, Context context)
        ...
        */
        ...
        config.eap_sim_slot = credential.getSimNum();
    ...
    private static WifiConfiguration buildTLSConfig(HomeSP homeSP, Context context)
            throws IOException, GeneralSecurityException {
        ...
        WifiConfiguration config = buildBaseConfiguration(homeSP);
        WifiEnterpriseConfig enterpriseConfig = config.enterpriseConfig;
        enterpriseConfig.setClientCertificateAlias(credential.mClientCertAlias);
        enterpriseConfig.setCaCertificateAlias(credential.mCACertAlias);

        return config;
    }


    public static WifiConfiguration buildConfig(HomeSP homeSP, Context context)
            throws IOException, GeneralSecurityException {
        ...
        switch (eapMethodID) {
            case EAP_TTLS:
                ...
                break;
            case EAP_TLS:
                config = buildTLSConfig(homeSP, context);
                break;
            case EAP_AKA:
            case EAP_AKAPrim:
            case EAP_SIM:
        ...
        if (eapMethodID == EAP.EAPMethodID.EAP_TTLS || eapMethodID == EAP.EAPMethodID.EAP_TLS)
            enterpriseConfig.setAnonymousIdentity("anonymous@" + credential.getRealm());
        enterpriseConfig.setRealm(credential.getRealm());
