...
public class Credential {
    ...
    /**
     * Priority determines the preference given to a cred
     * when choosing an access point with which to associate.
     */
    public int mPriority = 0;
    public int mSimNum = -1;

    //Only for TLS
    public String mCACertAlias;
    ...
    public Credential(WifiEnterpriseConfig enterpriseConfig, KeyStore keyStore, boolean update)
        ...
        if (enterpriseConfig.getClientCertificate() != null) {
            ...
            //NOTE: Add for SPRD Passpoint R1 Feature -->
            mCACertAlias = enterpriseConfig.getCaCertificateAlias();
            mClientCertAlias = enterpriseConfig.getClientCertificateAlias();
            //<-- Add for SPRD Passpoint R1 Feature
    ...
    public static EAP.EAPMethodID mapEapMethodID(int eapMethod) {
        switch (eapMethod) {
            case WifiEnterpriseConfig.Eap.TTLS:
                return EAP.EAPMethodID.EAP_TTLS;
            case WifiEnterpriseConfig.Eap.SIM:
                return EAP.EAPMethodID.EAP_SIM;
            case WifiEnterpriseConfig.Eap.AKA:
                return EAP.EAPMethodID.EAP_AKA;
            case WifiEnterpriseConfig.Eap.AKA_PRIME:
                return EAP.EAPMethodID.EAP_AKAPrim;
            default:
                return EAP.EAPMethodID.EAP_TLS;
        }
    }
