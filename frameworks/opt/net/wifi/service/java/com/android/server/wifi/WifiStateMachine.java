...
public class WifiStateMachine extends StateMachine implements WifiNative.WifiPnoEventHandler {
    ...
    //add for 7731gea with 2351 that cannot support wifi/p2p coexistence
    private final boolean mP2pWifiCoexistSupported;
    private static final String WIFI_P2P_COEXISTENCE = "ro.wifip2p.coexist";
    ...
    private final boolean mPasspointSupported;
    //<-- Add for Passpoint R1 Feature
    ...
    */
    ...
    // SPRD: for bug#430212: scan request fail because of busy such scan
    //then mIsFullScanOngoing will be false, and the scan result will be broadcasted
    private boolean mIsFullScanRequest = false;
    ...
    //NOTE: Add For SoftAp Advance Feature -->
    static final int CMD_AP_BLOCK_STATION         = BASE + 162;
    static final int CMD_AP_UNBLOCK_STATION     = BASE + 163;
    static final int CMD_AP_GET_BLOCK_STATION     = BASE + 164;
    static final int CMD_AP_ADD_WHITE_STATION         = BASE + 165;
    static final int CMD_AP_DEL_WHITE_STATION     = BASE + 166;
    static final int CMD_AP_GET_WHITE_STATION     = BASE + 167;
    static final int CMD_AP_SET_STATION_BLOCKMODE     = BASE + 168;
    static final int CMD_AP_GET_STATION_BLOCKMODE     = BASE + 169;
    //<-- Add For SoftAp Advance Feature



    //NOTE: Add for SPRD Passpoint R1 Feature -->
    static final int CMD_PASSPOINT_SET_ENABLE    = BASE + 173;
    static final int CMD_PASSPOINT_ADD_CONFIG                 = BASE + 174;
    static final int CMD_PASSPOINT_REMOVE_CONFIG            = BASE + 175;
    static final int CMD_PASSPOINT_GET_ALL_CONFIG_CREDS            = BASE + 176;
    static final int CMD_PASSPOINT_GET_CONFIG_CRED            = BASE + 177;
    ...
    private boolean mSupportCUCC = false;
    private int mNetworkType = 1;
    private int mMobileDataStatus = 1;
    /* @} */
    ...
    private static final boolean IS_SUPPORT_POCKETMODE = SystemProperties.getBoolean("persist.sys.wifi.pocketmode", false);
    ...
    public WifiStateMachine(Context context, String wlanInterface,
        ...
        mSigMotion = mSensorManager.getDefaultSensor(Sensor.TYPE_SPRDHUB_POCKET_MODE); //Sensor.TYPEPROXIMITY is used in code development phase
        mSensorEventListener = new WifiSensorEventListener();

        // 7731gea with 2351 can not support wifi / p2p coexistense
        if (SystemProperties.get(WIFI_P2P_COEXISTENCE, "true").equals("false")) {
            mP2pWifiCoexistSupported = false;
        } else {
            mP2pWifiCoexistSupported = true; //default is true
        }
                    ...
                    new ContentObserver(getHandler()) {
                        ...
                        public void onChange(boolean selfChange) {
                            if(Settings.Global.getInt(mContext.getContentResolver(),
                                ...
                                mWifiNative.startBgscan(true);
                            } else {
                                mAutoRoamSwitchFlag.set(false);
                                mWifiNative.startBgscan(false);
                            }
                        }
                    });

        /* SPRD: disable wifi when change to MOBILE, add for cucc @{ */
        mSupportCUCC = SystemProperties.getBoolean("ro.wifi.support_cucc", false);
        if(mSupportCUCC) {
            mNetworkType = Settings.Secure.getInt(mContext.getContentResolver(),
                    Settings.Global.NETWORK_PREFERENCE, WIFI_DATA_TYPE);
            mMobileDataStatus = Settings.Secure.getInt(mContext.getContentResolver(),
                    Settings.Global.MOBILE_DATA, MOBILE_DATA_ENABLE);
            mContext.getContentResolver().registerContentObserver(Settings.Global.getUriFor(
                    Settings.Global.NETWORK_PREFERENCE), false,
                    new ContentObserver(getHandler()) {
                @Override
                public void onChange(boolean selfChange) {
                    mNetworkType = Settings.Secure.getInt(mContext.getContentResolver(),
                            Settings.Global.NETWORK_PREFERENCE, WIFI_DATA_TYPE);
                    if(mNetworkType == MOBILE_DATA_TYPE) {
                        setOperationalMode(SCAN_ONLY_MODE);
                    } else {
                        setOperationalMode(CONNECT_MODE);
                        reconnectCommand();
                    }
                }
            });
            mContext.getContentResolver().registerContentObserver(Settings.Global.getUriFor(
                    Settings.Global.MOBILE_DATA), false,
                    new ContentObserver(getHandler()) {
                @Override
                public void onChange(boolean selfChange) {
                    mMobileDataStatus = Settings.Secure.getInt(mContext.getContentResolver(),
                            Settings.Global.MOBILE_DATA, MOBILE_DATA_ENABLE);
                    if(mNetworkType == MOBILE_DATA_TYPE) {
                        if(mMobileDataStatus == MOBILE_DATA_ENABLE){
                            setOperationalMode(SCAN_ONLY_MODE);
                        } else {
                            setOperationalMode(CONNECT_MODE);
                            reconnectCommand();
        ...
        mContext.getContentResolver().registerContentObserver(Settings.Global.getUriFor(
                ...
                new ContentObserver(getHandler()) {
                    ...
                    public void onChange(boolean selfChange) {
                        mPocketModeSwitch = Settings.Global.getInt(mContext.getContentResolver(),
                                Settings.Global.POWER_SAVING, 0);
                        if (IS_SUPPORT_POCKETMODE == true) {
                            if (mPocketModeSwitch == 1 && mWifiState.get() == WIFI_STATE_ENABLED) {
                                if (mIsRequestTriggerSensor == false) {
                                    mSensorManager.registerListener(mSensorEventListener, mSigMotion, SensorManager.SENSOR_DELAY_NORMAL);
        ...
        mWifiApClientStats = new WifiApClientStats(context, this);

        //NOTE: Add for SPRD Passpoint R1 Feature -->
        mPasspointSupported = mWifiConfigStore.mPasspointSupported;
        //<-- Add for SPRD Passpoint R1 Feature
    ...
    public void setOperationalMode(int mode) {
        ...
        if(mSupportCUCC && (mMobileDataStatus == MOBILE_DATA_ENABLE) && (mNetworkType == MOBILE_DATA_TYPE)){
            mode = SCAN_ONLY_MODE;
        }
    ...
    public int syncAddOrUpdateNetwork(AsyncChannel channel, WifiConfiguration config) {
        int i = 0;
        while (mWifiState.get() != WIFI_STATE_ENABLED && ++i <= 15) {
            try {
                Thread.sleep(100);
            } catch (InterruptedException ignore) {
            }
            Log.i(TAG, "waiting " + i + "x100 ms for wifi enabled");
        }
    ...
    public synchronized void setCountryCode(String countryCode, boolean persist) {
        ...
        if (TextUtils.isEmpty(countryCode)) {
            ...
            if (countryCodeSequence == 0
                    || countryCode.equals(mSetCountryCode) == false || countryCode.equals(mDriverSetCountryCode) == false) {
    ...
    private void setCountryCode() {
         ...
         * which is from the current operator numeric property and is not empty.
         */
    ...
    private void setWifiApState(int wifiApState, int reason) {
        ...
        if (!supportBtWifiSoftApCoexit) {
            ...
            if (wifiApState == WIFI_AP_STATE_DISABLING || wifiApState == WIFI_AP_STATE_DISABLED) {
                if (Settings.Global.getInt(cr, Settings.Global.SOFTAP_ENABLING_OR_ENABLED, 0) == 1) {
                    Settings.Global.putInt(cr, Settings.Global.SOFTAP_ENABLING_OR_ENABLED, 0);
                }
            } else {
                if (Settings.Global.getInt(cr, Settings.Global.SOFTAP_REENABLING, 0) == 1) {
                    Settings.Global.putInt(cr, Settings.Global.SOFTAP_ENABLING_OR_ENABLED, 1);
                    Settings.Global.putInt(cr, Settings.Global.SOFTAP_REENABLING, 0);
                }
            }
        }
    ...
    private void setScanResults() {
        ...
        tsf = TimeUnit.NANOSECONDS.toMicros(SystemClock.elapsedRealtimeNanos());
        ...
        if (attemptAutoJoin) {
            if (mP2pWifiCoexistSupported ||
                    (!mP2pWifiCoexistSupported && !WifiP2pServiceImpl.mIsP2pConnectingOrConnected))
            synchronized (mScanResultCache) {
                // AutoJoincontroller will directly acces the scan result list and update it with
                // ScanResult status
                if (WifiManager.SUPPORT_CMCC) attemptAutoJoin = false; // Do not dirctly acces ap in cmcc versions
            ...
            case CMD_UPDATE_LINKPROPERTIES:
                ...
                if (lostProvisioning) {
                    if (getCurrentState() == mObtainingIpState) {
                        loge("mObtainingIpState do not send CMD_IP_CONFIGURATION_LOST msg.");
                        break;
                    }
    ...
    private void startSoftApWithConfig(final WifiConfiguration configuration) {
        ...
        if (mWifiNative.isHalStarted()) {
            ...
            config.apBand = 0;
            //NOTE: CHANGED for SoftAp Advance Feature -->
            if (config.apChannel <= 0 || config.apChannel > 14)
                config.apChannel = 6;
            //<-- CHANGED for SoftAp Advance Feature
        }
        // Start hostapd on a separate thread
        new Thread(new Runnable() {
            public void run() {
                try {
                    if (mWifiApClientStats != null) {//config white list enabled or not
                        mNwService.setAccessPointWhiteListEnable(mWifiApClientStats.isWhiteListEnabled());
                    }
    ...
    class DefaultState extends State {
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case WifiMonitor.HOSTAPD_CONNECTION_EVENT:
                //<-- Add For SoftAp Advance Feature
                // Broadcom, WAPI
                ...
                //NOTE: Add for SoftAp Advance Feature -->
                case WifiMonitor.HOSTAPD_DISCONNECTION_EVENT:
                    if(mConnectedToHostapd) {
                        mWifiNative.closeHostapdConnection();
                        mConnectedToHostapd = false;
                        if (DBG) log("WifiMonitor: Close to hostapd sockets!");
                    }
                    mHostapdMonitorStarted = false;
                    break;
                ...
                case CMD_AP_GET_BLOCK_STATION:
                    replyToMessage(message, message.what, (String)"");

                    break;
                //For softap advance end
                //--> softap White list feature
                case CMD_AP_ADD_WHITE_STATION:
                    SoftApClient client = (SoftApClient)message.obj;
                    if (client == null) {
                        replyToMessage(message, message.what, FAILURE);
                    } else {
                ...
                //NOTE: Add for SPRD Passpoint R1 Feature -->
                case CMD_PASSPOINT_SET_ENABLE:
                    enable = (message.arg1 == 1);
                    if (mPasspointSupported) {
                        mWifiConfigStore.setPasspointAutoConnections(enable);
                    }
                    break;
                case CMD_PASSPOINT_ADD_CONFIG:
                    WifiConfiguration credConfig = (WifiConfiguration) message.obj;
                    if (mPasspointSupported) {
                        String res = mWifiConfigStore.addPasspointCred(credConfig);
                ...
                case CMD_PASSPOINT_REMOVE_CONFIG:
                    String credStringId = (String) message.obj;
                    if (mPasspointSupported) {
                        boolean res = mWifiConfigStore.removePasspointCred(credStringId);
                        replyToMessage(message, message.what, res);
                    } else {
                        replyToMessage(message, message.what, FAILURE);
                    }
                ...
                case CMD_PASSPOINT_GET_ALL_CONFIG_CREDS:
                    if (mPasspointSupported) {
                        replyToMessage(message, message.what,
                                mWifiConfigStore.getPasspointConfiguredCredList());
                    } else {
                        replyToMessage(message, message.what, (List<WifiConfiguration>) null);
                    }
                    break;

                case CMD_PASSPOINT_GET_CONFIG_CRED:
                    credStringId = (String) message.obj;
                    if (mPasspointSupported && credStringId != null) {
                        replyToMessage(message, message.what,
                                mWifiConfigStore.getPasspointConfiguredCred(credStringId));
                    } else {
                        replyToMessage(message, message.what, (WifiConfiguration) null);
                    }
                    break;
                //<-- Add for Passpoint R1 Feature
        ...
        public boolean processMessage(Message message) {
            ...
            switch(message.what) {
                case WifiMonitor.SUP_CONNECTION_EVENT:
                    ...
                    mWifiNative.startBgscan(mAutoRoamSwitchFlag.get());
        ...
        public boolean processMessage(Message message) {
            ...
            switch(message.what) {
                case CMD_STOP_SUPPLICANT:   /* Supplicant stopped by user */
                    ...
                    if(WifiManager.SUPPORT_CMCC && mNetworkInfo.isConnectedOrConnecting()) {
                        Intent wifiDisconnectIntent = new Intent(WifiManager.WIFI_DISABLED_WHEN_CONNECTED_ACTION);
                ...
                case WifiMonitor.SCAN_FAILED_EVENT:
                    ...
                    noteScanEnd();
                    setScanResults();
                    if (mIsFullScanOngoing || mSendScanResultsBroadcast || mIsFullScanRequest) {
...
//                        startScanNative(WifiNative.SCAN_WITHOUT_CONNECTION_SETUP, null);
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case WifiMonitor.SUPPLICANT_STATE_CHANGE_EVENT:
                    ...
                    //SPRD: restartDhcp if the supplicant state changes from CONNECTED to ASSOCIATING
                ...
                case WifiManager.CONNECT_NETWORK:
                    ...
                    if (config != null) {
                        ...
                        //NOTE: Add for SPRD Passpoint R1 Feature -->
                        if (mPasspointSupported && savedConfig == null) {
                            if (config.createdFromPasspointCred && config.passpointCredIdentifier != null
                                && (config.enterpriseConfig == null ||
                                (config.enterpriseConfig != null && config.enterpriseConfig.getEapMethod() == WifiEnterpriseConfig.Eap.NONE))
                                ) {
                                logw("This config is simply generated for passpoint matched!!");
                                WifiConfiguration newConfig = mWifiConfigStore.wifiConfigurationFromPasspointMatch(config);
                ...
                case WifiMonitor.NETWORK_CONNECTION_EVENT:
                    ...
                    sendNetworkStateChangeBroadcast(mLastBssid);
                    mWifiAutoJoinController.updateConfigurationHistory(mLastNetworkId, true, true);
                    transitionTo(mObtainingIpState);
                    break;
                case WifiMonitor.NETWORK_DISCONNECTION_EVENT:
    ...
    class L2ConnectedState extends State {
        ...
        public void enter() {
            ...
            //Add for the SPRD AUTO Roam Feature -->
                ...
                mIpReachabilityMonitor = new IpReachabilityMonitor(
                        ...
                        new IpReachabilityMonitor.Callback() {
                            ...
                            public void notifyLost(InetAddress ip, String logMsg) {
                                ...
                                logd("Do not send CMD_IP_REACHABILITY_LOST when notifyLost");
                            }
                        });
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
              case DhcpStateMachine.CMD_PRE_DHCP_ACTION:
                  ...
                  break;
              case DhcpStateMachine.CMD_POST_DHCP_ACTION:
                  //Changed for the SPRD AUTO Roam Feature -->
                  mIsAutoRoamOngoing = false;
                  //<-- Changed for the SPRD AUTO Roam Feature
                  handlePostDhcpSetup();
                    ...
                    if (message.arg1 == SCAN_ALARM_SOURCE) {
                        ...
                        if (currentConfiguration != null) {
                            ...
                            if (tryFullBandScan) {
                                ...
                                if (mAutoRoamSwitchFlag.get()) {
                                    handleScanRequest(
                                            WifiNative.SCAN_WITH_CONNECTION_SETUP, message);
                                } else {
                                    handleScanRequest(
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case WifiMonitor.NETWORK_DISCONNECTION_EVENT:
                    ...
                    if(WifiManager.SUPPORT_CMCC && (mWifiInfo.getSupplicantState() == SupplicantState.COMPLETED)) {
                        WifiInfo tmpInfo = new WifiInfo(mWifiInfo);
                        mWifiInfo.setSupplicantState(SupplicantState.DISCONNECTED);
                        Intent apDiscIntent = new Intent(WifiManager.WIFI_CONNECTED_AP_ABSENT_ACTION);
    ...
    class WpsRunningState extends State {
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                 ...
                 */
                case CMD_STOP_DRIVER:
                //Bug#303655,for case turning off/on screen while in wpsrunningstate with WIFI_SLEEP_POLICY_DEFAULT
                case CMD_START_DRIVER:
                case CMD_SET_OPERATIONAL_MODE:
                case WifiManager.CONNECT_NETWORK:
                case CMD_ENABLE_NETWORK:
        ...
        public boolean processMessage(Message message) {
            ...
            switch(message.what) {
                case CMD_STOP_AP:
                    ...
                    //NOTE: Add for SoftAp Advance Feature -->
                    //Clear connected stattion when stop softap
                    mConnectedStations = "";
                    if(mConnectedToHostapd) {
                        if (DBG) log("Stopping Soft AP Monitor");
                        mWifiMonitor.stopSoftApMonitoring();
                    }
                    //<-- Add for SoftAp Advance Feature
                    break;
                case CMD_START_AP:
                ...
                case CMD_AP_GET_BLOCK_STATION:
                    if(mConnectedToHostapd) {
                        ...
                        replyToMessage(message, message.what, mWifiNative.softApGetBlockStationList());
                ...
                case CMD_AP_ADD_WHITE_STATION:
                    ...
                    if (client == null) {
                        ...
                        if(mConnectedToHostapd) {
                            mWifiNative.softApSetStationToWhiteList(client.mac, true);
                        }
                        if (mWifiApClientStats != null) {
                            mWifiApClientStats.addClientToWhiteList(client.mac, client.name, true);
                        }
                ...
                case CMD_AP_DEL_WHITE_STATION:
                    ...
                    if (client == null) {
                        ...
                        if(mConnectedToHostapd) {
                            mWifiNative.softApSetStationToWhiteList(client.mac, false);
                        }
                        if (mWifiApClientStats != null) {
                            mWifiApClientStats.delClientFromWhiteList(client.mac, client.name, true);
                        }
                ...
                case CMD_AP_SET_STATION_BLOCKMODE:
                    boolean enable = (message.arg1 == 1);
                    if(mConnectedToHostapd) {
                        mWifiNative.softApSetWhiteListEnabled(enable);
                    }
                    if (mWifiApClientStats != null) {
                        mWifiApClientStats.setClientWhiteListEnabled(enable, true);
                    }
            ...
            switch(message.what) {
                case CMD_TETHER_STATE_CHANGE:
                    ...
                    if (isWifiTethered(stateChange.active)) {
                        ...
                        //NOTE: Add for SoftAp Advance Feature -->
                        //start softap monitor, to receive softap event such as STA connected
                        if(!mHostapdMonitorStarted) {
                            mHostapdMonitorStarted = true;
                            mWifiMonitor.startSoftApMonitoring();
                            //Note: socket connect to hostapd will be closed when stop ap in SoftapStartedState
                        }
                        //<-- Add for SoftAp Advance Feature
                    }
                    return HANDLED;
    ...
    class TetheredState extends State {
        ...
        @Override
        public void enter() {
            /* start Sofap client info collection */
            if (mWifiApClientStats != null) {
                if (DBG) log("start Sofap client info collection");
        ...
        public boolean processMessage(Message message) {
            ...
            switch(message.what) {
                ...
                //NOTE: Add for SoftAp Advance Feature -->
                case WifiMonitor.HOSTAPD_DISCONNECTION_EVENT:
                    if (DBG) log("Unexcept HOSTAPD_DISCONNECTION_EVENT restart");
                    if(!mHostapdMonitorStarted) log("ERROR:Unexcept HOSTAPD_DISCONNECTION_EVENT, something error!!");
                    if(!mConnectedToHostapd) log("ERROR222:Unexcept HOSTAPD_DISCONNECTION_EVENT, something error!!");
               ...
               case WifiManager.SOFTAP_WPS_CHECK_PIN:
                    ...
                    boolean ok = mWifiNative.softApCheckWpsPin(wpsPin);
                    replyToMessage(message, message.what, ok ? SUCCESS : FAILURE);
                    break;
        ...
        @Override
        public void exit() {
            /* stop Sofap client info collection */
            if (mWifiApClientStats != null) {
                if (DBG) log("stop Sofap client info collection");
    ...
    void handleGsmAuthRequest(SimAuthRequestData requestData) {
        ...
        if (tm != null) {
            ...
            for (String challenge : requestData.data) {
                 ...
                 */
                int appType = 2;
                String tmResponse = tm.getIccSimChallengeResponse(subId,appType, base64Challenge);
                if (tmResponse == null) {
    ...
    /**
     * SPRD: Get the current country code
     * @return country code
     */
    private String getCurrentCountryCode() {
        /*
        //operatorNumeric contains mcc and mnc
        String operatorNumeric = SystemProperties.get(TelephonyProperties.PROPERTY_OPERATOR_NUMERIC, "");
        int mcc = 0;
        try {
            mcc = Integer.parseInt(operatorNumeric.substring(0,3));
        } catch (Exception e) {
            // TODO: handle exception
            Log.e(TAG, "Error getting operatorNumeric = " + operatorNumeric  + ", "+ e);
            return null;
        }

        if (mcc != 0) {
            String country = PublicMccTable.countryCodeForMcc(mcc);
            return country;
        }
        return null;
        */
     ...
     */
    public List<String> syncSoftApGetClientWhiteList(AsyncChannel channel) {
        if (mWifiApClientStats != null)
    ...
    public boolean syncSoftApIsWhiteListEnabled(AsyncChannel channel) {
        if (mWifiApClientStats != null)
    ...
    public boolean setPasspointEnable(boolean enable) {
        if (DBG) log("setPasspointEnable to " + enable);
        sendMessage(CMD_PASSPOINT_SET_ENABLE, enable ? 1 : 0, 0);
        return true;
    }

    public List<ScanResult> syncGetPasspointApList() {
        if (!mPasspointSupported) {
        ...
        synchronized (mScanResultCache) {
            List<ScanResult> scanList = new ArrayList<ScanResult>();
            for (ScanDetail result : mScanResults) {
                if (result.getScanResult().isPasspointNetwork()) {
                    scanList.add(new ScanResult(result.getScanResult()));
                }
            }
            return scanList;
    ...
    /**
     * Add a passpoint cred synchronously
     *
     * @return cred String id of the new cred, if fail, return null
     */
    public String syncAddPasspointCred(AsyncChannel channel, WifiConfiguration config) {
        Message resultMsg = channel.sendMessageSynchronously(CMD_PASSPOINT_ADD_CONFIG, config);
        String result = (String) resultMsg.obj;
        resultMsg.recycle();
        return result;
    }
    ...
    /**
     * Delete a cred config
     *
     * @param cred String id of the cred config to be removed
     */
    public boolean syncRemovePasspointCred(AsyncChannel channel, String credStringId) {
        Message resultMsg = channel.sendMessageSynchronously(CMD_PASSPOINT_REMOVE_CONFIG, credStringId);
        boolean result = (resultMsg.arg1 != FAILURE);
        resultMsg.recycle();
        return result;
    }
    ...
    /**
     * Get the configured creds synchronously
     */
    public List<WifiConfiguration> syncGetPasspointConfiguredCreds(AsyncChannel channel) {
        Message resultMsg = channel.sendMessageSynchronously(CMD_PASSPOINT_GET_ALL_CONFIG_CREDS);
        List<WifiConfiguration> result = (List<WifiConfiguration>) resultMsg.obj;
        resultMsg.recycle();
        return result;
    }
    ...
    public WifiConfiguration syncGetPasspointConfiguredCred(AsyncChannel channel, String credStringId) {
        Message resultMsg = channel.sendMessageSynchronously(CMD_PASSPOINT_GET_CONFIG_CRED, credStringId);
