...
static jboolean android_net_wifi_connectToHostapd(JNIEnv* env, jobject, jstring jIface)
{
    ScopedUtfChars ifname(env, jIface);
    return (jboolean)(::wifi_connect_to_hostapd(ifname.c_str()) == 0);
}

static void android_net_wifi_closeHostapdConnection(JNIEnv* env, jobject, jstring jIface)
{
    ScopedUtfChars ifname(env, jIface);
    ::wifi_close_hostapd_connection(ifname.c_str());
}

static void android_net_wifi_stopConnectToHostapd(JNIEnv* env, jobject, jstring jIface)
{
    ScopedUtfChars ifname(env, jIface);
    ::wifi_stop_connect_to_hostapd(ifname.c_str());
}

static bool hostapd_doCommand(JNIEnv* env, jstring javaCommand,
                      char* reply, size_t reply_len) {
    ScopedUtfChars command(env, javaCommand);
    if (command.c_str() == NULL) {
        return false; // ScopedUtfChars already threw on error.
    ...
    --reply_len; // Ensure we have room to add NUL termination.
    if (::wifi_hostapd_command(command.c_str(), reply, &reply_len) != 0) {
        return false;
    }
...
static jstring android_net_wifi_hostapd_doStringCommand(JNIEnv* env, jobject, jstring javaCommand) {
    char reply[REPLY_BUF_SIZE];
    if (!hostapd_doCommand(env, javaCommand, reply, sizeof(reply))) {
        return NULL;
    }
    return env->NewStringUTF(reply);
}

static jboolean android_net_wifi_hostapd_doBooleanCommand(JNIEnv* env, jobject, jstring javaCommand) {
    char reply[REPLY_BUF_SIZE];
    if (!hostapd_doCommand(env, javaCommand, reply, sizeof(reply))) {
        return JNI_FALSE;
    }
...
static jint android_net_wifi_hostapd_doIntCommand(JNIEnv* env, jobject, jstring javaCommand) {
    char reply[REPLY_BUF_SIZE];
    if (!hostapd_doCommand(env, javaCommand, reply, sizeof(reply))) {
        return -1;
    }
...
static jstring android_net_wifi_hostapd_waitForEvent(JNIEnv* env, jobject)
{
    char buf[EVENT_BUF_SIZE];
    int nread = ::wifi_hostapd_wait_for_event(buf, sizeof buf);
    if (nread > 0) {
...
//NOTE: Add for SoftAp Advance Feature -->
#ifndef CONFIG_NO_HOSTAPD_ADVANCE
    { "connectToHostapdNative", "(Ljava/lang/String;)Z",
            (void *)android_net_wifi_connectToHostapd},
    { "closeHostapdConnectionNative", "(Ljava/lang/String;)V",
            (void *)android_net_wifi_closeHostapdConnection },
    { "stopConnectToHostapdNative", "(Ljava/lang/String;)V",
            (void *)android_net_wifi_stopConnectToHostapd },
    { "waitForHostapdEventNative", "()Ljava/lang/String;", (void*)android_net_wifi_hostapd_waitForEvent },
    { "doHostapdBooleanCommandNative", "(Ljava/lang/String;)Z", (void*)android_net_wifi_hostapd_doBooleanCommand },
    { "doHostapdIntCommandNative", "(Ljava/lang/String;)I", (void*)android_net_wifi_hostapd_doIntCommand },
    { "doHostapdStringCommandNative", "(Ljava/lang/String;)Ljava/lang/String;",
            (void*) android_net_wifi_hostapd_doStringCommand }
#endif
//<-- Add for SoftAp Advance Feature
};
