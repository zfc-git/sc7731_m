 ...
 * @hide
 */
public class ImsUt implements ImsUtInterface,ImsUtInterfaceEx {
    /**
    ...
    private final IImsUtEx mIImsUtEx;//SPRD:add for VoLTE
    private HashMap<Integer, Message> mPendingCmds =
            new HashMap<Integer, Message>();

    //SPRD:add IImsUtEx for VoLTE
    public ImsUt(IImsUt iUt, IImsUtEx iImsUtEx) {
        miUt = iUt;
        mIImsUtEx = iImsUtEx;
        if (miUt != null) {
            try {
    ...
    @Override
    public void setCallForwardingOption(int serviceId, int commandInterfaceCFAction, int commandInterfaceCFReason,
            int serviceClass, String dialingNumber, int timerSeconds, String ruleSet,
            Message result) {
        if (DBG) {
            log("getCallForwardingOption :: mIImsUtEx=" + mIImsUtEx + ", serviceId=" + serviceId
                    + ", commandInterfaceCFAction=" + commandInterfaceCFAction
                    + ", commandInterfaceCFReason=" + commandInterfaceCFReason
                    + ", serviceClass=" + serviceClass +", dialingNumber=" + dialingNumber
                    + ", timerSeconds=" + timerSeconds +", ruleSet=" + ruleSet);
        }

        synchronized(mLockObj) {
            try {
                int id = mIImsUtEx.setCallForwardingOption(serviceId, commandInterfaceCFAction,
    ...
    public void getCallForwardingOption(int serviceId, int commandInterfaceCFReason, int serviceClass,
        ...
        if (DBG) {
            log("getCallForwardingOption :: mIImsUtEx=" + mIImsUtEx + ", serviceId=" + serviceId
                    + ", commandInterfaceCFReason=" + commandInterfaceCFReason
                    + ", serviceClass=" + serviceClass +", ruleSet=" + ruleSet);
        }

        synchronized(mLockObj) {
            try {
                int id = mIImsUtEx.getCallForwardingOption(serviceId,commandInterfaceCFReason,serviceClass,
                        ruleSet);
