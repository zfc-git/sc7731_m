...
import com.sprd.android.fdn.util.FdnServiceManger.FdnInterface;
import com.sprd.android.fdn.util.FdnServiceUtil;
import com.sprd.android.fdn.util.FdnCommandUtil;
import com.sprd.android.fdn.util.FdnServiceManger;
...
public final class SmsManager {
    ...
    public void setProperty(String key, String value) {
        try {
            ...
            if (iccISms != null) {
                iccISms.setSmsRetryProperty(key, value);
            }
        } catch (Exception ex) {
            // ignore it
        }
    ...
    public String copyMessageToIccEfWithResult(String scAddr, String destAddr, String message, int status, long time) {
        ...
        if (copyMessageToIccEfWithResult(null, data, status)) {
            return "ok";
        } else {
            return "notok";
        }
    ...
    public String copyMessageToIccEfWithResult(String scAddr, String destAddr, String message, int status, long time, int subId) {
        Time time1 = new Time(Time.TIMEZONE_UTC);
        time1.set(time);
        byte[] data = SmsMessage.getReceivedPdu(destAddr, message, status, time1);
        if (data == null) {
        ...
        if (copyMessageToIccEfWithResult(null, data, status, subId)) {
            return "ok";
        } else {
            return "notok";
        }
    ...
    public boolean copyMessageToIccEfWithResult(byte[] smsc, byte[] pdu, int status) {
        Log.d(TAG, "copyMessageToIccEfWithResult");
        return copyMessageToIcc(smsc, pdu, status);
    }

    public boolean copyMessageToIccEfWithResult(byte[] smsc, byte[] pdu, int status, int subId) {
        Log.d(TAG, "copyMessageToIccEfWithResult with subid");
        return copyMessageToIcc(smsc, pdu, status, subId);
    }

    private boolean copyMessageToIcc(byte[] smsc, byte[] pdu, int status, int subId) {
        ...
        try {
            Log.d(TAG, "copyMessageToIcc getISmsService");
            ISms iccISms = getISmsService();
            if (iccISms != null) {
                success = iccISms.copyMessageToIccEfForSubscriber(subId,
                        ActivityThread.currentPackageName(),
                        status, pdu, smsc);
                Log.d(TAG, "copyMessageToIcc subId:[" + subId + "]success:[" + success + "]");
            }
        } catch (RemoteException ex) {
            // ignore it
        }
        ...
        try {
            ...
            if (iccISms != null) {
                success = iccISms.copyMessageToIccEfForSubscriber(getSubscriptionId(),
                        ActivityThread.currentPackageName(),
                        status, pdu, smsc);
                Log.d(TAG, "copyMessageToIcc success:[" + success + "]");
            }
        } catch (RemoteException ex) {
            // ignore it
        }
    ...
    //489223 begin
    //getsmsindex begin
    public ArrayList<SmsMessage> getMessagesFromIccEfByIndex(String index) {
        List<SmsRawData> records = null;
        ...
        try {
            ...
            if (iccISms != null) {
                records = iccISms.getMessagesFromIccEfByIndex(mSubId, index);
            }
        } catch (RemoteException ex) {
            // ignore it
        }
    ...
    public String[] getSmsCopyToSimIndex() {
        String[] smsIndex = null;

        try {
            ISms iccISms = ISms.Stub.asInterface(ServiceManager.getService("isms"));
            //ISms iccISms = ISms.Stub.asInterface(ServiceManager.getService(PhoneFactory.getServiceName("isms",mPhoneId)));
            if (iccISms != null) {
                smsIndex = iccISms.getSmsCopyToSimIndex(mSubId);
            }
        } catch (RemoteException ex) {
            // ignore it
            Log.d("SmsManager", "RemoteException hanppend when call getSmsCopyToSimIndex method!!", ex);
        }
        return smsIndex;
    }
    //getsmsindex end
    ...
    public void sendMultimediaMessage(Context context, Uri contentUri, String locationUrl,
        ...
        Log.i("jerry", "Send an MMS message -stack--->", new Exception());
        if (contentUri == null) {
    ...
    public boolean[] checkFdnContacts(List<String> listContacts) {
        final Context context = ActivityThread.currentApplication()
                .getApplicationContext();
        final long lParam = new Long((long)SubscriptionManager.getPhoneId(getSubscriptionId()));
        boolean[] enable = new boolean[listContacts.size()];
        byte[] bs = new byte[64];
        if(!FdnServiceManger.isPkgInstalled(ActivityThread.currentApplication().getApplicationContext(),
                    FdnServiceManger.FDNSERVICE_PACKAGENAME)){
            Log.i(FdnServiceUtil.TAG, "can't find FdnService.apk");
            for(int i = 0 ; i < listContacts.size() ; i ++){
                 enable[i] = false;// stand for no need to filter , just let it go
            }
            return enable;
        }
        if(mFdnServiceManger == null){
            mFdnServiceManger = new FdnServiceManger(ActivityThread.currentApplication().getApplicationContext());
            mFdnServiceManger.setFdnListner(new FdnInterface() {
                ...
                public boolean getFdnEnable() {
                    return SubscriptionManager.from(ActivityThread.currentApplication().
                         getApplicationContext()).getFdnEnable((int)lParam);
                }
            });
        }
        ...
        for(int i = 0; i < listContacts.size() ; i++){
            ...
            Log.i(FdnServiceUtil.TAG, "mService.process is begin");
            Log.i(FdnServiceUtil.TAG, "destination --->["+listContacts.get(i)+"]");
            int resault = mFdnServiceManger.process(context, FdnCommandUtil.COMPARE_SINGEL_NUMBER , 
                    getSubscriptionId(), tempList, bs);
            if(resault > 0){
               Log.i(FdnServiceUtil.TAG, "mService.process is ok and filtering is well");
               enable[i] = false;
            }else if(resault == 0){
               Log.i(FdnServiceUtil.TAG, "mService.process is no need to filter or fdn disenable.");
               enable[i] = false;
            }else{
               Log.i(FdnServiceUtil.TAG, "mService.process is end, and filtering is no match");
               enable[i] = true;// can' t sending ok , reture failed for sening action
            }
            tempList.remove(0);
        }
        Log.i(FdnServiceUtil.TAG, "checkFdnContacts is successful");
        return enable;
    }
    ...
    public boolean checkUriValid(Uri uri){
        final Context context = ActivityThread.currentApplication()
                .getApplicationContext();
        return SubscriptionManager.from(context).checkUriValid(uri);
    }
}
