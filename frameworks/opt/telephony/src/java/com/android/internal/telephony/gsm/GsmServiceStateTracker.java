...
import com.android.internal.telephony.utils.DefaultTimeZoneUtil;
...
final class GsmServiceStateTracker extends ServiceStateTracker {
    ...
    /* SPRD: for DCT restart radio, bug496617@{*/
    protected boolean mNeedRestartRadio = false;
    /* @} */
    ...
    public GsmServiceStateTracker(GSMPhone phone) {
        ...
        mCi.registerForOemHookRaw(this, EVENT_PHY_CELL_ID, null);
        /* @} */
    }
    ...
    public void handleMessage (Message msg) {
        ...
        switch (msg.what) {
            ...
            case EVENT_RADIO_STATE_CHANGED:
                ...
                // available case
                /* SPRD: add set Primary card feature@{ */
                // setPowerStateToDesired();
                /* @} */
                /*SPRD: for bug 496617 @{*/
                if(DBG) log("EVENT_RADIO_STATE_CHANGED mNeedRestartRadio = " + mNeedRestartRadio);
                if(mNeedRestartRadio){
                    setPowerStateToDesired();
                    mNeedRestartRadio = false;
                }
                /*@}*/
                pollState();
                break;
            ...
            case EVENT_LOCATION_UPDATES_ENABLED:
                ...
                if (ar.exception == null) {
                    /*SPRD: Bug 474224 Add for AGPS feature
                     *@orig
                     *
            ...
            case EVENT_IMS_CAPABILITY_CHANGED:
                ...
                break;
                /* SPRD: add AGPS feature for bug 474224 @{ */
            case EVENT_PHY_CELL_ID:
                log("EVENT_PHY_CELL_ID");
                byte[] ret;
                ar = (AsyncResult) msg.obj;
                if(ar.exception == null){
                    ret = ((byte[])ar.result);

                    int funcId = Integer.parseInt(new String(ret, 0, 1));
                    if(RILConstants.OEM_UNSOL_FUNCTION_ID_SPPCI == funcId) {
                        int pci = Integer.parseInt(new String(ret, 1, ret.length-2));
                        log("EVENT_PHY_CELL_ID  ret.length = " + ret.length + ", pci = " + pci + ", funcId = "+ funcId);

                        mCellLoc.setPci(pci);
                        mNewCellLoc.setPci(pci);
                    }
                }
                break;
                /* @} */
    ...
    protected void setPowerStateToDesired() {
        ...
        } /* SPRD: Move this to the front for bug 494416 @{ */
    ...
    protected void updateSpnDisplay() {
        ...
        if (mSS.getVoiceRegState() == ServiceState.STATE_OUT_OF_SERVICE
            ...
            // SPRD: add by fix bug 520086 to translation the display name
        ...
        /* SPRD: Get spn value from system property instead of IccRecords to support MVNO. @{ */
//        String spn = (iccRecords != null) ? iccRecords.getServiceProviderName() : "";
        String spn = TelephonyManager.getDefault().getSimOperatorNameForPhone(mPhone.getPhoneId());
        // SPRD: add by fix bug 520086 to translation the display name
        ...
        if (ar.exception != null) {
            ...
            switch (what) {
                case EVENT_POLL_STATE_REGISTRATION: {
                    ...
                    /*@{*/
                    if (!(mNewSS.getRilDataRadioTechnology() == ServiceState.RIL_RADIO_TECHNOLOGY_LTE && mNewSS
                ...
                case EVENT_POLL_STATE_GPRS: {
                    ...
                    /* SPRD: add AGPS feature for bug 474224 @{ */
                    if(type == ServiceState.RIL_RADIO_TECHNOLOGY_LTE
                            && dataRegState == ServiceState.STATE_IN_SERVICE) {
                        log("handlePollStateResult: Current type is LTE & data is in service");
                        mNewCellLoc.setLacAndCid(lac, cid);
                    }
                    /* @} */
                    break;
                ...
                case EVENT_POLL_STATE_OPERATOR: {
                    ...
                    if (opNames != null && opNames.length >= 3) {
                        ...
                        if (brandOverride != null) {
                            ...
                            /* SPRD: Modified by fix bug 531801 to get plmn name from numeric_to_operator.xml */
                            mNewSS.setOperatorName(updateOperatorNameByXML(opNames[2],opNames[0]), updateOperatorNameByXML(opNames[2],opNames[1]), opNames[2]);
                        }
                    }
                    break;
                }
    ...
    private void pollStateDone() {
        ...
        if (hasChanged) {
            ...
            if (operatorNumeric == null) {
                ...
                if (!mNitzUpdatedTime && !mcc.equals("000") && !TextUtils.isEmpty(iso) &&
                    ...
                    /** SPRD Modified: Bug#474251 No NITZ and more than one TZ from ISO. @{ **/
                    } else if (uniqueZones.size() > 1){
                        zone = DefaultTimeZoneUtil.getDefaultTimezoneByIso(iso);
                        if (zone != null){
                            setAndBroadcastNetworkSetTimeZone(zone.getID());
                            if (DBG) log("pollStateDone: no nitz but get default time zone by iso-cc=" + iso +
                                    " with zone.getID=" + zone.getID());
                        } else {
                            if (DBG) log("pollStateDone: no nitz and no defualt time zone for iso-cc= " + iso);
                        }
                    /** @} **/
                    } else {
                        if (DBG) {
        ...
        if (hasLocationChanged) {
            mPhone.notifyLocationChanged();
            /* SPRD: Make sure spn and plmn can be translated into current Language after location changed. @{ */
            updatePlmnDisplay();
            updateSpnDisplay();
            /* @} */
        }
    ...
    public CellLocation getCellLocation() {
        if ((mCellLoc.getLac() >= 0) && (mCellLoc.getCid() >= 0)) {
            ...
            /* SPRD: add AGPS feature for bug 474224 @{ */
            if (DBG) log("getCellLocation(): X good mCellLoc.mPci = " + mCellLoc.getPci());
            /* @} */
            return mCellLoc;
        } else {
                ...
                for (CellInfo ci : result) {
                    if (ci instanceof CellInfoGsm) {
                        ...
                        if (cellIdentityLte.getPci() != Integer.MAX_VALUE) {
                            cellLocOther.setPci(cellIdentityLte.getPci());
                        }
                        /* @} */
                    }
                }
    ...
    /* SPRD: add AGPS feature for bug 474224 @{ */
    private static final int EVENT_PHY_CELL_ID = 1002;
    /* @} */

    /* SPRD:add by EXT @{ */
    /**
     * SPRD: Update plmn network name,try to get operator name from sim if ONS exists or regplmn matched OPL/PNN
     * Plmn showed priorities: OPL/PNN > ONS(CPHS) > NITZ > numeric_operator.xml > mcc+mnc
     * See 3GPP TS 22.101 for details.
     */
    private void updatePlmnDisplay() {
        if (mIccRecords != null && mIccRecords instanceof SIMRecords) {
            SIMRecords simRecords = (SIMRecords) mIccRecords;
            String plmn = null;

            if (simRecords.isSimOplPnnSupport()) {
                int lac = mCellLoc != null ? mCellLoc.getLac() : -1;
                plmn = simRecords.getPnn(mSS.getOperatorNumeric(), lac);
            }

            if (TextUtils.isEmpty(plmn)) {
                plmn = simRecords.getSimOns(mSS.getOperatorNumeric());
                if (DBG) {
                    log("Didn't get pnn from sim, try ons next. ONS = " + plmn);
                }
            }

            mSS.setOperatorName(TextUtils.isEmpty(plmn) ? mSS.getOperatorAlphaLong() : plmn,
                    mSS.getOperatorAlphaShort(), mSS.getOperatorNumeric());
        }
    }
    /* @} */

    /*SPRD: for bug 496617 @{*/
    public void setNeedRestartRadio(boolean mNeedRestartRadio){
        this.mNeedRestartRadio = mNeedRestartRadio;
    }
    public boolean getNeedRestartRadio(){
        return mNeedRestartRadio;
    }
    /*@}*/

    /** SPRD: add by fix bug 531801 to get plmn name from numeric_to_operator.xml @{
     * @param mccmnc operator numeric PLMN
     * @param origName the operator name reports by radio
     * @return if there is operator name in XML return it otherwise return original name.
     */
    private String updateOperatorNameByXML(String mccmnc, String origName){
        if(mccmnc.length() < 4){
            return mccmnc;
        }
        ...
        String plmnNumeric = mcc + mncShort;//4601
        String plmnName = TeleUtils.updateOperator(plmnNumeric, "numeric_to_operator");
        if(plmnNumeric.equals(plmnName)){
            plmnName = origName;
        }
        return plmnName;
    }
