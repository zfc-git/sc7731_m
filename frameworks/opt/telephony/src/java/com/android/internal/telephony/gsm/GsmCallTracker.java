...
public final class GsmCallTracker extends CallTracker {
    ...
    GsmCallTracker (GSMPhone phone) {
       ...
       /* SPRD: add for bug479433 @{ */
       // ((RIL)mCi).registerForVideoCallStateChanged(this, EVENT_CALL_STATE_CHANGE, null);
       // mCi.registerForCallCsFallBack(this, EVENT_CALL_CSFALLBACK, null);
       // mCi.registerForCallCsFallBackFinish(this, EVENT_CALL_CSFALLBACK_FINISH, null);
       mCi.registerForOemHookRaw(this, RILConstants.RIL_UNSOL_RESPONSE_VIDEOCALL_STATE_CHANGED, null);
       /* @} */
    }
    ...
    /* SPRD: add for bug479433 @{ */
    public boolean isMoVideoCall() {
        return mMoVideoState != VideoProfile.STATE_AUDIO_ONLY ? true : false;
    }
    /* @} */
    ...
    dial (String dialString, int clirMode, UUSInfo uusInfo, Bundle intentExtras)
        ...
        // SPRD: add for bug479433
        mMoVideoState = intentExtras.getShort(GSMPhone.VIDEO_STATE,
                (short) VideoProfile.STATE_AUDIO_ONLY);
        Rlog.d(LOG_TAG, "====callTrack-dial: videoState = " + mMoVideoState);
        ...
        if ( mPendingMO.getAddress() == null || mPendingMO.getAddress().length() == 0
            ...
            /* SPRD: add for bug479433 @{ */
            if (mMoVideoState != VideoProfile.STATE_AUDIO_ONLY) {
                Rlog.d(LOG_TAG, "====callTrack-dial video");
                mCi.dialVP(mPendingMO.getAddress(), null, 0, obtainCompleteMessage());
            } else {
    ...
    acceptCall () throws CallStateException {
        acceptCall (MultiPartCallHelper.MPC_MODE_HF);
    }
     ...
     */
    void
    acceptCall (int mpcMode) throws CallStateException {
        ...
        if (mRingingCall.getState() == GsmCall.State.INCOMING) {
            ...
            if (mpcMode == MultiPartCallHelper.MPC_MODE_HB) {
                if(!mBackgroundCall.isIdle() && !mForegroundCall.isIdle()) {
                    sendMessageDelayed(obtainMessage(EVENT_CALL_ACCEPT), 1000);
                } else {
                    setMute(false);
                    switchWaitingOrHoldingAndActive();
                }
                ...
                if(!mBackgroundCall.isIdle() && !mForegroundCall.isIdle()) {
                    Rlog.i("phone", "acceptCall: hangup FgCall because BackgroundCall is not null...");
                ...
                Rlog.e("phone", "acceptCall: Invalid Multi-Part-Call Mode...");
            }
        } else {
            ...
            if (conn == null && dc != null) {
                ...
                if (mPendingMO != null && mPendingMO.compareTo(dc)) {
                    ...
                    if (hoConnection != null) {
                        ...
                        /* SPRD: fix bug 513309 @{ */
                        try {
                            if (!dc.isVoice && !mPhone.isVideoEnabled()) {
                                fallBack();
                            } else {
                                newRinging = mConnections[i];
                            }
                        } catch (CallStateException ex) {
                            Rlog.w(LOG_TAG, "GsmCallTracker WARN: unexpected error on fallback "
                                    + ex);
                        }
                        /* @} */
                    } else {
    ...
    handleMessage (Message msg) {
        ...
        switch (msg.what) {
            ...
            case EVENT_GET_LAST_CALL_FAIL_CAUSE:
                ...
                int mCallFailCauseCode = CallFailCause.ERROR_UNSPECIFIED;
                ...
                if (ar.exception != null) {
                    ...
                    // SPRD: Call Fail Cause Feature
                    Rlog.i(LOG_TAG, "vendorCause: " + vendorCause + "causeCode: " + causeCode);
                    if (vendorCause != null) {
                        try {
                            mCallFailCauseCode = Integer.parseInt(vendorCause);
                        } catch (NumberFormatException e) {
                            Rlog.e(LOG_TAG, "NumberFormatException when parse vendorCause.");
                        }
                    }
                }
                ...
                for (int i = 0, s =  mDroppedDuringPoll.size()
                    ...
                    // SPRD: Call Fail Cause Feature
                    conn.onDisconnectSetCallFailCause(mCallFailCauseCode);
                }
            ...
            // SPRD: add for bug479433
            case RILConstants.RIL_UNSOL_RESPONSE_VIDEOCALL_STATE_CHANGED:
                Rlog.i(LOG_TAG,
            ...
            case EVENT_CALL_ACCEPT:{
                try {
                    ...
                    log("error happend when accept call !");
                }
            }
            break;
        }
    ...
    void fallBack() throws CallStateException {
        if (mRingingCall.getState().isRinging()) {
            try {
                RilVideoEx.fallBackVP(mCi, obtainCompleteMessage());
            } catch (IllegalStateException ex) {
                Rlog.w(LOG_TAG, "fallBack failed");
            }
        } else {
            throw new CallStateException("phone not ringing");
        }
