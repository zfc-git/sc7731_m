...
public class IccSmsInterfaceManager {
    ...
    private static final int EVENT_LOAD_SMS_BY_INDEX_DONE = 7; //getsmsindex

    //getsmsindex begin
    private ArrayList<String> mSmsIndexList = new ArrayList<String>();

    public String[] getSmsCopyToSimIndex() {
        return mSmsIndexList.toArray(new String[mSmsIndexList.size()]);
    }

    public static final Uri INSERT_URI = Uri.parse("content://sms/icc");
    ...
    protected Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case EVENT_UPDATE_DONE:
                    ...
                    synchronized (mLock) {
                        ...
                        if (ar.result != null && ar.result instanceof int[] &&
                            ...
                            log("index : " + index);
                            mSmsIndexList.add(String.valueOf(index));
                        }
                        //getsmsindex end
                        //489223 end
                        mLock.notifyAll();
                ...
                //489223 begin
                //getsmsindex begin
                case EVENT_LOAD_SMS_BY_INDEX_DONE:
                    ar = (AsyncResult) msg.obj;
                    synchronized (mLock) {
                        if (ar.exception == null && ar.result != null) {
                            ArrayList<byte[]> records = new ArrayList<byte[]>();
                            records.add((byte[]) ar.result);
                            mSms = buildValidRawData(records);
                        } else {
                            if (DBG) log("Cannot load Sms by index records");
                            if (mSms != null)
                                mSms.clear();
                        }
                        mLock.notifyAll();
    ...
    //getsmsindex begin
    public List<SmsRawData> getMessagesFromIccEfByIndex(String index) {
        if (DBG) log("getMessagesFromEFByIndex");
        ...
        synchronized (mLock) {
            ...
            Message response = mHandler.obtainMessage(EVENT_LOAD_SMS_BY_INDEX_DONE);
            fh.loadEFLinearFixedByIndex(IccConstants.EF_SMS, Integer.parseInt(index), response);
    ...
    class CellBroadcastRangeManager extends IntRangeManager {
        ...
        //clear the IntRange so that the CB can set channels again. bug 489257
    ...
    public void enablePWSMessage(int pri, int sec, int test, int cmas){
        int[] ints = new int[4];
        ints[0] = pri;
        ints[1] = sec;
        ints[2] = test;
        ...
        byte[] data= IccUtils.getIntRequestRawBytes(ints,
                RILConstants.OEM_REQ_FUNCID_ENABLE_BROADCAST_SMS,
                RILConstants.OEM_REQ_NO_SUBFUNCID);
        mPhone.mCi.invokeOemRilRequestRaw(data, null);
    }
    /*@}*/

    //add for customed channel or language set ATCommand bug 489257
