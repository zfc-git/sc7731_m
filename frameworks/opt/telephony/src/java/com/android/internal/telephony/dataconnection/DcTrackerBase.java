...
public abstract class DcTrackerBase extends Handler {
    ...
    protected boolean mIsClearCode = false;
    protected static final String INTENT_RETRY_CLEAR_CODE =
            "com.android.internal.telephony.data-retry-clear-code";
    protected static final String INTENT_RETRY_FROM_FAILURE =
            "com.android.internal.telephony.data-retry-from-failure";
    protected static final String INTENT_RETRY_FROM_FAILURE_ALARM_EXTRA_TYPE =
    ...
    protected static final String INTENT_CLEANUP_DATA_ALARM =
            "com.android.internal.telephony.data-cleanup";
    protected static final int DATA_KEEP_ALIVE_DURATION = 30 * 60 * 1000; // 30 min

    private boolean mIsNetworkShared = false;
    ...
    /* SPRD: Bug 426185 Notify SUSPENDED when other phone is in call @{ */
    // True when other phone in voice call
    protected boolean mOtherPhoneInVoiceCall = false;
    /* @} */
    ...
    protected BroadcastReceiver mIntentReceiver = new BroadcastReceiver ()
    {
        ...
        public void onReceive(Context context, Intent intent)
        {
            ...
            if (action.equals(Intent.ACTION_SCREEN_ON)) {
                mIsScreenOn = true;
                /*
                 * SPRD: for bug 476626 screen on/off when voiceCalling,
                 * restartDataStallAlarm will lead DCT do pdp recovery @{
                 */
                if (!(mInVoiceCall && !mPhone.getServiceStateTracker()
                        .isConcurrentVoiceAndDataAllowed())
                        && !(mOtherPhoneInVoiceCall && !isSupportDSDA())) {
                    stopNetStatPoll();
                    startNetStatPoll();
            ...
            /* @} */
                ...
                /* SPRD: Add to clear code. @{ */
            } else if (mIsClearCode && action.startsWith(INTENT_RETRY_CLEAR_CODE)) {
                if (DBG) log("Retry for clear code");
                onActionIntentRetryClearCode(intent);
            } else if (mIsClearCode && action.startsWith(INTENT_RETRY_FROM_FAILURE)) {
                if (DBG) log("Retry from failure");
            ...
            /* SPRD: Add for always online feature @{ */
                ...
                mIsNetworkShared = tetheredIf != null && !tetheredIf.isEmpty();
    ...
    private final OnSubscriptionsChangedListener mOnSubscriptionsChangedListener =
        ...
        public void onSubscriptionsChanged() {
            ...
            if (mIsLoaded && SubscriptionManager.isValidSubscriptionId(subId)) {
                mIsLoaded = false;
                sendMessage(obtainMessage(DctConstants.EVENT_SUBINFO_READY));
                log("sendMessage EVENT_SUBINFO_READY");
            }
            /* @} */
        }
    };
    ...
    protected void onActionIntentReconnectAlarm(Intent intent) {
        ...
        /* SPRD: for bug 474453 set retry timer to 5s when ps detach @{*/
        SystemProperties.set("persist.radio.apn_delay", "20000");
        /* @} */
    ...
    /* SPRD: Add for always online feature @{ */
    protected void onActionIntentCleanupData() {
        sendMessage(obtainMessage(DctConstants.EVENT_DEEP_SLEEP_CLEANUP_DATA));
    }
    /* @} */
    /**
     * Return true if dual-sim-dual-active
     */
    protected boolean isSupportDSDA() {
        TelephonyManager.MultiSimVariants mscf =
    ...
    protected DcTrackerBase(PhoneBase phone) {
        ...
        int phoneSubId = mPhone.getSubId();
        PowerManager mPm = (PowerManager)mPhone.getContext().getSystemService(Context.POWER_SERVICE);
        mIsScreenOn = mPm.isInteractive();//get screen state when initial for bug 448366
        ...
        // SPRD: Add to clear code.
        mIsClearCode = ClearCodeAndRAUUtils.getInstance().supportSpecialClearCode();
    }
    ...
    public boolean getDataOnRoamingEnabled() {
        ...
        final int phoneSubId = mPhone.getSubId();
        // SPRD: National Data Roaming.
        int dataRoamingSetting = DataRoamType.DISABLE.ordinal();
        ...
        /* SPRD: National Data Roaming. @{ */
        DataRoamType roamType = DataRoamType.fromInt(dataRoamingSetting);
        ...
        switch (roamType) {
            ...
            case ALL:
                ...
                if (mConfig != null && mConfig.isNationalDataRoamingEnabled()) {
                    if (DataRoamType.isNationalRoamType(roamType)) {
    ...
    public boolean isOtherPhoneInVoiceCall() {
        return mOtherPhoneInVoiceCall;
    }
    /* @} */
    ...
    protected abstract void onVoiceCallStarted(int phoneId);
    protected abstract void onVoiceCallEnded(int phoneId);
    /* @} */
    ...
    protected abstract void onBatteryChanged(boolean charging);
    /* @} */
            ...
            if (mUserDataEnabled != enabled) {
                ...
                if (TelephonyManager.getDefault().getSimCount() == 1) {
                    ...
                    Settings.Global.putInt(mResolver, Settings.Global.MOBILE_DATA + phoneSubId,
                            enabled ? 1 : 0);
                    /*
                    * SPRD: Change value of "mobile_data" every time to make it easy to monitor
                    * change of "mobile_data" for each card. If wanted to be notified no matter
                    * which sim's "mobile_data" value changed,only need to register ContentObserver
                    * for "mobile_data" instead of "'mobile_data' + subId".
                    */
                    boolean mobileEnabled = Settings.Global.getInt(mResolver,
                            Settings.Global.MOBILE_DATA, 0) == 1;
                    Settings.Global.putInt(mResolver, Settings.Global.MOBILE_DATA,
                            !mobileEnabled ? 1 : 0);
                    /* @} */
    ...
    protected void startDataStallAlarm(boolean suspectedStall) {
        ...
        if (mDataStallDetectionEnabled && getOverallState() == DctConstants.State.CONNECTED) {
            ...
            if (VDBG_STALL) {
                log(" startDataStallAlarm: mIsScreenOn= " + mIsScreenOn + " suspectedStall= " + suspectedStall);
            }
            /*SPRD: for bug497384,when the screen is off, it is not necessary to start data stall alarm in 60s time gap @{*/
    ...
    /* SPRD: Add for always online feature @{ */
    /** Whether data connection is tethered */
    protected boolean isNetworkShared() {
        return mIsNetworkShared;
    }

    /** Whether the battery is charing */
