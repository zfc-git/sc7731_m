...
public class UiccCard {
    ...
    public void update(Context c, CommandsInterface ci, IccCardStatus ics) {
        synchronized (mLock) {
            ...
            /*
             * SPRD: [bug452051] Don't create instance of UiccCarrierPrivilegeRules if app state
             * response DETECTED which most probably happen when hot swap sim card @{
             */
            boolean isAppStateDetected = false;
            for (int j = 0; j < ics.mApplications.length; j++) {
                if (ics.mApplications[j] != null && ics.mApplications[j].app_state == AppState.APPSTATE_DETECTED) {
                    isAppStateDetected = true;
                    break;
                }
            }
            /*@}*/
            ...
            if (mCarrierPrivilegeRules == null && mCardState == CardState.CARDSTATE_PRESENT && !isAppStateDetected) {
    ...
    public void iccExchangeSimIOUpdate(int fileID, int command, int p1, int p2, int p3, String data,
            String pathID, Message response) {
        mCi.iccIO(command, fileID, pathID, p1, p2, p3, data, null,
                mHandler.obtainMessage(EVENT_SIM_IO_DONE, response));
    }
}
