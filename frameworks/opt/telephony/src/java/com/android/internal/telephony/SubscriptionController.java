...
public class SubscriptionController extends ISub.Stub {
     ...
     */
     private List<SubscriptionInfo> getSubInfo(String selection, Object queryKey) {
        ...
        /*SPRD: Bug 490495 The queried result of SubInfoRecords from databases should be ordered by 'sim_id'.
         *@orig
    ...
    private int getUnusedColor(String callingPackage) {
        ...
        PersistableBundle persistableBundle = configManager.getConfig();
        ...
        if (!isColorfulSignal) {
            ...
            colorArr = mContext.getResources().getIntArray(com.android.internal.R.array.sim_colors_light);
        }
        /*@{*/
        int colorIdx = 0;
        ...
        try {
            ...
            if (subList != null) {
                for (SubscriptionInfo si : subList) {
                    ...
                    //SPRD: modify for bug496681
                    if (si.getIccId().equals(iccId)) {
                        if (DBG)
        ...
        try {
            ...
            try {
                if (cursor == null || !cursor.moveToFirst()) {
                    ...
                    if (nameSource != SubscriptionManager.NAME_SOURCE_USER_INPUT && nameSource != SubscriptionManager.NAME_SOURCE_USER_INPUT_NULL) {
                        logdl("[addSubInfoRecord] nameSource is not user input: nameSource = " + nameSource);
                    /* @} */
                        setDisplayName = true;
                    }
            ...
            if (setDisplayName) {
                ...
                if (!TextUtils.isEmpty(simCarrierName)) {
                    ...
                    nameToSet = TeleUtils.updateOperator(simCarrierName, "spn");
                } else {
    ...
    private int setCarrierText(String text, int subId) {
        ...
        } catch (SQLiteDiskIOException e) {        // SPRD: modify for bug 522069
    ...
    public void setDefaultDataSubId(int subId) {
        ...
        if(TeleUtils.isSuppSetDataAndPrimaryCardBind()){
            RadioTaskManager radioTaskManager = RadioTaskManager.getDefault();
            int phoneId = getSlotId(subId);
            if(radioTaskManager != null && SubscriptionManager.isValidPhoneId(phoneId)){
                radioTaskManager.manualSetPrimaryCard(phoneId);
            }
        }
    }
     ...
     * @hide
     */
    public boolean getFdnEnable(int phoneId) {
          return sProxyPhones[phoneId].getIccCard().getIccFdnEnabled();
    }

    public boolean checkUriValid(Uri uri){
        Cursor cursor = null;
        try {
            cursor = mContext.getContentResolver().query(uri, new String[]{MediaStore.Audio.Media.DATA}, null, null, null);
            if (cursor != null && cursor.getCount() > 0) {
                int column_index = cursor.getColumnIndexOrThrow(MediaStore.Images.Media.DATA);
                if (cursor.moveToFirst()) {
                    File filePath = new File(cursor.getString(column_index));
                    if (filePath.exists()) {
                        slogd("filePath exists! column_index = " + column_index);
                        return true;
                    }
                }
            }
