...
public abstract class IccFileHandler extends Handler implements IccConstants {
    ...
    public void getEFLinearRecordSize(int fileid, String path, Message onLoaded) {
        ...
        /* SPRD: add SIMPhoneBook for bug 474587 @{ */
        logd("getEFLinearRecordSize, fileid:"+Integer.toHexString(fileid)+", ef path:"+getEFPath(fileid));
        /* @} */
        mCi.iccIOForApp(COMMAND_GET_RESPONSE, fileid, efPath,
                    0, 0, GET_RESPONSE_EF_SIZE_BYTES, null, null, mAid, response);
    }
        ...
        try {
            ...
            case EVENT_GET_EF_LINEAR_RECORD_SIZE_DONE:
             ...
             case EVENT_GET_RECORD_SIZE_DONE:
                ...
                if (EF_TYPE_LINEAR_FIXED != data[RESPONSE_DATA_STRUCTURE] && EF_TYPE_CYCLIC != data[RESPONSE_DATA_STRUCTURE]) {
                    throw new IccFileTypeMismatch();
                }
    ...
    protected String getCommonIccEFPath(int efid) {
        ...
        case EF_EXT3:
        case EF_PSI:
        /* SPRD: add LND path for bug 474587 @{ */
        case EF_LND:
        /*@}*/
    ...
    public void loadEFLinearFixed(int fileid, int recordNum, int recordSize, Message onLoaded) {
        Message response
        = obtainMessage(EVENT_READ_EF_LINEAR_RECORD_DONE,
                    fileid, recordNum, onLoaded);

        mCi.iccIOForApp(COMMAND_READ_RECORD, fileid, getEFPath(fileid),
                        recordNum, READ_RECORD_MODE_ABSOLUTE, recordSize, null, null, mAid, response);
    }

    public void updateEFCYCLICLinearFixed(int fileid, int recordNum, byte[] data,
            String pin2, Message onComplete) {
        logd("updateEFCYCLICLinearFixed, fileid:"+Integer.toHexString(fileid)+", ef path:"+getEFPath(fileid)+",recordNum:"+recordNum);
        mCi.iccIO(COMMAND_UPDATE_RECORD, fileid, getEFPath(fileid),
    ...
    //489223 begin
    //getsmsindex begin
    public void loadEFLinearFixedByIndex(int fileid, int recordNum, Message onLoaded) {
        Message response = obtainMessage(EVENT_GET_RECORD_SIZE_DONE,
