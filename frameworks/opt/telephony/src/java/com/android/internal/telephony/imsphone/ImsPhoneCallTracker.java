...
public final class ImsPhoneCallTracker extends CallTracker {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(ImsManager.ACTION_IMS_INCOMING_CALL)) {
                ...
                try {
                    ...
                    boolean isUnknownCall = intent.getBooleanExtra(ImsManager.EXTRA_IMS_UNKNOWN_CALL,false);
                    int unknownCallState = intent.getIntExtra(ImsManager.EXTRA_IMS_UNKNOWN_CALL_STATE, 0);
                    int serviceId = intent.getIntExtra(ImsManager.EXTRA_SERVICE_ID,-1);
                    String unknownCallAddress = intent.getStringExtra(ImsManager.EXTRA_IMS_UNKNOWN_CALL_ADDRESS);

                    if (DBG) {
                        log("onReceive : isUnknownCall :" + isUnknownCall +" state : " + unknownCallState +
                                " ForegroundCall :" + mForegroundCall.getState() +
                                " ForegroundCall :" + mBackgroundCall.getState() +
                                " address = " + unknownCallAddress +
                                " PhoneId = " + mPhone.getPhoneId() +
                                " serviceId = " + serviceId);
                    }
                    ImsPhoneCall.State state = convertUnknownState(unknownCallState);
                    if((serviceId == -1 || serviceId != (mPhone.getPhoneId()+1)) && isUnknownCall){
                        updatePhoneState();
                    ...
                    /* SPRD: Added for VoLTE
                     * @Orig:ImsPhoneConnection conn = new ImsPhoneConnection(mPhone.getContext(), imsCall,
                            ImsPhoneCallTracker.this, mRingingCall);
                     * { */
                    ImsPhoneCall call = null;
                    if (!isUnknownCall) {
                        call = mRingingCall;
                    } else if ((isUnknownCall) && (state == ImsPhoneCall.State.HOLDING)) {
                        call = mBackgroundCall;
                    } else {
                        call = mForegroundCall;
                    }
                    ImsPhoneConnection conn = new ImsPhoneConnection(mPhone.getContext(), imsCall,
                            ImsPhoneCallTracker.this, call, isUnknownCall, state, unknownCallAddress);
                    /* @} */
                    addConnection(conn);
                    ...
                    if (isUnknownCall) {
                        if(mPhone.mDefaultPhone instanceof GSMPhone){
                            GSMPhone gsmPhone = (GSMPhone)mPhone.mDefaultPhone;
                            gsmPhone.notifyUnknownConnection(conn);
                        } else if(mPhone.mDefaultPhone instanceof CDMAPhone){
                            CDMAPhone cdmaPhone = (CDMAPhone)mPhone.mDefaultPhone;
                            cdmaPhone.notifyUnknownConnection(conn);
                        }
                    } else {
                    /* @} */
    ...
    private boolean mSwitchingConferenceCalls = false;//SPRD: add for VoLTE
    ...
    dial(String dialString, int videoState, Bundle intentExtras) throws CallStateException {
        ...
        if(intentExtras != null){
            isConferenceDial= intentExtras.getBoolean(TelephonyIntents.EXTRA_IMS_CONFERENCE_REQUEST,false);
        }
    ...
    dial(String dialString, int clirMode, int videoState, Bundle intentExtras)
        ...
        synchronized (mSyncHold) {
            ...
            mPendingMO.setVideoState(videoState);//SPRD: add for bug535400
        }
    ...
    switchWaitingOrHoldingAndActive() throws CallStateException {
        ...
        if (mForegroundCall.getState() == ImsPhoneCall.State.ACTIVE) {
            ...
            /* SPRD: Added for VoLTE @{ */
            if(mForegroundCall.getConnections().size()>1 || mBackgroundCall.getConnections().size()>1){
                mSwitchingConferenceCalls = true;
                if (DBG) log("switchWaitingOrHoldingAndActive-> is switching conference calls.");
            }
            /* @} */
            ...
            try {
                ...
                if (mCallExpectedToResume == null) {
                    ...
                    mSwitchingConferenceCalls = false;//SPRD:add for VoLTE
                }
            } catch (ImsException e) {
                mForegroundCall.switchWith(mBackgroundCall);
    ...
    private void processCallStateChange(ImsCall imsCall, ImsPhoneCall.State state, int cause,
        ...
        if (DBG) log("processCallStateChange imsCall=" + imsCall.toString() + " conn=" + conn);//SPRD: add for VoLTE
        if (conn == null) {
        ...
        boolean multiPartyChange = conn.isMultiPartyChange(imsCall);//SPRD:Add for bug548426
        if (ignoreState) {
        ...
        changed = changed || multiPartyChange;//SPRD:Add for bug548426
    ...
    private ImsCall.Listener mImsCallListener = new ImsCall.Listener() {
        ...
        public void onCallResumed(ImsCall imsCall) {
            ...
            if (mSwitchingFgAndBgCalls && imsCall != mCallExpectedToResume
                    && !mSwitchingConferenceCalls) { //SPRD:add mSwitchingConferenceCalls for VoLTE
                if (DBG) {
    ...
    handleMessage (Message msg) {
        ...
        switch (msg.what) {
            ...
            case EVENT_RESUME_BACKGROUND:
                /* SPRD: Added for VoLTE @{ */
                if(mForegroundCall.getState() != ImsPhoneCall.State.HOLDING){
                    log("handleMessage EVENT_RESUME_BACKGROUND mForegroundCall state is not HOLDING.");
                    break;
                }
                /* @} */
                try {
    ...
    dialConferenceCall(String dialString, Bundle extras) throws CallStateException {
        ...
        String[] callees = extras.getStringArray(TelephonyIntents.EXTRA_IMS_CONFERENCE_PARTICIPANTS);
        if(callees == null){
        ...
        mPendingMO.setInGroup(true); //SPRD: add for bug531808
        addConnection(mPendingMO);
        ...
        if ((!isPhoneInEcmMode) || (isPhoneInEcmMode && isEmergencyNumber)) {
            ...
            if (mPendingMO.getAddress()== null || mPendingMO.getAddress().length() == 0
                ...
                // Phone number is invalid
                mPendingMO.setDisconnectCause(DisconnectCause.INVALID_NUMBER);
                sendEmptyMessageDelayed(EVENT_HANGUP_PENDINGMO, TIMEOUT_HANGUP_PENDINGMO);
                return null;
            }

            setMute(false);
            int serviceType = PhoneNumberUtils.isEmergencyNumber(mPendingMO.getAddress()) ?
                    ImsCallProfile.SERVICE_TYPE_EMERGENCY : ImsCallProfile.SERVICE_TYPE_NORMAL;
            ...
            try {
                ...
                ImsCall imsCall = mImsManager.makeCall(mServiceId, profile,
                        callees, mImsCallListener);
                mPendingMO.setImsCall(imsCall);

                setVideoCallProvider(mPendingMO, imsCall);
            } catch (ImsException e) {
                loge("dialInternal : " + e);
                mPendingMO.setDisconnectCause(DisconnectCause.ERROR_UNSPECIFIED);
                sendEmptyMessageDelayed(EVENT_HANGUP_PENDINGMO, TIMEOUT_HANGUP_PENDINGMO);
    ...
    private ImsPhoneCall.State convertUnknownState(int state) {
        switch (state) {
            case ImsDriverCall.STATE_ACTIVE:        return ImsPhoneCall.State.ACTIVE;
            case ImsDriverCall.STATE_HOLDING:       return ImsPhoneCall.State.HOLDING;
            case ImsDriverCall.STATE_DIALING:       return ImsPhoneCall.State.DIALING;
            case ImsDriverCall.STATE_ALERTING:      return ImsPhoneCall.State.ALERTING;
            case ImsDriverCall.STATE_INCOMING:      return ImsPhoneCall.State.INCOMING;
            case ImsDriverCall.STATE_WAITING:       return ImsPhoneCall.State.WAITING;
            default:
                log("convertUnknownState: invalid call state:" + state);
