...
public class UiccCardApplication {
    ...
    /** Add for sim lock @{ */
    private static final int SIM_LOCK_BASE = 1000;
    private static final int EVENT_CHANGE_FACILITY_NETWORK_DONE = SIM_LOCK_BASE + 5;
    private static final int EVENT_CHANGE_FACILITY_SIM_DONE =SIM_LOCK_BASE + 6;
    private static final int EVENT_CHANGE_FACILITY_NETWORK_SUBSET_DONE =SIM_LOCK_BASE + 7;
    private static final int EVENT_CHANGE_FACILITY_SERVICE_PROVIDER_DONE =SIM_LOCK_BASE + 8;
    private static final int EVENT_CHANGE_FACILITY_CORPORATE_DONE =SIM_LOCK_BASE + 9;
    private static final int EVENT_CHANGE_FACILITY_NETWORK_PUK_DONE = SIM_LOCK_BASE + 10;
    private static final int EVENT_CHANGE_FACILITY_SIM_PUK_DONE =SIM_LOCK_BASE + 11;
    private static final int EVENT_CHANGE_FACILITY_NETWORK_SUBSET_PUK_DONE =SIM_LOCK_BASE + 12;
    private static final int EVENT_CHANGE_FACILITY_SERVICE_PROVIDER_PUK_DONE =SIM_LOCK_BASE + 13;
    private static final int EVENT_CHANGE_FACILITY_CORPORATE_PUK_DONE =SIM_LOCK_BASE + 14;
    /** @} */
    ...
    private RegistrantList mSimLockedRegistrants = new RegistrantList(); //corporate lock
    private RegistrantList mSimLockedForeverRegistrants = new RegistrantList();
    private RegistrantList mNetworkSubsetLockedPukRegistrants = new RegistrantList(); //network subset  lock puk
    private RegistrantList mServiceProviderLockedPukRegistrants = new RegistrantList(); //service provider lock puk
    private RegistrantList mCorporateLockedPukRegistrants = new RegistrantList(); //corporate lock puk
    private RegistrantList mSimLockedPukRegistrants = new RegistrantList(); //sim lock puk
        ...
        synchronized (mLock) {
            ...
            if (ar.exception == null) {
                ...
                if(mIccFdnEnabled != mDesiredFdnEnabled){
                   int phoneId = mUiccCard.getPhoneId();
                   log("EVENT_CHANGE_FACILITY_FDN_DONE: " + mDesiredFdnEnabled + " phoneId " + phoneId);
                   Intent intent = new Intent("android.intent.action.FDN_STATE_CHANGED" + phoneId);
                   intent.putExtra(PhoneConstants.PHONE_KEY, phoneId);
                   intent.putExtra(IccCardConstants.INTENT_KEY_FDN_STATUS, mDesiredFdnEnabled);
        ...
        public void handleMessage(Message msg){
            ...
            switch (msg.what) {
                case EVENT_PIN1_PUK1_DONE:
                    // SPRD: [Bug528479] update PIN state after supply PUK
                    queryPin1State();
                ...
                case EVENT_CHANGE_FACILITY_CORPORATE_DONE:
                    ar = (AsyncResult)msg.obj;
                    onChangeFacilitySimLock(ar);
                    break;
                case EVENT_CHANGE_FACILITY_SIM_PUK_DONE:
                case EVENT_CHANGE_FACILITY_NETWORK_PUK_DONE:
                case EVENT_CHANGE_FACILITY_NETWORK_SUBSET_PUK_DONE:
                case EVENT_CHANGE_FACILITY_SERVICE_PROVIDER_PUK_DONE:
                case EVENT_CHANGE_FACILITY_CORPORATE_PUK_DONE:
                    ar = (AsyncResult)msg.obj;
                    onChangeFacilitySimLockPuk(ar);
                    break;
                /* @} */
                default:
    ...
    private void notifySimLockedRegistrantsIfNeeded(Registrant r) {
        ...
        if (mAppState == AppState.APPSTATE_SUBSCRIPTION_PERSO) {
            if (r == null) {
                if (mPersoSubState == PersoSubState.PERSOSUBSTATE_SIM_SIM){
                    log("Notifying registrants: simLocked");
                    mSimLockedRegistrants.notifyRegistrants();
                } else if (mPersoSubState == PersoSubState.PERSOSUBSTATE_SIM_NETWORK_SUBSET) {
                    log("Notifying registrants: NetworkSubsetLocked");
                    mNetworkSubsetLockedRegistrants.notifyRegistrants();
                } else if (mPersoSubState == PersoSubState.PERSOSUBSTATE_SIM_SERVICE_PROVIDER) {
                    log("Notifying registrants: ServiceProviderLocked");
                    mServiceProviderLockedRegistrants.notifyRegistrants();
                } else if (mPersoSubState == PersoSubState.PERSOSUBSTATE_SIM_CORPORATE) {
                    log("Notifying registrants: corporateLocked");
                    mCorporateLockedRegistrants.notifyRegistrants();
                }  else if(mPersoSubState == PersoSubState.PERSOSUBSTATE_SIM_LOCK_FOREVER) {
                    log("Notifying registrants: simlock forever");
                    mSimLockedForeverRegistrants.notifyRegistrants();
                }  else if(mPersoSubState == PersoSubState.PERSOSUBSTATE_SIM_NETWORK_PUK) {
                    log("Notifying registrants: NetworkLocked puk");
                    mNetworkLockedPukRegistrants.notifyRegistrants();
                }  else if(mPersoSubState == PersoSubState.PERSOSUBSTATE_SIM_NETWORK_SUBSET_PUK) {
                    log("Notifying registrants: NetworkSubsetLocked puk");
                    mNetworkSubsetLockedPukRegistrants.notifyRegistrants();
                }  else if(mPersoSubState == PersoSubState.PERSOSUBSTATE_SIM_CORPORATE_PUK) {
                    log("Notifying registrants: corporateLocked puk");
                    mCorporateLockedPukRegistrants.notifyRegistrants();
                }  else if(mPersoSubState == PersoSubState.PERSOSUBSTATE_SIM_SERVICE_PROVIDER_PUK) {
                    log("Notifying registrants: ServiceProviderLocked puk");
                    mServiceProviderLockedPukRegistrants.notifyRegistrants();
                }  else if(mPersoSubState == PersoSubState.PERSOSUBSTATE_SIM_SIM_PUK) {
                    log("Notifying registrants: simLocked puk");
                    mSimLockedPukRegistrants.notifyRegistrants();
                }
            } else {
                if (DBG)
                    log("Notifying 1 registrant: mPersoSubState :"+mPersoSubState);
                r.notifyRegistrant(new AsyncResult(null, null, null));
            }
        }
    }
 ...
 // network subset lock
    public void registerForNetworkSubsetLocked(Handler h, int what, Object obj) {
        synchronized (mLock) {
            ...
            if (mPersoSubState == PersoSubState.PERSOSUBSTATE_SIM_NETWORK_SUBSET) {
                notifySimLockedRegistrantsIfNeeded(r);
            }
        }
    }
    ...
    public void registerForCorporateLocked(Handler h, int what, Object obj) {
        synchronized (mLock) {
            ...
            if (mPersoSubState == PersoSubState.PERSOSUBSTATE_SIM_CORPORATE) {
                notifySimLockedRegistrantsIfNeeded(r);
            }
        }
    }
    ...
    public void registerForServiceProviderLocked(Handler h, int what, Object obj) {
        synchronized (mLock) {
            ...
            if (mPersoSubState == PersoSubState.PERSOSUBSTATE_SIM_SERVICE_PROVIDER) {
                notifySimLockedRegistrantsIfNeeded(r);
            }
        }
    }
    ...
    public void registerForSimLockedForever(Handler h, int what, Object obj) {
        synchronized (mLock) {
            ...
            if (mPersoSubState == PersoSubState.PERSOSUBSTATE_SIM_LOCK_FOREVER) {
                notifySimLockedRegistrantsIfNeeded(r);
            }
        }
    }
    ...
    public void registerForNetworkLockedPuk(Handler h, int what, Object obj) {
        synchronized (mLock) {
            ...
            if (mPersoSubState == PersoSubState.PERSOSUBSTATE_SIM_NETWORK_PUK) {
                notifySimLockedRegistrantsIfNeeded(r);
            }
        }
    }
    ...
    public void registerForSimLockedPuk(Handler h, int what, Object obj) {
        synchronized (mLock) {
            ...
            if (mPersoSubState == PersoSubState.PERSOSUBSTATE_SIM_SIM_PUK) {
                notifySimLockedRegistrantsIfNeeded(r);
            }
        }
    }
    ...
    public void registerForNetworkSubsetLockedPuk(Handler h, int what, Object obj) {
        synchronized (mLock) {
            ...
            if (mPersoSubState == PersoSubState.PERSOSUBSTATE_SIM_NETWORK_SUBSET_PUK) {
                notifySimLockedRegistrantsIfNeeded(r);
            }
        }
    }
    ...
    public void registerForCorporateLockedPuk(Handler h, int what, Object obj) {
        synchronized (mLock) {
            ...
            if (mPersoSubState == PersoSubState.PERSOSUBSTATE_SIM_CORPORATE_PUK) {
                notifySimLockedRegistrantsIfNeeded(r);
            }
        }
    }
    ...
    public void registerForServiceProviderLockedPuk(Handler h, int what, Object obj) {
        synchronized (mLock) {
            ...
            if (mPersoSubState == PersoSubState.PERSOSUBSTATE_SIM_SERVICE_PROVIDER_PUK) {
                notifySimLockedRegistrantsIfNeeded(r);
            }
        }
    }
    ...
    public void setSimLockEnabled(boolean enabled,
        ...
        synchronized (mLock) {
            int serviceClassX = 0;
            mCi.setFacilityLock(CommandsInterface.CB_FACILITY_BA_PS,
    ...
    public void setNetworkLockEnabled(boolean enabled,
        ...
        synchronized (mLock) {
            int serviceClassX = 0;
            mCi.setFacilityLock(CommandsInterface.CB_FACILITY_BA_PN,
                    enabled, password, serviceClassX,
    ...
    public void setNetworkSubsetLockEnabled (boolean enabled,
        ...
        synchronized (mLock) {
            int serviceClassX = 0;
            mCi.setFacilityLock(CommandsInterface.CB_FACILITY_BA_PU,
                    enabled, password, serviceClassX,
    ...
    public void setServiceProviderLockEnabled(boolean enabled,
        ...
        synchronized (mLock) {
            int serviceClassX = 0;
            mCi.setFacilityLock(CommandsInterface.CB_FACILITY_BA_PP,
                    enabled, password, serviceClassX,
    ...
    public void setCorporateLockEnabled(boolean enabled,
        ...
        synchronized (mLock) {
            int serviceClassX = 0;
            mCi.setFacilityLock(CommandsInterface.CB_FACILITY_BA_PC,
                    enabled, password, serviceClassX,
                    mHandler.obtainMessage(EVENT_CHANGE_FACILITY_CORPORATE_DONE, onComplete));
        }
    }

    /** REMOVE when mSimLockEnabled is not needed */
    private void onChangeFacilitySimLock(AsyncResult ar) {
        synchronized (mLock) {
            int attemptsRemaining = -1;

            if (ar.exception != null) {
                attemptsRemaining = parsePinPukErrorResultEx(ar);
                loge("Error change facility lock with exception " + ar.exception);
            }
            Message response = (Message)ar.userObj;
            AsyncResult.forMessage(response).exception = ar.exception;
    ...
    public void setNetworkLockPukEnabled(boolean enabled, String password, Message onComplete) {
        synchronized (mLock) {
            int serviceClassX = 0;
            mCi.setFacilityLock(CommandsInterface.CB_FACILITY_BA_PN_PUK,
                    enabled, password, serviceClassX,
                    mHandler.obtainMessage(EVENT_CHANGE_FACILITY_NETWORK_PUK_DONE, onComplete));
        }
    }
    ...
    public void setNetworkSubsetLockPukEnabled(boolean enabled, String password, Message onComplete) {
        synchronized (mLock) {
            int serviceClassX = 0;
            mCi.setFacilityLock(CommandsInterface.CB_FACILITY_BA_PU_PUK,
                    enabled, password, serviceClassX,
                    mHandler.obtainMessage(EVENT_CHANGE_FACILITY_NETWORK_SUBSET_PUK_DONE, onComplete));
        }
    }
    ...
    public void setServiceProviderLockPukEnabled(boolean enabled, String password, Message onComplete) {
        synchronized (mLock) {
            int serviceClassX = 0;
            mCi.setFacilityLock(CommandsInterface.CB_FACILITY_BA_PP_PUK,
                    enabled, password, serviceClassX,
                    mHandler.obtainMessage(EVENT_CHANGE_FACILITY_SERVICE_PROVIDER_PUK_DONE, onComplete));
        }
    }
    ...
    public void setCorporateLockPukEnabled(boolean enabled, String password, Message onComplete) {
        synchronized (mLock) {
            int serviceClassX = 0;
            mCi.setFacilityLock(CommandsInterface.CB_FACILITY_BA_PC_PUK,
                    enabled, password, serviceClassX,
                    mHandler.obtainMessage(EVENT_CHANGE_FACILITY_CORPORATE_PUK_DONE, onComplete));
        }
    }
    ...
    public void setSimLockPukEnabled(boolean enabled, String password, Message onComplete) {
        synchronized (mLock) {
            int serviceClassX = 0;
            mCi.setFacilityLock(CommandsInterface.CB_FACILITY_BA_PS_PUK,
                    enabled, password, serviceClassX,
                    mHandler.obtainMessage(EVENT_CHANGE_FACILITY_SIM_PUK_DONE, onComplete));
        }
    }

    private void onChangeFacilitySimLockPuk(AsyncResult ar) {
        synchronized (mLock) {
            int attemptsRemaining = -1;
