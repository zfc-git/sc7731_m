...
public class IccUtils {
    ...
    adnStringFieldToString(byte[] data, int offset, int length) {
        ...
        /*@{*/
        String str = getGsm8BitString(data, offset, length,
                defaultCharset.trim());
        return str != null ? str : getGsm7BitString(data, offset, length);
        /*@}*/
    }
    ...
    private static String getGsm8BitString(byte[] data, int offset, int length,
        ...
        try {
            ...
            gsm8Bit = null;
            Rlog.e(LOG_TAG, "getGsm8BitString/exception is happen here!", e);
        }
        return gsm8Bit;
    }
    ...
    private static String getGsm7BitString(byte[] data, int offset, int length) {
        ...
        try {
            ...
            gsm7Bit = null;
            Rlog.e(LOG_TAG, "getGsm7BitString/exception is happen here!", e);
        }
        return gsm7Bit;
   }
   /* @} */
    ...
    public static byte[] intsToByteArray(int[] value) {
        ...
        byte[] bytes = new byte[4];
        for (int i = 0; i < len; i++) {
            bytes = intToByteArray(value[i]);
            System.arraycopy(bytes, 0, ret, i * 4, bytes.length);
        }
        return ret;
    }

    public static byte[] getIntRequestRawBytes(int[] value, byte funcId,
            byte subFuncId) {
        byte[] rawBytes = new byte[4 * value.length + 8];
        rawBytes[0] = funcId;
        rawBytes[1] = subFuncId;
        byte[] len = intToByteArray(value.length);
        System.arraycopy(len, 0, rawBytes, 4, len.length);
        byte[] data = intsToByteArray(value);
        System.arraycopy(data, 0, rawBytes, 8, data.length);
        return rawBytes;
    }
