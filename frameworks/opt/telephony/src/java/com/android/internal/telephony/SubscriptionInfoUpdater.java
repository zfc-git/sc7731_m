        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (Intent.ACTION_LOCALE_CHANGED.equals(action)) {
                ...
                for (int i = 0; i < subIds.length; i++) {
                    updateSubInfoDisplayNames(subIds[i]);
                }
                return;
            }
            /* @} */
            ...
            case EVENT_SIM_LOCKED_QUERY_ICCID_DONE: {
                ...
                if (ar.exception == null) {
                    if (ar.result != null) {
                        ...
                        mIccId[slotId] = IccUtils.bcdToStringForIccId(data, 0, data.length);
                        /* @} */
                    } else {
                        logd("Null ar");
              ...
              /* SPRD: add for Bug 494095, if IccId is empty, set it to -1@{ */
...
//                }
            ...
            if (subInfo != null && subInfo.getNameSource() !=
                    SubscriptionManager.NAME_SOURCE_USER_INPUT && subInfo.getNameSource() !=
                    SubscriptionManager.NAME_SOURCE_USER_INPUT_NULL) {
            /* @} */
                if (!TextUtils.isEmpty(simCarrierName)) {
                    nameToSet = simCarrierName;
                    /* SPRD: add by fix bug 520086 to translation the display name @{ */
                    nameToSet = TeleUtils.updateOperator(nameToSet, "spn");
                    logd("nameToSet Translate :" + nameToSet);
                    /* @} */
    ...
    synchronized private void updateSubscriptionInfoByIccId() {
        ...
        for (int i=0; i < nSubCount; i++) {
            ...
            /* SPRD: modify for bug492214 @{ */
            if (temp != null && temp.getNameSource() ==
                    SubscriptionManager.NAME_SOURCE_USER_INPUT_NULL) {
                if (temp.getDisplayName().toString().contains("SIM")) {
                    String displayname = "SIM" + (temp.getSimSlotIndex() + 1);
                    logd("sim name = " + displayname + "upload the displayname.");
                    SubscriptionController.getInstance().setDisplayNameUsingSrc(displayname, temp.getSubscriptionId(), SubscriptionManager.NAME_SOURCE_USER_INPUT_NULL);
                }
            }
            /* @} */
    ...
    /* SPRD: add by fix bug 520086 to translation the display name @{ */
    private void updateSubInfoDisplayNames(int subId) {
        logd("updateSubInfoDisplayNames  subid :" +subId);
        ContentResolver contentResolver = mContext.getContentResolver();
        TelephonyManager tm = TelephonyManager.getDefault();
        SubscriptionInfo subInfo = mSubscriptionManager.getActiveSubscriptionInfo(subId);
        String nameToSet;
        String simCarrierName = tm.getSimOperatorNameForSubscription(subId);
        logd("simCarrierName = " + simCarrierName);
        ContentValues name = new ContentValues(1);
