...
public class DcSwitchStateMachine extends StateMachine {
    ...
    // SPRD: Bug 497947 Move DcSwitchStateMachine to IdleState when radio off
    private static final int EVENT_RADIO_OFF       = BASE + 4;
...
//    }
        ...
        public boolean processMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case DcSwitchAsyncChannel.REQ_DISCONNECT_ALL: {
                    if (DBG) log("IdleState: REQ_DISCONNECT_ALL" );
    ...
    private class AttachingState extends State {
        ...
        public void enter() {
            ...
            /* SPRD: Bug 497947 Move DcSwitchStateMachine to IdleState when radio off @{ */
            final PhoneBase pb = (PhoneBase)((PhoneProxy)mPhone).getActivePhone();
            pb.mCi.registerForOffOrNotAvailable(getHandler(), EVENT_RADIO_OFF, null);
            /* @} */
            doEnter();
        }
        ...
        public void exit() {
            final PhoneBase pb = (PhoneBase)((PhoneProxy)mPhone).getActivePhone();
            pb.mCi.unregisterForOffOrNotAvailable(getHandler());
        }
        /* @} */
            ...
            switch (msg.what) {
                ...
                case EVENT_DATA_ALLOWED: {
                    ...
                    if (mCurrentAllowedSequence != msg.arg1) {
                        ...
                        final PhoneBase pb = (PhoneBase)((PhoneProxy)mPhone).getActivePhone();
                        if (pb.mDcTracker.getAutoAttachOnCreation()) {
                            if (DBG) log("AttachingState releasingAll due to autoAttach");
                            DctController.getInstance().releaseAllRequests(mId);
                        }
                        /* @} */
                ...
                case EVENT_RADIO_OFF: {
                    if (DBG) {
                        log("AttachingState: EVENT_RADIO_OFF");
                    }
                    // If radio is off, we will never receive EVENT_DATA_ATTACHED, we should move
                    // to IdleState so that we won't block other phnones
                    final PhoneBase pb = (PhoneBase)((PhoneProxy)mPhone).getActivePhone();
                    if (pb.mDcTracker.getAutoAttachOnCreation()) {
                        if (DBG) log("AttachingState releasingAll due to autoAttach");
                        DctController.getInstance().releaseAllRequests(mId);
    ...
    private class AttachedState extends State {
        ...
        public void enter() {
            ...
            /* SPRD: modify for bug495063 @{ */
            if(!getHandler().hasMessages(DcSwitchAsyncChannel.REQ_DISCONNECT_ALL)) {
                DctController.getInstance().executeAllRequests(mId);
            }
            /* @} */
        }
        ...
        public boolean processMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case DcSwitchAsyncChannel.EVENT_DATA_DETACHED: {
                     ...
                     */
                    DctController.getInstance().notifySwitchDataCardComplete();
                    /* @} */
                    break;
                }
