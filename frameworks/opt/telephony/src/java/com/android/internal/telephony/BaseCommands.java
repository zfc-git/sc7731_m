...
public abstract class BaseCommands implements CommandsInterface {
    ...
    protected RegistrantList mOemHookRawRegistrants = new RegistrantList();
    /* @} */
    /* SPRD: add for DATA CLEAR CODE @{ */
    protected Registrant mRauSucessRegistrant;
    protected Registrant mClearCodeFallBackRegistrant;
    /* @} */
     ...
     * PWS is Public Warning System, consisted of ETWS and CMAS. reference 3GPP TS23.041 and TS23.401
     * @{
     */
    protected Registrant mPWSMessageRegistrant;
    public void setOnPWSMessage(Handler h, int what, Object obj) {
        mPWSMessageRegistrant = new Registrant(h, what, obj);
    }

    public void unSetOnPWSMessage(Handler h) {
        mPWSMessageRegistrant.clear();
    }

    protected Object responsePWSMessage(Parcel p) {
        PWSMessage pwsMsg = new PWSMessage();
        pwsMsg.segmentId = p.readInt();
        pwsMsg.totalSegments = p.readInt();
        pwsMsg.serialNumber = p.readInt();
        pwsMsg.messageIdentifier = p.readInt();
        pwsMsg.dcs = p.readInt();
        pwsMsg.length = p.readInt();
        pwsMsg.data = p.readString();
        return pwsMsg;
    }
    public static class PWSMessage {
