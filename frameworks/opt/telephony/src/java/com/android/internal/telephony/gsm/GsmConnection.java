...
public class GsmConnection extends Connection {
    ...
    int mCallFailCause = CallFailCause.ERROR_UNSPECIFIED;
    ...
    GsmConnection (Context context, DriverCall dc, GsmCallTracker ct, int index) {
        /* SPRD: fix bug 493710,499749 @{ */
        mIsVideo = !dc.isVoice && ct.mPhone.isVideoEnabled();
        mIsMultiParty = dc.isMpty;
        mContext = context;
        /* @} */
    ...
    GsmConnection (Context context, String dialString, GsmCallTracker ct, GsmCall parent) {
        /* SPRD: add for bug493710,499749 @{ */
        mIsVideo = ct.isMoVideoCall() && ct.mPhone.isVideoEnabled();
        mContext = context;
        /* @} */
        createWakeLock(context);
        acquireWakeLock();
        ...
        switch (causeCode) {
            ...
            default:
                ...
                if (serviceState == ServiceState.STATE_POWER_OFF) {
                    ...
                    /* SPRD: return NORMAL when mAddress is emergency number @{
                    * @orig
                    * return DisconnectCause.OUT_OF_SERVICE; */
                    if (PhoneNumberUtils.isEmergencyNumber(mAddress)){
    ...
    update (DriverCall dc) {
        ...
        /* SPRD: add for bug479433 @{ */
        if(mContext != null && ImsManager.isVolteEnabledByPlatform(mContext)){
            mIsMultiParty = dc.isMpty;
            boolean videoStateChange = (mIsVideo != !dc.isVoice);
            if(videoStateChange){
                changed = true;
                mIsVideo = !dc.isVoice;
                if(mIsVideo){
    ...
    onConnectedInOrOut() {
        ...
        /* SPRD: Add for 542365, can't releaseWakeLock when call is video @{ */
        if (!mIsVideo) {
            releaseWakeLock();
        }
        /* @} */
    ...
    createWakeLock(Context context) {
        ...
        /* SPRD: add for bug493710 @{ */
        mPartialWakeLock = pm.newWakeLock(mIsVideo ? PowerManager.FULL_WAKE_LOCK
     ...
     */
    @Override
    public Integer getCallFailCause() {
        if (Phone.DEBUG_PHONE) {
            log("getCallFailCause: mCallFailCause is " + mCallFailCause);
        }
        return mCallFailCause;
    }

    void onDisconnectSetCallFailCause(int callfailcauseCode) {
        if (Phone.DEBUG_PHONE) {
            log("onDisconnectSetCallFailCause: callfailcauseCode is " + callfailcauseCode);
        }
        mCallFailCause = callfailcauseCode;
    }
    /* @} */
}
