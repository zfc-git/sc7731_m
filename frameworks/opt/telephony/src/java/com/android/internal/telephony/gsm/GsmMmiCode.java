...
public final class GsmMmiCode extends Handler implements MmiCode {
    ...
    // SPRD: Porting USSD. Connected line identification presentation
    static final String SC_COLP    = "76";
    // SPRD: Porting USSD. Connected line identification restriction
    ...
    static final int EVENT_QUERY_LI_COMPLETE    = 8;         // SPRD: Porting USSD.
...
*/
            ...
            /* SPRD: Porting USSD.@{ */
                ...
                if (isInterrogate()) {
                    byte[] data = IccUtils.getRequestRawBytes(
                            RILConstants.OEM_REQ_FUNCID_QUERY_COLP_COLR,
                            RILConstants.OEM_REQ_SUBFUNCID_QUERY_COLP);
                ...
                if (isInterrogate()) {
                    byte[] data = IccUtils.getRequestRawBytes(
                            RILConstants.OEM_REQ_FUNCID_QUERY_COLP_COLR,
                            RILConstants.OEM_REQ_SUBFUNCID_QUERY_COLR);
                    ((CommandsInterface)(mPhone.mCi)).invokeOemRilRequestRaw(
                            data, obtainMessage(EVENT_QUERY_LI_COMPLETE, this));
                } else {
                    throw new RuntimeException ("Invalid or Unsupported MMI Code");
                ...
                if (isActivate() || isRegister()) {
                    ...
                    byte[] data = IccUtils.getStringRequestRawBytes(mPoundString,
                            RILConstants.OEM_REQ_FUNCID_MMI_ENTER_SIM,
                ...
                if (isRegister()) {
                    if (!newPinOrPuk.equals(mSic)) {
                        ...
                        if (mSc.equals(SC_PIN)) {
                            /** SPRD: Porting USSD.
                             *@orig mUiccApplication.changeIccLockPassword(oldPinOrPuk, newPinOrPuk,
                                      obtainMessage(EVENT_SET_COMPLETE, this));
                             */
                            /* SPRD: modify for bug494346 @{ */
                ...
                if (err == CommandException.Error.PASSWORD_INCORRECT) {
                    if (isPinPukCommand()) {
                        ...
                        if (mSc.equals(SC_PUK)) {
                            sb.append(mContext.getText(
                                    com.android.internal.R.string.badPuk));
                            type = TelephonyManager.UNLOCK_PUK;
                        } else if (mSc.equals(SC_PUK2)){
                            sb.append(mContext.getText(
                                    com.android.internal.R.string.badPuk2));
                            type = TelephonyManager.UNLOCK_PUK2;
                        } else if (mSc.equals(SC_PIN)){
                            sb.append(mContext.getText(
                                    com.android.internal.R.string.badPin));
                            type = TelephonyManager.UNLOCK_PIN;
                        } else if (mSc.equals(SC_PIN2)){
                            sb.append(mContext.getText(
                                    com.android.internal.R.string.badPin2));
                            type = TelephonyManager.UNLOCK_PIN2;
                        }
                         ...
                         */
                        int attemptsRemaining = mPhone.getRemainTimes(type);
                        if (attemptsRemaining <= 0) {
                            Rlog.d(LOG_TAG, "onSetComplete: PUK locked,"
                                    + " cancel as lock screen will handle this");
                            mState = State.CANCELLED;
                            /** SPRD: Porting USSD. @{ */
                            if (attemptsRemaining == 0 && type == TelephonyManager.UNLOCK_PIN2) {
                                mState = State.FAILED;
                    ...
                    sb.append(mContext.getText(
                            com.android.internal.R.string.badPin2/* SPRD: @orig badPin*/));
                    sb.append("\n");
                    sb.append(mContext.getText(
                            com.android.internal.R.string.needPuk2));
    ...
    /** SPRD: Porting USSD. Dismiss ussdinputdialog if network timeout. @{ */
    static GsmMmiCode
    ...
    onQueryLiComplete(AsyncResult ar) {
        ...
        if (ar.exception != null) {
            ...
            try {
                if (bytes != null) {
                    int ret1 = IccUtils.byteArrayToInt(bytes,0);
                    if (ret1 == 0) {
                        sb.append(mContext.getText(com.android.internal.R.string.serviceDisabled));
                    } else if (ret1 == 1) {
                        sb.append(mContext.getText(com.android.internal.R.string.serviceEnabled));
