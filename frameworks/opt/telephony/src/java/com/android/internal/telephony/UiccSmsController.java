...
public class UiccSmsController extends ISms.Stub {
    ...
    public boolean copyMessageToIccEfForSubscriber(int subId, String callingPackage, int status,
        ...
        IccSmsInterfaceManager iccSmsIntMgr = getIccSmsInterfaceManager(subId);
        //489223 begin
        Rlog.d(LOG_TAG, "copyMessageToIccEfForSubscriber subId:[" + subId + "]");
        String result = null;
        if (iccSmsIntMgr != null) {
    ...
    @Override
    public void setSmsRetryProperty(String key, String value) {
        SystemProperties.set(key, String.valueOf(value));
    }

    // bug 501030 Add for SPWRN feature
    public boolean enablePWSMessageForSubscriber(int subId, int pri, int sec,
            int test, int cmas) {
        IccSmsInterfaceManager iccSmsIntMgr = getIccSmsInterfaceManager(subId);
        if (iccSmsIntMgr != null) {
            Rlog.d(LOG_TAG, "Add for SPWRN.");
            iccSmsIntMgr.enablePWSMessage(pri, sec, test, cmas);
            return true;
        } else {
            Rlog.e(LOG_TAG, "enablePWSMessage iccSmsIntMgr is null for"
                    + " Subscription: " + subId);
        }
        return false;
    }
    ...
    public boolean setCellBroadcastSmsConfig(IccSmsInterfaceManager iccSmsIntMgr, long subId, int[] data) {
        if (iccSmsIntMgr != null) {
            Rlog.d(LOG_TAG, "Add for Disabled channel.");
            iccSmsIntMgr.setCellBroadcastSmsConfig(data);
            return true;
        } else {
            Rlog.e(LOG_TAG, "Disabled channel iccSmsIntMgr is null for"
                    + " Subscription: " + subId);
        }
        return false;
    }
    ...
    //489223 begin
    //getsmsindex begin
    public String[] getSmsCopyToSimIndex(int subId) throws android.os.RemoteException {
        IccSmsInterfaceManager iccSmsIntMgr = getIccSmsInterfaceManager(subId);
        return iccSmsIntMgr.getSmsCopyToSimIndex();
        //return mIccSmsInterfaceManager[getSimplePolicyPhoneId()].getSmsCopyToSimIndex();
    }

    public List<SmsRawData> getMessagesFromIccEfByIndex(int subId, String index) throws android.os.RemoteException {
        IccSmsInterfaceManager iccSmsIntMgr = getIccSmsInterfaceManager(subId);
        return iccSmsIntMgr.getMessagesFromIccEfByIndex(index);
        //return mIccSmsInterfaceManager[getSimplePolicyPhoneId()].getMessagesFromIccEfByIndex(index);
    }
    //getsmsindex end
    //489223 end
}
