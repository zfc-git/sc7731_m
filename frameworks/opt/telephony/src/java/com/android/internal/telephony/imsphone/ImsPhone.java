...
public class ImsPhone extends ImsPhoneBase {
    ...
    ** all these EVENTs should be defined in a unique place -- PhoneBase
    ** ORIGIN
    ...
    **/
    ...
    public void setCallForwardingOption(int commandInterfaceCFAction,
        ...
        if ((isValidCommandInterfaceCFAction(commandInterfaceCFAction)) &&
            ...
            resp = obtainMessage(EVENT_SET_CALL_FORWARD_DONE,
                    isCfEnable(commandInterfaceCFAction) ? 1 : 0, serviceClass, cf);
    ...
    private CallForwardInfo getCallForwardInfo(ImsCallForwardInfo info) {
        ...
        /* SPRD: add for VoLTE
         * @Orig:cfInfo.serviceClass = SERVICE_CLASS_VOICE;
         * { */
    ...
    public void handleMessage (Message msg) {
        ...
        switch (msg.what) {
            case EVENT_SET_CALL_FORWARD_DONE:
                IccRecords r = getIccRecords();
                Cf cf = (Cf) ar.userObj;
                //SPRD: add (msg.arg2 & SERVICE_CLASS_VOICE) != 0 for bug537023
                if (cf.mIsCfu && ar.exception == null && r != null && (msg.arg2 & SERVICE_CLASS_VOICE) != 0) {
            ...
            case EVENT_GET_CALL_FORWARD_DONE:
                ...
                break;
                /* SPRD: add for VoLTE. @{ */
            case EVENT_GET_IMS_CALL_FORWARD_DONE:
                ImsCallForwardInfo[] imsCfInfos = null;
                if (ar.exception == null) {
                    imsCfInfos = handleImsCfQueryResult((ImsCallForwardInfo[])ar.result);
                }
                sendResponse((Message) ar.userObj, imsCfInfos, ar.exception);
                break;
                /* }@ */
    ...
    public void setCallForwardingOption(int commandInterfaceCFAction,
        ...
        if ((isValidCommandInterfaceCFAction(commandInterfaceCFAction)) &&
            ...
            try {
                ImsUt ut = (ImsUt)mCT.getUtInterface();
                ut.setCallForwardingOption(getPhoneId()+1, commandInterfaceCFAction, commandInterfaceCFReason,
                        serviceClass, dialingNumber, timerSeconds, ruleSet, resp);
            } catch (ImsException e) {
                sendErrorResponse(onComplete, e);
            }
        } else if (onComplete != null) {
    ...
    public void getCallForwardingOption(int commandInterfaceCFReason, int serviceClass,
        ...
        if (isValidCommandInterfaceCFReason(commandInterfaceCFReason)) {
            if (DBG) Rlog.d(LOG_TAG, "requesting call forwarding query.");
            Message resp;
            resp = obtainMessage(EVENT_GET_IMS_CALL_FORWARD_DONE, onComplete);

            try {
                ImsUt ut = (ImsUt)mCT.getUtInterface();
                ut.getCallForwardingOption(getPhoneId()+1, commandInterfaceCFReason, serviceClass,
                        ruleSet, resp);
            } catch (ImsException e) {
                sendErrorResponse(onComplete, e);
            }
    ...
    private ImsCallForwardInfo[] handleImsCfQueryResult(ImsCallForwardInfo[] infos) {
        ...
        if (infos == null || infos.length == 0) {
            ...
            for (int i = 0, s = infos.length; i < s; i++) {
                if ((infos[i].mCondition == ImsUtInterface.CDIV_CF_UNCONDITIONAL)
                        //SPRD: add (infos[i].mServiceClass & SERVICE_CLASS_VOICE) != 0 for bug537023
                        && (infos[i].mServiceClass & SERVICE_CLASS_VOICE) != 0) {
                    if (r != null) {
                        r.setVoiceCallForwardingFlag(1, (infos[i].mStatus == 1),
                            infos[i].mNumber);
                    }
