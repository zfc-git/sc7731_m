...
import com.android.internal.telephony.plugin.ClearCodeRetryController;
...
public final class DcTracker extends DcTrackerBase {
    ...
    private class AolObserver extends ContentObserver {
        public AolObserver () {
            super(mDataConnectionTracker);
        }
        ...
        public void onChange(boolean selfChange) {
            ...
            if (mAlwaysOnline != aol) {
                ...
                } else if (!mIsScreenOn && !isCharging()) {
    ...
    // SPRD: Add the trafficClass when activate the pdp.
    private boolean mIsSupportTrafficClass = false;

    /* SPRD: Add to clear code. @{ */
    ...
    private PendingIntent mRestartTrySetupAlarmIntent = null;
    ...
    public DcTracker(PhoneBase p) {
        ...
        // SPRD: Bug 509822 Register and unregister doesn't match
        ...
        for (ApnContext apnContext : mApnContexts.values()) {
            ...
            /* SPRD: Add for clear code @{ */
            if (mIsClearCode) {
                filter.addAction(INTENT_RETRY_CLEAR_CODE + '.' + apnContext.getApnType());
                filter.addAction(INTENT_RETRY_FROM_FAILURE + '.' + apnContext.getApnType());
            }
            /* @} */
        ...
        /* SRPD: Add for always online feature @{ */
        mConfig = DctConfig.getInstance(p.getContext());
        mAlwaysOnline = Settings.Global.getInt(mPhone.getContext().getContentResolver(),
                Settings.Global.MOBILE_DATA_ALWAYS_ONLINE, 1) == 1;
        mAolObserver = new AolObserver();
        p.getContext().getContentResolver().registerContentObserver(
                Settings.Global.getUriFor(Settings.Global.MOBILE_DATA_ALWAYS_ONLINE), true,
                mAolObserver);
        ...
        // SPRD: Add the trafficClass when activate the pdp.
        mIsSupportTrafficClass = SupportTrafficClassUtils.getInstance().isSupportTracfficClass();
        /* SPRD: Add to clear code. @{*/
        if (mIsClearCode) {
            mRetryController = new ClearCodeRetryController(mPhone, this);
        }
        /* @} */
    }
    ...
    protected void registerForAllEvents() {
         ...
         * We move the voice call handling logic to DctController, because in DSDS, the other phone
         * need to process the voice call start/end event as well. DctController is proper place
         * because it has control to all DcTrackers
    ...
    public void dispose() {
        ...
        /* SRPD: Add for always online feature @{ */
        mPhone.getContext().getContentResolver().unregisterContentObserver(mAolObserver);
        mConfig.dispose();
        /* @} */
        mApnContexts.clear();
    ...
    private boolean isDataAllowed(ApnContext apnContext) {
        ...
        if ((apnContext.getApnType().equals(PhoneConstants.APN_TYPE_DEFAULT)
            ...
            /* SPRD: Add to clear code. @{ */
            if (mIsClearCode) {
                log("isDataAllowed--mPreFailcause = "+mPreFailcause);
                return apnContext.isReady()
                    && isDataAllowed()
                    && (apnContext.getReason().equals(Phone.REASON_ROUTING_AREA_UPDATE)
                            || isDataAllowed(mPreFailcause));
            }
            /* @} */
    ...
    protected boolean isDataAllowed() {
        ...
        boolean allowed =
                    ...
                    !psRestricted &&
                    desiredPowerState &&
                    !mDeepSleep; // SPRD: Add for always online feature
        if (!allowed && DBG) {
            String reason = "";
            ...
            // SPRD: Add for always online feature
            if (mDeepSleep) reason += " - deep sleeping= true";
    ...
    private boolean trySetupData(ApnContext apnContext, ArrayList<ApnSetting> waitingApns) {
        ...
        boolean checkUserDataEnabled =
                /* SPRD:bug 491486 don't check UserDataEnable for APN_TYPE_MMS when send MMS @{*/
    ...
    private ApnSetting makeApnSetting(Cursor cursor) {
        ...
        if (mIsSupportTrafficClass) {
            apn = SupportTrafficClassUtils.getInstance()
                    .getApnSettingForTrafficClass(cursor, types);
        }
        /* @} */
        return apn;
    }
    ...
    protected void restartRadio() {
        ...
        /*SPRD: for bug 496617 @{*/
        mPhone.getServiceStateTracker().setNeedRestartRadio(true);
        /*@}*/
    ...
    private void startAlarmForReconnect(int delay, ApnContext apnContext) {
        ...
        // SPRD: Add for clear code.
        mReconnectAlarmIntent = alarmIntent;
    ...
    public void stopFailRetryAlarm() {
    ...
    public void stopRestartSetupDataAlarm() {
        if (mRestartTrySetupAlarmIntent != null) {
            mAlarmManager.cancel(mRestartTrySetupAlarmIntent);
            mRestartTrySetupAlarmIntent = null;
        }
        if (mReconnectAlarmIntent != null) {
            mAlarmManager.cancel(mReconnectAlarmIntent);
            mReconnectAlarmIntent = null;
        }
    }

    @Override
    protected void onActionIntentRetryClearCode(Intent intent) {
        // unlock data
        ...
        if (DBG) {
            log("onActionIntentRetryClearCode: mState=" + mState + " reason=" + reason +
    ...
    @Override
    protected void onActionIntentRetryFromFailure(Intent intent) {
        mAlarmIntent = null;
        String apnType = intent
                .getStringExtra(INTENT_RETRY_FROM_FAILURE_ALARM_EXTRA_TYPE);
        ApnContext apnContext = mApnContexts.get(apnType);
        if (DBG) {
            log("onActionIntentRetryFromFailure: mState=" + mState
                    + " apnType=" + apnType + " apnContext=" + apnContext
    ...
    private boolean isDataAllowed(DcFailCause fc) {
        boolean allow = true;
        log("[ClearCode] fc=" + fc + ", mFailCount=" + mFailCount + ", mClearCodeLatch=" +
                mClearCodeLatch.get());
        if (ClearCodeAndRAUUtils.getInstance().isSpecialCode(fc)) {
    ...
    private void startAlarmForRestartTrySetup(int delay, ApnContext apnContext) {
        ...
        mRestartTrySetupAlarmIntent = alarmIntent;
        apnContext.setReconnectIntent(alarmIntent);
        mAlarmManager.set(AlarmManager.ELAPSED_REALTIME_WAKEUP,
                SystemClock.elapsedRealtime() + delay, alarmIntent);
    }
    ...
    private void onRecordsLoaded() {
        ...
        /* SPRD: Bug 512692 Data connection state is not properly notified @{ */
        // SPRD telephony framework loads sim records before radio on, so when onRecordsLoaded() is
        // called, the radio state is still off, the data connection state is not properly notified.
        // @orig
    ...
    protected void onRadioOffOrNotAvailable() {
        ...
        /* SPRD: Add to clear code. @{ */
        if (mIsClearCode && ClearCodeAndRAUUtils.getInstance().isSpecialCode(mPreFailcause)) {
            // shutdown radio, so reset state
            if (mRetryController != null) {
                mRetryController.sendMessage(mRetryController.obtainMessage(
    ...
    protected void onDataSetupComplete(AsyncResult ar) {
        ...
        if (ar.exception == null) {
            ...
            /* SPRD: Add for clear code @{ */
            if (mIsClearCode && ClearCodeAndRAUUtils.getInstance().isSpecialCode(cause)) {
                // Handle special clear code
                mFailCount++;
                log("[ClearCode] mFailCount=" + mFailCount);
                if (mFailCount < MAX_RETRY) {
                    ...
                    if (is4G()) {
                        ...
                        apnContext.removeAllWaitingApns();
                    }
                }
                // We will do special retry, so stop other retry timer
                stopRestartSetupDataAlarm();
            } else if (mIsSupportTrafficClass) {
                // some other failure, reset the fail count
                mFailCount = 0;
            /* @} */
                /* SPRD: Add the trafficClass when activate the pdp. @{ */
                if (cause == DcFailCause.MISSING_UNKNOWN_APN) {
                    // Not retry when error is 27
            ...
            /* SPRD: Add for clear code @{ */
            if (!ClearCodeAndRAUUtils.getInstance().isSpecialCode(cause)) {
        ...
        if (apnContext.getWaitingApns().isEmpty()) {
            ...
            if (apnContext.getWaitingApnsPermFailCount() == 0) {
                ...
                if (ClearCodeAndRAUUtils.getInstance().isSpecialCode(cause)) {
                    ...
                    startAlarmForRetryFromFailure(ClearCodeAndRAUUtils.RETRY_FROM_FAILURE_DELAY,
                            apnContext);
                }
                /* @} */
                if (DBG) {
            ...
            if (ClearCodeAndRAUUtils.getInstance().isSpecialCode(cause)) {
                ...
                if (mFailCount < MAX_RETRY) {
                    int delay = getApnDelay();
                    if (is4G()) {
                        delay = ClearCodeAndRAUUtils.RETRY_DELAY_SHORT;
                    } else {
                        delay = ClearCodeAndRAUUtils.RETRY_DELAY_LONG;
                    }
                    log("[ClearCode] APN delay=" + delay);
    ...
    private void userNotification(DcFailCause cause) {
        ...
        mErrorDialog = ClearCodeAndRAUUtils.getInstance().getErrorDialog(cause);
    ...
    /* SPRD: Bug424898 Stop data stall alarm during call @{ */
    @Override
    protected void onVoiceCallStarted(int phoneId) {
        if (phoneId == mPhone.getPhoneId()) {
            onVoiceCallStarted();
        } else if (!isSupportDSDA()) {
            mOtherPhoneInVoiceCall = true;
            if (isConnected()) {
                if (DBG) log("Voice call started on other phone, stop polling");
                stopNetStatPoll();
                stopDataStallAlarm();
                notifyDataConnection(Phone.REASON_VOICE_CALL_STARTED);
            }
    ...
    @Override
    protected void onVoiceCallEnded(int phoneId) {
        if (phoneId == mPhone.getPhoneId()) {
            onVoiceCallEnded();
        } else {
            mOtherPhoneInVoiceCall = false;
            if (isConnected()) {
                if (!isSupportDSDA()) {
                    if (DBG) log("Voice call ended on other phone, start polling");
                    startNetStatPoll();
                    startDataStallAlarm(DATA_STALL_NOT_SUSPECTED);
                    notifyDataConnection(Phone.REASON_VOICE_CALL_ENDED);
                } else {
    ...
    public void handleMessage (Message msg) {
        ...
        switch (msg.what) {
            case DctConstants.EVENT_RECORDS_LOADED:
                /* SPRD: Read prefered APN when subinfo ready @{ */
                mIsLoaded = true;
                // onRecordsLoaded();
                /* @} */
                break;
            ...
            /* SPRD: Read prefered APN when subinfo ready @{ */
            case DctConstants.EVENT_SUBINFO_READY:
                onRecordsLoaded();
                break;
            /* @} */

            /* SPRD: Add for always online feature @{ */
            case DctConstants.EVENT_DEEP_SLEEP_CLEANUP_DATA:
    ...
    protected void onScreenOff() {
        if (!mAlwaysOnline && !isCharging()) {
            startAlarmForDeepSleep();
        }
    }

    @Override
    protected void onBatteryChanged(boolean charging) {
        if (charging) {
            cancelAlarmForDeepSleep();
