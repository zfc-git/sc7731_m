...
static int fan54015_set_value(BYTE reg, BYTE reg_bit,BYTE reg_shift, BYTE val)
{
	BYTE tmp;
	int ret = 0;
...
{
	printf("sprdchg_fan54015_start_chg\n");
	#if defined(CHG_IC_COMPAT)
	printf("charge charge ext_ic_uflag =%d\n",ext_ic_uflag);
	if(0 == ext_ic_uflag)
		return;
	#endif
...
{
	#if defined(CHG_IC_COMPAT)
	printf("stop charge ext_ic_uflag =%d\n",ext_ic_uflag);
	if(0 == ext_ic_uflag)
		return;
	#endif
...
//reg 6
	...
	ret = fan54015_set_value(FAN5405_REG_SAFETY, FAN5405_VSAFE,FAN5405_VSAFE_SHIFT, VSAFE4P38);  // VSAFE = 4.38V
	#if defined(CHG_IC_COMPAT)
	if (ret <= 0) {
		printf("==lxm==%s failed ret(%d)\n", __func__,ret);
		return;
	}
	#endif
